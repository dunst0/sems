...
struct ReconnectLegEvent : public ReliableB2BEvent
{
  ...
  enum Role
  {
    A,
    B
  } role; // reconnect as A or B leg

  void setMedia(AmB2BMedia* m, AmB2BSession::RTPRelayMode _mode)
  {
    media = m;
  ...
  ReconnectLegEvent(const std::string& tag, const AmSipRequest& relayed_invite)
      ...
      , session_tag(tag)
      , role(B) // we have relayed_invite (only in A leg) thus reconnect as
                // regular B leg
  {
    setSender(tag);
  }
  ...
  virtual ~ReconnectLegEvent()
  {
    if (media) media->releaseReference();
  }
};
...
struct ReplaceLegEvent : public ReliableB2BEvent
{
 ...
 public:
  ...
  {
    ev = new ReconnectLegEvent(tag, relayed_invite);
    ev->setMedia(m, mode);
    setSender(tag);
  }
  ...
  ReconnectLegEvent* getReconnectEvent()
  {
    ReconnectLegEvent* e = ev;
    ev                   = NULL;
    return e;
  }
  virtual ~ReplaceLegEvent()
  {
    if (ev) delete ev;
  }
};
...
struct DisconnectLegEvent : public B2BEvent
{
  ...
  DisconnectLegEvent(bool _put_remote_on_hold,
                     bool _preserve_media_session = false)
...
struct ApplyPendingUpdatesEvent : public B2BEvent
{
  ApplyPendingUpdatesEvent()
      : B2BEvent(ApplyPendingUpdatesEventId)
  {
  }
};

...
int SimpleRelayDialog::relayRequest(const AmSipRequest& req)
{
  ...
  if (keep_vias) hdrs = req.vias + hdrs;
  ...
  if (sendRequest(req.method, &req.body, hdrs, SIP_FLAGS_VERBATIM)) {
    ...
    B2BSipReplyEvent* b2b_ev =
        new B2BSipReplyEvent(error, true, req.method, getLocalTag());
...
int SimpleRelayDialog::relayReply(const AmSipReply& reply)
{
  ...
  if (transparent_dlg_id && ext_local_tag.empty() && !reply.to_tag.empty()) {
    setExtLocalTag(reply.to_tag);
  }

  if (this->reply(*uas_req, code, reason, &reply.body, hdrs,
                  SIP_FLAGS_VERBATIM)) {
    // TODO: what can we do???
    return -1;
  }
...
void SimpleRelayDialog::finalize()
{
  ...
  for (list<CCModuleInfo>::iterator i = cc_ext.begin(); i != cc_ext.end();
       ++i) {
...
void SimpleRelayDialog::onB2BReply(const AmSipReply& reply)
{
  ...
  if (reply.code >= 200) finished = true;
...
{
  if (auth_h.get()) auth_h->onSendRequest(req, flags);
}
                                   ...
                                   AmBasicSipDialog::Status old_dlg_status)
{
  ...
  B2BSipReplyEvent* b2b_ev =
      new B2BSipReplyEvent(reply, true, req.method, getLocalTag());
  b2b_ev->reply.cseq = t_req_it->second;
  if (reply.code >= 200) relayed_reqs.erase(t_req_it);
                               ...
                               const SBCCallProfile& cp)
{
  ...
  ParamReplacerCtx ctx(&cp);
  if (cp.apply_a_routing(ctx, req, *this) < 0) return -1;
                          ...
                          const AmSipRequest& req, const SBCCallProfile& cp)
{
  ...
  if (relay.first->initUAS(n_req, cp) || relay.second->initUAC(n_req, cp)) {

...
{
  void logEvent(long int timestamp, const string& id, const string& type,
                const AmArg& event)
  {
    ...
    if (NULL != MONITORING_GLOBAL_INTERFACE) {
      ...
      MONITORING_GLOBAL_INTERFACE->invoke("log", di_args, ret);
    }
#endif
  }
};
...
{
  if (log_handler.get()) {
    log_handler->logEvent(AmAppTimer::instance()->unix_clock.get(), id, type,
                          event);
  }
}
...
void _SBCEventLog::logCallStart(const AmSipRequest& req,
                                ...
                                const string& to_remote_ua, int code,
                                const string& reason)
{
  size_t      end;
  AmArg       start_event;
...
void _SBCEventLog::logCallEnd(const AmSipRequest& req, const string& local_tag,
...
void _SBCEventLog::logCallEnd(const AmBasicSipDialog* dlg, const string& reason,
                              struct timeval* tv)
{
  AmArg end_event;

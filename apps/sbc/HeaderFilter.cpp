...
FilterType String2FilterType(const char* ft)
{
  ...
  if (!strcasecmp(ft, "transparent")) return Transparent;

  if (!strcasecmp(ft, "whitelist")) return Whitelist;

  if (!strcasecmp(ft, "blacklist")) return Blacklist;
                ...
                bool keep_transparent_entry)
{
  ...
  if (!keep_transparent_entry && hf.filter_type == Transparent) return true;
  ...
  for (; p < hdr.length() && st != ST_LF && st != ST_CRLF; p++) {
    switch (st) {
      ...
      case H_VALUE:
        ...
        if (st == ST_CR || st == ST_LF) val_end = p;
        break;
...
{
  if (!hdrs.length() || !filter_list.size()) return 0;
  ...
  for (vector<FilterEntry>::const_iterator fe = filter_list.begin();
    ...
    if (!isActiveFilter(f_type)) continue;
    ...
    while (start_pos < hdrs.length()) {
      ...
      int    res;
      if ((res = skip_header(hdrs, start_pos, name_end, val_begin, val_end,
                             hdr_end))
          != 0) {
        return res;
      }
      string hdr_name = hdrs.substr(start_pos, name_end - start_pos);
      std::transform(hdr_name.begin(), hdr_name.end(), hdr_name.begin(),
                     ::tolower);
      bool erase = false;
      if (f_type == Whitelist) {

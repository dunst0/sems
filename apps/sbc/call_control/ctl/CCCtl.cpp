...
class CCCtlFactory : public AmDynInvokeFactory
{
 public:
  ...
  AmDynInvoke* getInstance() { return CCCtl::instance(); }

  int onLoad()
  {
    if (CCCtl::instance()->onLoad()) return -1;
...
{
  if (!_instance) _instance = new CCCtl();
  return _instance;
}
...
#define ENABLE_IN_CALL_PROFILE(cfgparam, member)                               \
  if (values.hasMember(cfgparam)) {                                            \
    call_profile->member = string(values[cfgparam].asCStr()) == "yes";         \
  ...
  if (values.hasMember("headerfilter")) {
    ...
    if (Undefined != t) {
      ...
      string hl;
      if (values.hasMember("header_list")) hl = values["header_list"].asCStr();

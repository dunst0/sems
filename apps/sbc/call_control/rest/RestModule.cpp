...
class RestModuleFactory : public AmDynInvokeFactory
{
 public:
  ...
  AmDynInvoke* getInstance() { return RestModule::instance(); }

  int onLoad()
  {
    if (RestModule::instance()->onLoad()) return -1;
...
{
  if (!_instance) _instance = new RestModule();
  return _instance;
}
...
static RestParams::Format getFormat(const AmArg&       values,
                                    RestParams::Format _default)
{
...
static void setHeaderFilter(SBCCallProfile* call_profile, const string& type,
                            const string& list)
{
                       ...
                       AmArg& res)
{
  ...
  try {
    ...
    if (!values.hasMember("url"))
      throw string(
          "configuration error: url must be configured for REST queries\n");
    ...
    if (params.retrieve(url, getFormat(values, RestParams::TEXT))) {
      ...
      params.getIfSet("append_headers",
                      call_profile->append_headers); // CRLF is handled in SBC

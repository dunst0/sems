    ...
    , local_media_connected(false)
{
  ...
  if (it != values.end()) startDiagName = it->second;

  it                                = values.find(DSM_SBC_CCVAR_APP_BUNDLE);
  if (it != values.end()) appBundle = it->second;

  if (startDiagName.empty()) {
    throw string("DSM SBC call control " DSM_SBC_CCVAR_START_DIAG
                 " parameter not set (see call profile)'");
  }

  map<string, string> config_vars;
  bool SetParamVariables; // unused

  if (!DSMFactory::instance()->addScriptDiagsToEngine(
          appBundle, &engine, config_vars, SetParamVariables)) {
    ERROR("initializing call with DSM app bundle '%s'\n", appBundle.c_str());
    throw string("initializing call with DSM app bundle '" + appBundle);
  }

  for (map<string, string>::const_iterator it = config_vars.begin();
       it != config_vars.end(); it++)
    var["config." + it->first] = it->second;

  // overwrite config. variables with cc-instance variables (map::insert doesn't
  // overwrite)
...
{
  DBG("SBCDSMInstance::~SBCDSMInstance()\n");
  for (std::set<DSMDisposable*>::iterator it = gc_trash.begin();
       it != gc_trash.end(); it++)
    delete *it;

  for (vector<AmAudio*>::iterator it = audiofiles.begin();
...
CCChainProcessing
SBCDSMInstance::onInitialInvite(SBCCallLeg*                 call,
...
void extractRequestParameters(VarMapT& event_params, AVarMapT& avar,
                              DSMSipRequest* request)
{
  ...
  if (NULL != request) {
    ...
    for (vector<string>::iterator it = hdrs.begin(); it != hdrs.end(); it++) {
      ...
      size_t p1 = p;
      if (++p >= it->size()) continue;
      while (p < it->size() && ((*it)[p] == ' ' || (*it)[p] == '\t')) p++;
...
void clearRequestParameters(AVarMapT& avar) { avar.erase(DSM_AVAR_REQUEST); }

void extractReplyParameters(VarMapT& event_params, AVarMapT& avar,
                            DSMSipReply* reply)
{
  ...
  for (list<AmSipHeader>::const_iterator it =
           reply->reply->unparsed_headers.begin();
...
void clearReplyParameters(AVarMapT& avar) { avar.erase(DSM_AVAR_REPLY); }

void SBCDSMInstance::onStateChange(SBCCallLeg*                       call,
...
/** called from A/B leg when in-dialog request comes in */
CCChainProcessing SBCDSMInstance::onInDialogRequest(SBCCallLeg*         call,
...
CCChainProcessing SBCDSMInstance::onInDialogReply(SBCCallLeg*       call,
...
CCChainProcessing SBCDSMInstance::onEvent(SBCCallLeg* call, AmEvent* event)
{
  ...
  if (b2b_ev && b2b_ev->ev_type == B2BEvent::B2BApplication) {
    ...
    if (b2b_ev->params[DSM_SBC_PARAM_PROCESSED] == DSM_TRUE) {
      ...
      if (NULL != rel_b2b_ev) {
        ...
        DBG("possible script writer error: marked #processed on non-reliable "
            "B2BEvent");
      }
    }
  ...
  AmPlaylistSeparatorEvent* sep_ev =
  ...
  if (audio_event
    ...
    map<string, string> params;
    params["type"] =
        audio_event->event_id == AmAudioEvent::cleared ? "cleared" : "noAudio";
...
CCChainProcessing SBCDSMInstance::onDtmf(SBCCallLeg* call, int event,
                                         int duration)
{
  ...
  if (params[DSM_SBC_PARAM_STOP_PROCESSING] == DSM_TRUE) return StopProcessing;
  return ContinueProcessing;
}

/** @return whether to continue processing */
CCChainProcessing SBCDSMInstance::onBLegRefused(SBCCallLeg*       call,
                                                const AmSipReply& reply)
{
...
CCChainProcessing SBCDSMInstance::resumeHeld(SBCCallLeg* call,
                                             bool        send_reinvite)
{
...
CCChainProcessing SBCDSMInstance::createHoldRequest(SBCCallLeg* call,
                                                    AmSdp&      sdp)
{
...
CCChainProcessing SBCDSMInstance::handleHoldReply(SBCCallLeg* call,
                                                  bool        succeeded)
{
...
void SBCDSMInstance::resetDummySession(SimpleRelayDialog* relay)
{
  if (NULL == dummy_session.get()) {
    ...
    // TODO: initialize stuff from relay dialog in dummy session to be visible
    // in DSM
...
{
  if (NULL == playlist.get()) playlist.reset(new AmPlaylist(call));
...
bool SBCDSMInstance::init(SBCCallProfile& profile, SimpleRelayDialog* relay)
{
  ...
  engine.runEvent(dummy_session.get(), this, DSMCondition::RelayInit,
                  &event_params);
...
void SBCDSMInstance::initUAC(SBCCallProfile& profile, SimpleRelayDialog* relay,
                             const AmSipRequest& req)
{
  ...
  extractRequestParameters(event_params, avar, &sip_req);
  engine.runEvent(dummy_session.get(), this, DSMCondition::RelayInitUAC,
                  &event_params);
  clearRequestParameters(avar);
  avar.erase(DSM_SBC_AVAR_PROFILE);
}

void SBCDSMInstance::initUAS(SBCCallProfile& profile, SimpleRelayDialog* relay,
                             const AmSipRequest& req)
{
  ...
  DSMSipRequest sip_req(&req);
  extractRequestParameters(event_params, avar, &sip_req);
  engine.runEvent(dummy_session.get(), this, DSMCondition::RelayInitUAS,
                  &event_params);
  clearRequestParameters(avar);
  avar.erase(DSM_SBC_AVAR_PROFILE);
}
...
void SBCDSMInstance::finalize(SBCCallProfile& profile, SimpleRelayDialog* relay)
{
  ...
  avar[DSM_SBC_AVAR_PROFILE]  = AmArg(&profile);
  engine.runEvent(dummy_session.get(), this, DSMCondition::RelayFinalize,
                  &event_params);
  avar.erase(DSM_SBC_AVAR_PROFILE);
}

void SBCDSMInstance::onSipRequest(SBCCallProfile&     profile,
                                  ...
                                  const AmSipRequest& req)
{
  ...
  DSMSipRequest sip_req(&req);
  extractRequestParameters(event_params, avar, &sip_req);
  engine.runEvent(dummy_session.get(), this, DSMCondition::RelayOnSipRequest,
                  &event_params);
  clearRequestParameters(avar);
  avar.erase(DSM_SBC_AVAR_PROFILE);
}

void SBCDSMInstance::onSipReply(SBCCallProfile&          profile,
                                SimpleRelayDialog*       relay,
                                const AmSipRequest&      req,
                                const AmSipReply&        reply,
                                AmBasicSipDialog::Status old_dlg_status)
{
  ...
  extractRequestParameters(event_params, avar, &sip_req);
  DSMSipReply dsm_reply(&reply);
  extractReplyParameters(event_params, avar,
                         &dsm_reply); // TODO: shadows request
  event_params["old_dlg_status"] =
      AmBasicSipDialog::getStatusStr(old_dlg_status);
  engine.runEvent(dummy_session.get(), this, DSMCondition::RelayOnSipReply,
                  &event_params);
  clearReplyParameters(avar);
  clearRequestParameters(avar);
  avar.erase(DSM_SBC_AVAR_PROFILE);
...
void SBCDSMInstance::onB2BRequest(SBCCallProfile&     profile,
                                  SimpleRelayDialog*  relay,
                                  const AmSipRequest& req)
{
  ...
  DSMSipRequest sip_req(&req);
  extractRequestParameters(event_params, avar, &sip_req);
  engine.runEvent(dummy_session.get(), this, DSMCondition::RelayOnB2BRequest,
                  &event_params);
  clearRequestParameters(avar);
  avar.erase(DSM_SBC_AVAR_PROFILE);
}

void SBCDSMInstance::onB2BReply(SBCCallProfile&    profile,
                                SimpleRelayDialog* relay,
                                const AmSipReply&  reply)
{
  ...
  DSMSipReply dsm_reply(&reply);
  extractReplyParameters(event_params, avar, &dsm_reply);
  engine.runEvent(dummy_session.get(), this, DSMCondition::RelayOnB2BReply,
                  &event_params);
  clearReplyParameters(avar);
  avar.erase(DSM_SBC_AVAR_PROFILE);
}
...
void SBCDSMInstance::transferOwnership(DSMDisposable* d) { gc_trash.insert(d); }

void SBCDSMInstance::releaseOwnership(DSMDisposable* d) { gc_trash.erase(d); }
...
void SBCDSMInstance::playRingtone(int length, int on, int off, int f, int f2,
                                  bool front)
{
...
void SBCDSMInstance::connectMedia()
{
  ...
  if (NULL == media) {
    ...
    media = new AmB2BMedia(call->isALeg() ? call : NULL,
                           call->isALeg() ? NULL : call);

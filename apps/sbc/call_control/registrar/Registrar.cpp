...
class CCRegistrarFactory : public AmDynInvokeFactory
{
 public:
  ...
  AmDynInvoke* getInstance() { return CCRegistrar::instance(); }

  int onLoad()
  {
    if (CCRegistrar::instance()->onLoad()) return -1;
...
{
  if (!_instance) _instance = new CCRegistrar();
  return _instance;
}
...
void CCRegistrar::invoke(const string& method, const AmArg& args, AmArg& ret)
{
  ...
  if (method == "start") {
    ...
    const AmSipRequest* msg = dynamic_cast<const AmSipRequest*>(
...
bool retarget(const string& r_uri, const AmArg& values,
              SBCCallProfile* call_profile)
{
  bool nat_handling = true; // default
  bool sticky_iface = true; // default
  if (values.hasMember("nat_handling")
      && string(values["nat_handling"].asCStr()) == "no") {
    nat_handling = false;
  }
  if (values.hasMember("sticky_iface")
      && string(values["sticky_iface"].asCStr()) == "no") {
    sticky_iface = false;
  }
...
void CCRegistrar::start(const string& cc_name, const string& ltag,
                        ...
                        AmArg& res, const AmSipRequest* req)
{
  if (NULL == req) REFUSE_WITH_404;

  if ((req->method == "INVITE")
      && (retarget(req->r_uri, values, call_profile))) {
    return;
  }
...
void CCRegistrar::route(const string& cc_name, SBCCallProfile* call_profile,
                        const AmSipRequest* ood_req, const AmArg& values,
                        AmArg& res)
{
  DBG("CCRegistrar: route '%s %s'\n", ood_req->method.c_str(),
      ood_req->r_uri.c_str());
  ...
  if (ood_req->method == "REGISTER") {
    ...
    bool replied =
        RegisterCache::instance()->saveSingleContact(reg_cache_ctx, *ood_req);

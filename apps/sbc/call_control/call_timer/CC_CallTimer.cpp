...
class CallTimerFactory : public AmDynInvokeFactory
{
 public:
  ...
  AmDynInvoke* getInstance() { return CallTimer::instance(); }

  int onLoad()
  {
    if (CallTimer::instance()->onLoad()) return -1;
...
void CallTimer::invoke(const string& method, const AmArg& args, AmArg& ret)
{
  ...
  if (method == "start") {
    start(args[CC_API_PARAMS_CFGVALUES], args[CC_API_PARAMS_TIMERID].asInt(),
          ret);
  }

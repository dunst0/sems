...
class CCTemplateFactory : public AmDynInvokeFactory
{
 public:
  ...
  AmDynInvoke* getInstance() { return CCTemplate::instance(); }

  int onLoad()
  {
    if (CCTemplate::instance()->onLoad()) return -1;
...
CCChainProcessing
CCTemplate::onInitialInvite(SBCCallLeg*                 call,
...
void CCTemplate::onStateChange(SBCCallLeg*                       call,
...
CCChainProcessing CCTemplate::onBLegRefused(SBCCallLeg*       call,
...
CCChainProcessing CCTemplate::onInDialogRequest(SBCCallLeg*         call,
...
CCChainProcessing CCTemplate::onInDialogReply(SBCCallLeg*       call,
                                              const AmSipReply& reply)
{
...
CCChainProcessing CCTemplate::onDtmf(SBCCallLeg* call, int event, int duration)
{
  DBG("ExtCC: onDtmf(%i;%i) - call instance: '%p' isAleg==%s\n", event,
      duration, call, call->isALeg() ? "true" : "false");
  return ContinueProcessing;
}
...
void CCTemplate::finalize(void* user_data) { DBG("finalize simple relay\n"); }
...
void CCTemplate::onSipReply(const AmSipRequest& req, const AmSipReply& reply,

...
class PrepaidXMLRPCFactory : public AmDynInvokeFactory
{
 public:
  ...
  AmDynInvoke* getInstance() { return PrepaidXMLRPC::instance(); }

  int onLoad()
  {
    if (PrepaidXMLRPC::instance()->onLoad()) return -1;
...
{
  if (!_instance) _instance = new PrepaidXMLRPC();
  return _instance;
}
...
void PrepaidXMLRPC::connect(const string& cc_name, const string& ltag,
                            SBCCallProfile* call_profile,
                            const string& other_tag, int connect_ts_sec,
                            int connect_ts_usec)
{
...
void PrepaidXMLRPC::end(const string& cc_name, const string& ltag,
                        SBCCallProfile* call_profile, int start_ts_sec,
                        int start_ts_usec, int connect_ts_sec,
                        int connect_ts_usec, int end_ts_sec, int end_ts_usec)
{
  ...
  SBCVarMapIterator vars_it =
      call_profile->cc_vars.find(cc_name + "::" + SBCVAR_PREPAID_XMLRPC_UUID);

...
void RedisConnectionPool::run()
{
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (!m_failed_connections) break;

      // add connections until error occurs
      redisContext* context =
          redisConnect((char*) redis_server.c_str(), redis_port);
      if (!context->err) {
        ...
        DBG("connection to %s%u failed: '%s'\n", redis_server.c_str(),
            redis_port, context->errstr);
        redisFree(context);
        if (retry_timers.size()) {
          DBG("waiting for retry %u ms (index %u)\n", retry_timers[retry_index],
              retry_index);
          usleep(retry_timers[retry_index] * 1000);
          if (retry_index < retry_timers.size() - 1) retry_index++;
        }
        else {

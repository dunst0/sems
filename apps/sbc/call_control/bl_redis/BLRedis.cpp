...
class CCBLRedisFactory : public AmDynInvokeFactory
{
 public:
  ...
  AmDynInvoke* getInstance() { return CCBLRedis::instance(); }

  int onLoad()
  {
    if (CCBLRedis::instance()->onLoad()) return -1;
...
{
  if (!_instance) _instance = new CCBLRedis();
  return _instance;
}
...
int CCBLRedis::onLoad()
{
  ...
  if (cfg.loadPluginConf(MOD_NAME)) {
    ...
    redis_connections =
        cfg.getParameter("redis_connections", redis_connections);
    redis_max_conn_wait =
        cfg.getParameter("redis_max_conn_wait", redis_max_conn_wait);
    full_logging = cfg.getParameter("redis_full_logging", "no") == "yes";

    pass_on_bl_unavailable =
        cfg.getParameter("pass_on_bl_unavailable", "no") == "yes";
  }
  ...
  for (std::vector<string>::iterator it = timeouts_v.begin();
       it != timeouts_v.end(); it++) {
    int r;
    if (!str2int(*it, r)) {
  ...
  connection_pool.set_config(redis_server, i_redis_port, reconnect_timers,
                             i_redis_max_conn_wait);
...
void CCBLRedis::invoke(const string& method, const AmArg& args, AmArg& ret)
{
  ...
  if (method == "start") {
    ...
    start(args[CC_API_PARAMS_CC_NAMESPACE].asCStr(),
          ...
          args[CC_API_PARAMS_CFGVALUES], args[CC_API_PARAMS_TIMERID].asInt(),
          ret);
  }
...
int CCBLRedis::handle_redis_reply(redisContext* redis_context,
                                  redisReply* reply, bool& hit)
{
  hit = false;
  ...
  if (!reply) {
    switch (redis_context->err) {
      ...
      case REDIS_ERR_PROTOCOL:
        ...
        return RWT_E_CONNECTION;
      case REDIS_ERR_OTHER: return RWT_E_CONNECTION;
    }
  }
      ...
      for (size_t i = 0; i < reply->elements; i++) {
        switch (reply->element[i]->type) {
          ...
          case REDIS_REPLY_STRING:
            ...
            break;
          default:
            ERROR("unknown REDIS reply %d!", reply->element[i]->type);
            break;
        }
      }
    }; break;
...
void CCBLRedis::start(const string& cc_name, const string& ltag,
                      SBCCallProfile* call_profile, int start_ts_sec,
                      int start_ts_usec, const AmArg& values, int timer_id,
                      AmArg& res)
{
  // start code here
  res.push(AmArg());
...
#define MAX_ARGV_ITEMS 20
  ...
  if (!values.hasMember("argc") || str2i(values["argc"].asCStr(), argv_max)
      || (!argv_max)) {
  ...
  for (; retries < max_retries; retries++) {
    ...
    redisReply* reply = (redisReply*) redisCommandArgv(
        redis_context, argv_index, argv, argvlen);
...
void CCBLRedis::connect(const string& cc_name, const string& ltag,
                        SBCCallProfile* call_profile, const string& other_tag,
...
void CCBLRedis::end(const string& cc_name, const string& ltag,
                    SBCCallProfile* call_profile, int end_ts_sec,
                    int end_ts_usec)
{
  // end code here
}

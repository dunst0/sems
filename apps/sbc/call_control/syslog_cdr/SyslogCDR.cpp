...
class SyslogCDRFactory : public AmDynInvokeFactory
{
 public:
  ...
  AmDynInvoke* getInstance() { return SyslogCDR::instance(); }

  int onLoad()
  {
    DBG(" syslog CSV CDR generation loaded.\n");

    if (SyslogCDR::instance()->onLoad()) return -1;
...
{
  if (!_instance) _instance = new SyslogCDR();
  return _instance;
}
...
int SyslogCDR::onLoad()
{
  ...
  syslog_prefix = cfg.hasParameter("cdr_prefix")
                      ? cfg.getParameter("cdr_prefix")
                      : syslog_prefix;

  level =
      cfg.hasParameter("loglevel") ? cfg.getParameterInt("loglevel") : level;
  ...
  quoting_enabled = cfg.hasParameter("quoting_enabled")
                        ? cfg.getParameter("quoting_enabled") == "yes"
                        : quoting_enabled;
...
string timeString(time_t tv_sec)
{
  ...
  char      outstr[200];
  struct tm tmp;
  if (!localtime_r(&tv_sec, &tmp)
      || strftime(outstr, sizeof(outstr), "%F %T", &tmp) == 0) {
...
string getTimeDiffString(int from_ts_sec, int from_ts_usec, int to_ts_sec,
                         int to_ts_usec, bool ms_precision)
{
  string res;
  ...
  if (ms_precision) {
    ...
    if (diff.tv_usec >= 500000) diff.tv_sec++;
...
void SyslogCDR::end(const string& ltag, SBCCallProfile* call_profile,
                    int start_ts_sec, int start_ts_usec, int connect_ts_sec,
                    int connect_ts_usec, int end_ts_sec, int end_ts_usec)
{
  if (!call_profile) return;
  ...
  if (vars_it != call_profile->cc_vars.end()) {
    ...
    if (!values.hasMember(HANGUP_CAUSE)) {
      ...
      if (i != call_profile->cc_vars.end())
        values[HANGUP_INITIATOR] = i->second;
    }
  }

  if (cdr_format.size()) {
    for (vector<string>::iterator it = cdr_format.begin();
         it != cdr_format.end(); it++) {
      if (it->size() && (*it)[0] == '$') {
        if (*it == "$ltag") {
          cdr += csv_quote(ltag) + ",";
        }
          ...
          cdr += csv_quote(int2str(connect_ts_sec) + "."
                           + int2str(connect_ts_usec))
                 + ",";
        }
        else if (*it == "$end_ts") {
          cdr +=
  ...
  if (cdr.size() && cdr[cdr.size() - 1] == ',') cdr.erase(cdr.size() - 1, 1);
...
void SyslogCDR::onStateChange(SBCCallLeg*                       call,
                              const CallLeg::StatusChangeCause& cause)
{
  ...
  if ((s == CallLeg::Connected || s == CallLeg::Disconnected)
      && !have_disposition) {
    switch (cause.reason) {
  ...
  if (s == CallLeg::Disconnected && answered) {
    switch (cause.reason) {
      ...
      case CallLeg::StatusChangeCause::Other:
        if (cause.param.desc)
          cdr[HANGUP_CAUSE] = cause.param.desc;
        else
    ...
    if (!other.empty())
      AmSessionContainer::instance()->postEvent(
          other, new HangupCause(cdr[HANGUP_CAUSE].asCStr(),
                                 cdr.hasMember(HANGUP_INITIATOR)
                                     ? cdr[HANGUP_INITIATOR].asCStr()
                                     : ""));
  }
}
...
CCChainProcessing SyslogCDR::onEvent(SBCCallLeg* call, AmEvent* e)
{
  ...
  if (e->event_id == B2BSipReply && // replies received in our peer leg
      call->isALeg()
      && call->getCallStatus() != CallLeg::Connected) // just replies before we
                                                      // get connected, then no
                                                      // failover is expected
  {
    ...
    if (ev) {
      ...
      if (cdr) {
        ...
        ERROR("can't update CDR variables with destination IP\n");
    }
  }

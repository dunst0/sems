...
class PrepaidFactory : public AmDynInvokeFactory
{
 public:
  ...
  AmDynInvoke* getInstance() { return Prepaid::instance(); }

  int onLoad()
  {
    if (Prepaid::instance()->onLoad()) return -1;
...
{
  if (!_instance) _instance = new Prepaid();
  return _instance;
}
...
void Prepaid::invoke(const string& method, const AmArg& args, AmArg& ret)
{
  ...
  if (method == "start") {
    ...
    bool found;
    ret.push(subtractCredit(args.get(0).asCStr(), args.get(1).asInt(), found));
    ret.push(found);
  }
  else if (method == "addCredit") {
    assertArgCStr(args.get(0));
    assertArgInt(args.get(1));
    ret.push(addCredit(args.get(0).asCStr(), args.get(1).asInt()));
  }
  else if (method == "setCredit") {
    assertArgCStr(args.get(0));
    assertArgInt(args.get(1));
    ret.push(setCredit(args.get(0).asCStr(), args.get(1).asInt()));
  }
...
void Prepaid::connect(const string& cc_name, const string& ltag,
...
void Prepaid::end(const string& cc_name, const string& ltag,
                  SBCCallProfile* call_profile, int start_ts_sec,
                  ...
                  int end_ts_sec, int end_ts_usec)
{
  ...
  SBCVarMapIterator vars_it =
      call_profile->cc_vars.find(cc_name + "::" + SBCVAR_PREPAID_UUID);
  ...
  if (vars_it == call_profile->cc_vars.end() || !isArgCStr(vars_it->second)) {
    ERROR("internal: could not find UUID for call '%s' - "
          ...
          ltag.c_str(), start_ts_sec, start_ts_usec, connect_ts_sec,
          connect_ts_usec, end_ts_sec, end_ts_usec);
    return;
  }

...
SBCFactory::~SBCFactory() { RegisterCache::dispose(); }
...
int SBCFactory::onLoad()
{
  ...
  for (vector<string>::iterator it = profiles_names.begin();
    ...
    string profile_file_name =
        AmConfig::ModConfigPath + *it + ".sbcprofile.conf";
  ...
  for (vector<string>::iterator it = active_profile.begin();
    ...
    active_profile_s += *it;
    if (it != active_profile.end() - 1) active_profile_s += ", ";
  }

  INFO("SBC: active profile: '%s'\n", active_profile_s.c_str());

  vector<string> regex_maps = explode(cfg.getParameter("regex_maps"), ",");
  for (vector<string>::iterator it = regex_maps.begin(); it != regex_maps.end();
       it++) {
  ...
  subnot_processor.addThreads(
      cfg.getParameterInt("out_of_dialog_threads", DEFAULT_OOD_THREADS));
                                ...
                                const map<string, string>& app_params)
{
  ...
  if (call_profile.auth_aleg_enabled) {
    ...
    if (NULL == uac_auth_f) {
      INFO("uac_auth module not loaded. uac auth for caller session NOT "
           "enabled.\n");
    }
    else {
...
void oodHandlingTerminated(const AmSipRequest&   req,
                           vector<AmDynInvoke*>& cc_modules,
                           SBCCallProfile&       call_profile)
{
  for (vector<AmDynInvoke*>::iterator m = cc_modules.begin();
       m != cc_modules.end(); ++m) {
    AmArg args, ret;
...
void SBCFactory::onOoDRequest(const AmSipRequest& req)
{
  ...
  if (req.method == SIP_METH_REGISTER) {
    ...
    relay =
        simpleRelayCreator->createSubscriptionRelay(call_profile, cc_modules);
  }
  else {
  ...
  if (SBCSimpleRelay::start(relay, req, call_profile)) {
    AmSipDialog::reply_error(req, 500, SIP_REPLY_SERVER_INTERNAL_ERROR, "",
                             call_profile.log_sip ? call_profile.get_logger(req)
                                                  : NULL);
    delete relay.first;
    delete relay.second;
  }
...
void SBCFactory::reloadProfiles(const AmArg& args, AmArg& ret)
{
  ...
  for (std::map<string, SBCCallProfile>::iterator it = call_profiles.begin();
    ...
    if (!new_call_profiles[it->first].readFromConfiguration(
            it->first, it->second.profile_file)) {
      ERROR("reading call profile file '%s'\n",
            it->second.profile_file.c_str());
      res = "Error reading call profile for " + it->first + " from "
            + it->second.profile_file + +"; no profiles reloaded";
      failed = true;
      break;
    }
    AmArg p;
...
void SBCFactory::loadProfile(const AmArg& args, AmArg& ret)
{
  ...
  SBCCallProfile cp;
  if (!cp.readFromConfiguration(args[0]["name"].asCStr(),
                                args[0]["path"].asCStr())) {
    ret.push(500);
    ret.push("Error reading sbc call profile for "
             + string(args[0]["name"].asCStr()) + " from file "
             + string(args[0]["path"].asCStr()));
    return;
  }
...
void SBCFactory::getRegexMapNames(const AmArg& args, AmArg& ret)
{
  ...
  for (vector<string>::iterator it = reg_names.begin(); it != reg_names.end();
       it++) {
...
void SBCFactory::setRegexMap(const AmArg& args, AmArg& ret)
{
  if (!args[0].hasMember("name") || !args[0].hasMember("file")
    ...
    ret.push(400);
    ret.push(
        "Parameters error: expected ['name': <name>, 'file': <file name>]");
    return;
  }
        ...
        switch (ret[i][SBC_CC_ACTION].asInt()) {
          ...
          case SBC_CC_REFUSE_ACTION: {
            ...
            AmBasicSipDialog::reply_error(
                req, ret[i][SBC_CC_REFUSE_CODE].asInt(),
                ret[i][SBC_CC_REFUSE_REASON].asCStr(), headers,

...
class CallLeg : public AmB2BSession
{
 public:
  ...
  enum CallStatus
  {
    ...
    NoReply, //< there is at least one call leg we are connected to but without
             // any response
    Ringing, //< this leg or one of legs we are connected to rings
    Connected,    //< there is exactly one call leg we are connected to, in this
                  // case AmB2BSession::other_id holds the other leg id
    Disconnecting //< we were connected and now going to be disconnected
                  //(waiting for reINVITE reply for example)
  };
 ...
 private:
   ...
   * until first 2xx response which moves the A leg to Connected state and
   * terminates all other B legs.
   *
  ...
  // resume held call directly; returns CSeq of unhold request or -1 if no
  // request was sent out (either error or not on hold)
  int resumeHeldImpl();
  ...
  {
    return !(dlg->getUACInvTransPending() || dlg->getUASPendingInv());
  }
 ...
 public:
  ...
  virtual void disconnect(bool hold_remote,
                          bool preserve_media_session = false);
  ...
  void addCallee(const std::string& session_tag, const std::string& hdrs)
  {
    addExistingCallee(session_tag,
                      new ReconnectLegEvent(
                          a_leg ? ReconnectLegEvent::B : ReconnectLegEvent::A,
  ...
  int readStreams(unsigned long long int ts, unsigned char* buffer);
  int writeStreams(unsigned long long int ts, unsigned char* buffer);

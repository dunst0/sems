                                  ...
                                  vector<AmUriParser>& cv)
{
  ...
  if (parse_nameaddr_list(contact_list, contacts.c_str(), contacts.length())
      < 0) {
...
int RegisterDialog::replyFromCache(const AmSipRequest& req)
{
  ...
  for (map<string, AmUriParser>::iterator contact_it = alias_map.begin();
    ...
    if (contact_it != alias_map.begin()) contact_hdr += ", ";
...
int RegisterDialog::fixUacContacts(const AmSipRequest& req)
{
  ...
  if (!expires.empty()) {
    if (str2i(expires, requested_expires)) {
      reply_error(req, 400, "Bad Request", "Warning: Malformed expires\r\n",
                  logger);
      return -1;
    }

    if (star_contact && (requested_expires != 0)) {
      reply_error(req, 400, "Bad Request", "Warning: Expires not equal 0\r\n",
                  logger);
      return -1;
    }
  }
  ...
  if (!star_contact) {
    ...
    for (vector<AmUriParser>::iterator contact_it = uac_contacts.begin();
      ...
      if (!reg_cache->getAlias(aor, uri, req.remote_ip, reg_binding)) {
        ...
        reg_binding.alias =
            _RegisterCache::compute_alias_hash(aor, uri, req.remote_ip);
      }
                                        ...
                                        const SBCCallProfile& cp)
{
  ...
  for (unsigned int i = 0; i < uac_contacts.size(); i++) {
    if (contact_hiding) {
      uac_contacts[i].uri_user = encodeUsername(uac_contacts[i], req, cp, ctx);
    }
    else if (!reg_caching) {
...
int RegisterDialog::removeTransport(AmUriParser& uri)
{
  ...
  if (parse_gen_params(&uri_params, &c, old_params.length(), 0) < 0) {
    DBG("could not parse Contact URI parameters: '%s'", uri.uri_param.c_str());
...
int RegisterDialog::initUAC(const AmSipRequest& req, const SBCCallProfile& cp)
{
  ...
  if (fixUacContacts(req) < 0) return -1;

  if (SimpleRelayDialog::initUAC(req, cp) < 0) return -1;
                                ...
                                AmBasicSipDialog::Status old_dlg_status)
{
  ...
  if (contact_hiding || reg_caching) {
    ...
    for (vector<AmUriParser>::iterator it = uas_contacts.begin();
         it != uas_contacts.end(); it++) {
      if (contact_hiding) {
        decodeUsername(it->uri_user, *it);
      }
      else if (reg_caching) {
        map<string, AmUriParser>::iterator alias_it =
            alias_map.find(it->uri_user);
    ...
    DBG("reg_caching=%i; alias_map.empty()=%i", reg_caching, alias_map.empty());
    ...
    if (reg_caching && !alias_map.empty()) {
      for (map<string, AmUriParser>::iterator alias_it = alias_map.begin();
        ...
        DBG("removing '%s' -> '%s'", contact_uri.c_str(),
...
bool RegisterDialog::decodeUsername(const string& encoded_user,
                                    AmUriParser&  uri)
{
  ...
  if (!vars.hasMember("u") || !isArgCStr(vars["u"]) || !vars.hasMember("h")
      || !isArgCStr(vars["h"]) || !vars.hasMember("p")
      || !isArgCStr(vars["p"])) {

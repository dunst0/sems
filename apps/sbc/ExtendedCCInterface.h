...
struct InitialInviteHandlerParams
{
  ...
  InitialInviteHandlerParams(const string& to, const string& ruri,
      ...
      , modified_invite(modified)
  {
  }
};

enum CCChainProcessing
{
  ContinueProcessing,
  StopProcessing
};
...
class ExtendedCCInterface
{
 ...
 public:
  ...
  /** called when the call leg is being destroyed, useful to cleanup used
   * resources */
  virtual void onDestroyLeg(SBCCallLeg* call) {}

  /** one of existing B legs is refused,
   * handle redirect here or do serial fork or ... */
  virtual CCChainProcessing onBLegRefused(SBCCallLeg*       call,
  ...
  virtual CCChainProcessing onInitialInvite(SBCCallLeg*                 call,
  ...
  virtual CCChainProcessing onInDialogRequest(SBCCallLeg*         call,
                                              const AmSipRequest& req)
  {
    return ContinueProcessing;
  }

  virtual CCChainProcessing onInDialogReply(SBCCallLeg*       call,
                                            const AmSipReply& reply)
  {
    return ContinueProcessing;
  }
  ...
  virtual CCChainProcessing onDtmf(SBCCallLeg* call, int event, int duration)
  {
    return ContinueProcessing;
  };
  ...
  virtual bool init(SBCCallProfile& profile, SimpleRelayDialog* relay,
                    void*& user_data)
  {
    return true;
  }

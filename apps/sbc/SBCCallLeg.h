    ...
    , public CredentialHolder
{
  ...
  int cc_timer_id;
  int ext_cc_timer_id; // for assigning IDs to timers through "extended CC
                       // interface"
  ...
  void alterHoldRequestImpl(
      AmSdp& sdp); // do the SDP update (called by alterHoldRequest)
 ...
 public:
  ...
  int startTimer(double timeout)
  {
    setTimer(ext_cc_timer_id, timeout);
    return ext_cc_timer_id++;
  }
 ...
 protected:
  ...
  {
    return (get_random() % call_profile.max_491_retry_time) / 1000.0;
  }
};

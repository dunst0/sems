                                           ...
                                           const string  profile_file_name)
{
  ...
  aleg_force_outbound_proxy =
      cfg.getParameter("aleg_force_outbound_proxy") == "yes";
  ...
  allow_subless_notify =
      cfg.getParameter("allow_subless_notify", "yes") == "yes";
  ...
  if (have_aleg_sdpfilter) {
    if (!readFilter(cfg, "aleg_sdp_filter", "aleg_sdpfilter_list",
                    aleg_sdpfilter, true))
      return false;
  }
...
#define CP_SST_CFGVAR(cfgprefix, cfgkey, dstcfg)                               \
  if (cfg.hasParameter(cfgprefix cfgkey)) {                                    \
    ...
    dstcfg.setParameter(cfgkey,                                                \
                        SBCFactory::instance()->cfg.getParameter(cfgkey));     \
  }
...
#undef CP_SST_CFGVAR
  ...
  uas_auth_bleg_enabled =
      cfg.getParameter("enable_bleg_uas_auth", "no") == "yes";
  ...
  if (!cfg.getParameter("call_control").empty()) {
    ...
    for (vector<string>::iterator it = cc_sections.begin();
         it != cc_sections.end(); it++) {
      ...
      for (std::map<string, string>::const_iterator cfg_it = cfg.begin();
           cfg_it != cfg.end(); cfg_it++) {
        if (cfg_it->first.substr(0, cc_name_prefix_len) != *it + "_") continue;

        if (cfg_it->first.size() <= cc_name_prefix_len
            || cfg_it->first == *it + "_module")
          continue;

        cc_if.cc_values[cfg_it->first.substr(cc_name_prefix_len)] =
            cfg_it->second;
      }
      ...
      if (isArgArray(mandatory_values)) {
        for (size_t i = 0; i < mandatory_values.size(); i++) {
          ...
          if (cc_if.cc_values.find(mandatory_values[i].asCStr())
              == cc_if.cc_values.end()) {
            ERROR("value '%s' for SBC profile '%s' in '%s' not defined. set "
                  "%s_%s=...\n",
                  mandatory_values[i].asCStr(), name.c_str(),
                  profile_file_name.c_str(), it->c_str(),
  ...
  for (vector<string>::iterator it = reply_translations_v.begin();
       it != reply_translations_v.end(); it++) {
    ...
    while (s_pos < to_reply.length() && to_reply[s_pos] != ' ') s_pos++;
    ...
    if (s_pos < to_reply.length()) s_pos++;
  ...
  msgflags_symmetric_rtp =
      cfg.getParameter("rtprelay_msgflags_symmetric_rtp") == "yes";
  ...
  if (!refuse_with.empty()) {
    ...
    if (!next_hop.empty()) {
      INFO("SBC:      next hop = %s (%s, %s)\n", next_hop.c_str(),
           ...
           next_hop_fixed ? "fixed" : "not fixed");
    }
    ...
    filter_type = headerfilter.size()
                      ? FilterType2String(headerfilter.back().filter_type)
                      : "disabled";
    filter_elems =
        headerfilter.size() ? headerfilter.back().filter_list.size() : 0;
    INFO("SBC:      header filter  is %s, %zd items in list\n",
         filter_type.c_str(), filter_elems);

    filter_type = messagefilter.size()
                      ? FilterType2String(messagefilter.back().filter_type)
                      : "disabled";
    filter_elems =
        messagefilter.size() ? messagefilter.back().filter_list.size() : 0;
    INFO("SBC:      message filter is %s, %zd items in list\n",
         filter_type.c_str(), filter_elems);

    filter_type = sdpfilter.size()
                      ? FilterType2String(sdpfilter.back().filter_type)
                      : "disabled";
    ...
    if (have_aleg_sdpfilter) {
      filter_type = aleg_sdpfilter.size()
                        ? FilterType2String(aleg_sdpfilter.back().filter_type)
                        : "disabled";
      filter_elems =
          aleg_sdpfilter.size() ? aleg_sdpfilter.back().filter_list.size() : 0;
      INFO("SBC:      separate A-leg SDP filter is %sabled, %s, %zd items in "
           "list\n",
           sdpfilter.size() ? "en" : "dis", filter_type.c_str(), filter_elems);
    }
    else {
      INFO("SBC:      separate A-leg SDP filter is disabled (same SDP filter "
           "for both legs)\n");
    }

    filter_type = sdpalinesfilter.size()
                      ? FilterType2String(sdpalinesfilter.back().filter_type)
                      : "disabled";
    filter_elems =
        sdpalinesfilter.size() ? sdpalinesfilter.back().filter_list.size() : 0;
    INFO("SBC:      SDP alines-filter is %sabled, %s, %zd items in list\n",
         sdpalinesfilter.size() ? "en" : "dis", filter_type.c_str(),
         filter_elems);

    filter_type = mediafilter.size()
                      ? FilterType2String(mediafilter.back().filter_type)
                      : "disabled";
    filter_elems =
        mediafilter.size() ? mediafilter.back().filter_list.size() : 0;
    INFO("SBC:      SDP filter is %sabled, %s, %zd items in list\n",
    ...
    if (cc_interfaces.size()) {
      string cc_if_names;
      for (CCInterfaceListIteratorT it = cc_interfaces.begin();
           it != cc_interfaces.end(); it++) {
...
static bool payloadDescsEqual(const vector<PayloadDesc>& a,
                              const vector<PayloadDesc>& b)
{
...
{
  bool res =
      ruri == rhs.ruri && ruri_host == rhs.ruri_host && from == rhs.from
      && to == rhs.to &&
      // contact == rhs.contact &&
      ...
      && next_hop == rhs.next_hop && next_hop_1st_req == rhs.next_hop_1st_req
      && next_hop_fixed == rhs.next_hop_fixed
      && patch_ruri_next_hop == rhs.patch_ruri_next_hop
      && aleg_next_hop == rhs.aleg_next_hop && headerfilter == rhs.headerfilter
      &&
      ...
      sdpfilter == rhs.sdpfilter && aleg_sdpfilter == rhs.aleg_sdpfilter
...
string SBCCallProfile::print() const
{
  ...
  res += "aleg_force_outbound_proxy: "
         + string(aleg_force_outbound_proxy ? "true" : "false") + "\n";
  ...
  // + "\n"; res += "messagefilter:        " +
  // string(FilterType2String(messagefilter)) + "\n"; res +=
  // "messagefilter_list:   " + stringset_print(messagefilter_list) + "\n"; res
  // += "sdpfilter_enabled:    " + string(sdpfilter_enabled?"true":"false") +
  // "\n"; res += "sdpfilter:            " +
  // string(FilterType2String(sdpfilter)) + "\n"; res += "sdpfilter_list:
  // " + stringset_print(sdpfilter_list) + "\n"; res += "sdpalinesfilter:      "
  // + string(FilterType2String(sdpalinesfilter)) + "\n"; res +=
  // "sdpalinesfilter_list: " + stringset_print(sdpalinesfilter_list) + "\n";
  res += "sst_enabled:          " + sst_enabled + "\n";
  res += "sst_aleg_enabled:     " + sst_aleg_enabled + "\n";
  res +=
      "auth_enabled:         " + string(auth_enabled ? "true" : "false") + "\n";
...
static bool isTranscoderNeeded(const AmSipRequest&  req,
                               vector<PayloadDesc>& caps, bool default_value)
{
  ...
  for (vector<SdpMedia>::iterator m = sdp.media.begin(); m != sdp.media.end();
       ++m) {
    for (vector<SdpPayload>::iterator p = m->payloads.begin();
         p != m->payloads.end(); ++p) {
      for (vector<PayloadDesc>::iterator i = caps.begin(); i != caps.end();
           ++i) {
...
#define SST_CFG_PARAM_COUNT 5 // Change if you add/remove params in below
  ...
  static const char* _sst_cfg_params[] = {
      "session_expires",        "minimum_timer",    "maximum_timer",
      "session_refresh_method", "accept_501_reply",
  };

  for (unsigned int i = 0; i < SST_CFG_PARAM_COUNT; i++) {
    if (sst_cfg.hasParameter(_sst_cfg_params[i])) {
      string newval = ctx.replaceParameters(
          sst_cfg.getParameter(_sst_cfg_params[i]), _sst_cfg_params[i], req);
      if (newval.empty()) {
...
bool SBCCallProfile::evaluate(ParamReplacerCtx& ctx, const AmSipRequest& req)
{
  ...
  if (auth_enabled) {
    auth_credentials.user =
        ctx.replaceParameters(auth_credentials.user, "auth_user", req);
    auth_credentials.pwd =
        ctx.replaceParameters(auth_credentials.pwd, "auth_pwd", req);
  }

  if (auth_aleg_enabled) {
    auth_aleg_credentials.user = ctx.replaceParameters(
        auth_aleg_credentials.user, "auth_aleg_user", req);
    auth_aleg_credentials.pwd =
        ctx.replaceParameters(auth_aleg_credentials.pwd, "auth_aleg_pwd", req);
  }

  if (uas_auth_bleg_enabled) {
    uas_auth_bleg_credentials.realm = ctx.replaceParameters(
        uas_auth_bleg_credentials.realm, "uas_auth_bleg_realm", req);
    uas_auth_bleg_credentials.user = ctx.replaceParameters(
        uas_auth_bleg_credentials.user, "uas_auth_bleg_user", req);
    uas_auth_bleg_credentials.pwd = ctx.replaceParameters(
        uas_auth_bleg_credentials.pwd, "uas_auth_bleg_pwd", req);
  }

  fix_replaces_inv =
      ctx.replaceParameters(fix_replaces_inv, "fix_replaces_inv", req);
  fix_replaces_ref =
      ctx.replaceParameters(fix_replaces_ref, "fix_replaces_ref", req);
  ...
  /*  if ((!aleg_payload_order.empty() || !bleg_payload_order.empty()) &&
    (!sdpfilter_enabled)) { sdpfilter_enabled = true; sdpfilter = Transparent;
    }*/
...
{
  if (!aleg_outbound_interface.empty()) {
    string aleg_oi = ctx.replaceParameters(aleg_outbound_interface,
                                           "aleg_outbound_interface", req);

    if (apply_outbound_interface(aleg_oi, dlg) < 0) return -1;
  }
                                    ...
                                    AmBasicSipDialog&   dlg) const
{
  if (!outbound_interface.empty()) {
    ...
    if (apply_outbound_interface(oi, dlg) < 0) return -1;
  }
  ...
  if (!next_hop.empty()) {
    ...
    DBG("set next hop to '%s' (1st_req=%s,fixed=%s)\n", nh.c_str(),
        next_hop_1st_req ? "true" : "false", next_hop_fixed ? "true" : "false");
    dlg.setNextHop(nh);
...
void SBCCallProfile::replace_cc_values(ParamReplacerCtx&   ctx,
                                       const AmSipRequest& req, AmArg* values)
{
...
static void fixupCCInterface(const string& val, CCInterface& cc_if)
{
  ...
  while (spos < last) {
    ...
    if (epos == last) {
      cc_if.cc_values.insert(
          make_pair(val.substr(spos + 1, epos - spos - 1), ""));
...
void SBCCallProfile::eval_cc_list(ParamReplacerCtx&   ctx,
                                  const AmSipRequest& req)
{
  ...
  while (cc_rit != cc_interfaces.end()) {
    ...
    if (curr_if->cc_name.find('$') != string::npos) {
      curr_if->cc_name =
          ctx.replaceParameters(curr_if->cc_name, "cc_interfaces", req);
                                     ...
                                     const AmSipRequest& req)
{
  ...
  fix_append_hdr_list(req, ctx, aleg_append_headers_req,
...
static bool readPayloadList(std::vector<PayloadDesc>& dst,
...
static bool readPayload(SdpPayload& p, const string& src)
{
  ...
  for (int i = 0; i < IANA_RTP_PAYLOADS_SIZE; i++) {
    ...
    if (p.encoding_name == s
        ...
        && (p.encoding_param == -1
            || ((unsigned) p.encoding_param == IANA_RTP_PAYLOADS[i].channels)))
      p.payload_type = i;
  }
...
static bool read(const std::string& src, vector<SdpPayload>& codecs)
{
  ...
  for (vector<string>::iterator it = elems.begin(); it != elems.end(); ++it) {
    ...
    if (!payload) {
      ...
      p.sdp_format_parameters =
          plugin->getSdpFormatParameters(payload->codec_id, true, "");
...
void SBCCallProfile::CodecPreferences::orderSDP(AmSdp& sdp, bool a_leg)
{
  ...
  std::vector<PayloadDesc>& payload_order =
      a_leg ? bleg_payload_order : aleg_payload_order;
  ...
  for (vector<SdpMedia>::iterator m_it = sdp.media.begin();
    ...
    for (vector<PayloadDesc>::iterator i = payload_order.begin();
         i != payload_order.end(); ++i) {
      ...
      for (idx = pos; idx < cnt; idx++) {
        if (i->match(media.payloads[idx])) {
          ...
          if (idx != pos) {
            media.payloads.insert(media.payloads.begin() + pos,
                                  media.payloads[idx]);
...
bool SBCCallProfile::CodecPreferences::shouldOrderPayloads(bool a_leg)
{
  ...
  if (a_leg)
    return !bleg_payload_order.empty();
  else
    return !aleg_payload_order.empty();
}
...
bool SBCCallProfile::CodecPreferences::readConfig(AmConfigReader& cfg)
{
  ...
  bleg_prefer_existing_payloads_str =
      cfg.getParameter("prefer_existing_codecs");

  aleg_payload_order_str = cfg.getParameter("codec_preference_aleg");
  aleg_prefer_existing_payloads_str =
      cfg.getParameter("prefer_existing_codecs_aleg");
...
operator==(const CodecPreferences& rhs) const
{
  if (!payloadDescsEqual(aleg_payload_order, rhs.aleg_payload_order))
    return false;
  if (!payloadDescsEqual(bleg_payload_order, rhs.bleg_payload_order))
    return false;
  if (aleg_prefer_existing_payloads != rhs.aleg_prefer_existing_payloads)
    return false;
  if (bleg_prefer_existing_payloads != rhs.bleg_prefer_existing_payloads)
    return false;
  return true;
}
...
string SBCCallProfile::CodecPreferences::print() const
{
  ...
  for (vector<PayloadDesc>::const_iterator i = bleg_payload_order.begin();
       i != bleg_payload_order.end(); ++i) {
  ...
  if (bleg_prefer_existing_payloads)
    res += "yes\n";
  else
    res += "no\n";

  res += "codec_preference_aleg:    ";
  for (vector<PayloadDesc>::const_iterator i = aleg_payload_order.begin();
       i != aleg_payload_order.end(); ++i) {
  ...
  if (aleg_prefer_existing_payloads)
    res += "yes\n";
  else
    res += "no\n";
...
bool SBCCallProfile::CodecPreferences::evaluate(ParamReplacerCtx&   ctx,
                                                const AmSipRequest& req)
{
  REPLACE_BOOL(aleg_prefer_existing_payloads_str,
               aleg_prefer_existing_payloads);
  REPLACE_BOOL(bleg_prefer_existing_payloads_str,
               bleg_prefer_existing_payloads);

  REPLACE_NONEMPTY_STR(aleg_payload_order_str);
  REPLACE_NONEMPTY_STR(bleg_payload_order_str);

  if (!readPayloadList(bleg_payload_order, bleg_payload_order_str))
    return false;
  if (!readPayloadList(aleg_payload_order, aleg_payload_order_str))
    return false;
...
bool SBCCallProfile::TranscoderSettings::readTranscoderMode(
    const std::string& src)
{
  ...
  if (src == on_missing_compatible) {
    transcoder_mode = OnMissingCompatible;
    return true;
  }
  if (src.empty()) {
    transcoder_mode = Never;
...
bool SBCCallProfile::TranscoderSettings::readConfig(AmConfigReader& cfg)
{
  ...
  audio_codecs_norelay_aleg_str =
      cfg.getParameter("prefer_transcoding_for_codecs_aleg");
...
operator==(const TranscoderSettings& rhs) const
{
  ...
  res      = res
        && (payloadDescsEqual(callee_codec_capabilities,
                              rhs.callee_codec_capabilities));
...
string SBCCallProfile::TranscoderSettings::print() const
{
  ...
  for (vector<SdpPayload>::const_iterator i = audio_codecs.begin();
       i != audio_codecs.end(); ++i) {
    res += " ";
    res += payload2str(*i);
  }

  res += "\ncallee codec capabilities:";
  for (vector<PayloadDesc>::const_iterator i =
           callee_codec_capabilities.begin();
                                                  ...
                                                  const AmSipRequest& req)
{
  ...
  if (!read(audio_codecs_norelay_aleg_str, audio_codecs_norelay_aleg))
    return false;

  if (!readPayloadList(callee_codec_capabilities,
                       callee_codec_capabilities_str))
    return false;
...
void SBCCallProfile::create_logger(const AmSipRequest& req)
{
  ...
  string           log_path =
      ctx.replaceParameters(msg_logger_path, "msg_logger_path", req);
...
bool SBCCallProfile::HoldSettings::evaluate(ParamReplacerCtx&   ctx,
                                            const AmSipRequest& req)
{
...
bool SBCCallProfile::BLegContact::evaluate(ParamReplacerCtx&   ctx,
                                           const AmSipRequest& req)
{

                                     ...
                                     const SdpPayload& payload, int transport)
{
  ...
  for (vector<SdpPayload>::const_iterator p = payloads.begin();
       p != payloads.end(); ++p) {
    ...
    , cc_started(false)
{
  ...
  if (call_profile.rtprelay_bw_limit_rate > 0
    ...
    RateLimit* limit = new RateLimit(call_profile.rtprelay_bw_limit_rate,
                                     call_profile.rtprelay_bw_limit_peak, 1000);
    ...
    , cc_started(false)
{
  ...
  if (caller->rtp_relay_rate_limit.get()) {
    rtp_relay_rate_limit.reset(
        new RateLimit(*caller->rtp_relay_rate_limit.get()));
  }
...
void SBCCallLeg::onStart()
{
  ...
  CallLeg::onStart();
  if (!a_leg)
    applyBProfile(); // A leg needs to evaluate profile first
...
int SBCCallLeg::applySSTCfg(AmConfigReader& sst_cfg, const AmSipRequest* p_req)
{
  ...
  if (p_req
      && !SBCFactory::instance()->session_timer_fact->onInvite(*p_req,
                                                               sst_cfg)) {
    return -1;
  }

  AmSessionEventHandler* h =
      SBCFactory::instance()->session_timer_fact->getHandler(this);
...
void SBCCallLeg::applyBProfile()
{
  ...
  if (call_profile.uas_auth_bleg_enabled) {
    ...
    if (NULL != fact) {
      ...
      ERROR("B-leg UAS auth enabled (uas_auth_bleg_enabled), but uac_auth "
            "module not loaded!\n");
    }
  }
  ...
  if (!call_profile.next_hop.empty()) {
    DBG("set next hop to '%s' (1st_req=%s,fixed=%s)\n",
        ...
        call_profile.next_hop_1st_req ? "true" : "false",
  ...
  // was read from caller but reading directly from profile now
  if (!call_profile.callid.empty()) dlg->setCallid(call_profile.callid);
      ...
      if (call_profile.headerfilter.size()
        ...
        if (it != call_profile.reply_translations.end()) {
          DBG("translating reply %u %s => %u %s\n", reply_ev->reply.code,
              reply_ev->reply.reason.c_str(), it->second.first,
      ...
      if (di_ret.size() >= 3) {
        if (di_ret[0].asInt() != 200) {
          DBG("Auth: replying %u %s - hdrs: '%s'\n", di_ret[0].asInt(),
              di_ret[1].asCStr(), di_ret[2].asCStr());
          dlg->reply(req, di_ret[0].asInt(), di_ret[1].asCStr(), NULL,
                     di_ret[2].asCStr());
          return;
        }
        else {
...
void SBCCallLeg::onDtmf(int event, int duration)
{
  ...
  for (vector<ExtendedCCInterface*>::iterator i = cc_ext.begin();
       i != cc_ext.end(); ++i) {
    if ((*i)->onDtmf(this, event, duration) == StopProcessing) return;
  }
...
void SBCCallLeg::updateLocalSdp(AmSdp& sdp)
{
  ...
  if (call_profile.anonymize_sdp)
    normalizeSDP(sdp, call_profile.anonymize_sdp, advertisedIP());
...
void SBCCallLeg::onInvite(const AmSipRequest& req)
{
  ...
  call_profile.sst_aleg_enabled = ctx.replaceParameters(
      call_profile.sst_aleg_enabled, "enable_aleg_session_timer", req);
...
#undef REPLACE_VALS
  ...
  if (getCallStatus() == Disconnected) {
    ...
    connectCallee(to, ruri, from, req,
                  invite_req); // connect to the B leg(s) using modified request
  }
}

void SBCCallLeg::connectCallee(const string& remote_party,
                               const string& remote_uri, const string& from,
                               ...
                               const AmSipRequest& invite)
{
  ...
  SBCCallLeg* callee_session =
      SBCFactory::instance()->getCallLegCreator()->create(this);
...
void SBCCallLeg::onStop()
{
  ...
  if (a_leg
      && m_state == BB_Connected) { // m_state might be valid for A leg only
    stopCallTimers();
  }
...
void SBCCallLeg::clearCallTimer(int timer) { call_timers.erase(timer); }

void SBCCallLeg::clearCallTimers() { call_timers.clear(); }
...
bool SBCCallLeg::CCStart(const AmSipRequest& req)
{
  ...
  for (CCInterfaceListIteratorT cc_it = call_profile.cc_interfaces.begin();
    ...
    for (int i = 0; i < 4; i++) di_args.back().push((int) 0);
    ...
    if (exception_occured) {
      SBCEventLog::instance()->logCallStart(req, getLocalTag(),
                                            dlg->getRemoteUA(), "", 500,
    ...
    if (isArgArray(ret)) {
      for (size_t i = 0; i < ret.size(); i++) {
        if (!isArgArray(ret[i]) || !ret[i].size()) continue;
        ...
        switch (ret[i][SBC_CC_ACTION].asInt()) {
          ...
          case SBC_CC_REFUSE_ACTION: {
            if (ret[i].size() < 3 || !isArgInt(ret[i][SBC_CC_REFUSE_CODE])
                || !isArgCStr(ret[i][SBC_CC_REFUSE_REASON])) {
              ERROR("in call control module '%s' - REFUSE action parameters "
                    "missing/wrong: '%s'\n",
            ...
            if (ret[i].size() > SBC_CC_REFUSE_HEADERS) {
              for (size_t h = 0; h < ret[i][SBC_CC_REFUSE_HEADERS].size(); h++)
                headers +=
                    string(ret[i][SBC_CC_REFUSE_HEADERS][h].asCStr()) + CRLF;
            }

            DBG("replying with %d %s on call control action REFUSE from '%s' "
                "headers='%s'\n",
                ret[i][SBC_CC_REFUSE_CODE].asInt(),
                ret[i][SBC_CC_REFUSE_REASON].asCStr(), cc_if.cc_name.c_str(),
                headers.c_str());

            SBCEventLog::instance()->logCallStart(
                req, getLocalTag(), dlg->getRemoteUA(), "",
                ret[i][SBC_CC_REFUSE_CODE].asInt(),
                ret[i][SBC_CC_REFUSE_REASON].asCStr());

            dlg->reply(req, ret[i][SBC_CC_REFUSE_CODE].asInt(),
                       ret[i][SBC_CC_REFUSE_REASON].asCStr(), NULL, headers);
          ...
          case SBC_CC_SET_CALL_TIMER_ACTION: {
            ...
            if (ret[i].size() < 2
              ...
              ERROR("in call control module '%s' - SET_CALL_TIMER action "
...
void SBCCallLeg::CCConnect(const AmSipReply& reply)
{
  ...
  for (CCInterfaceListIteratorT cc_it = call_profile.cc_interfaces.begin();
    ...
    for (int i = 0; i < 2; i++) di_args.back().push((int) 0);
...
void SBCCallLeg::CCEnd() { CCEnd(call_profile.cc_interfaces.end()); }
...
void SBCCallLeg::onCallFailed(CallFailureReason reason, const AmSipReply* reply)
{
  switch (reason) {
    ...
    case CallCanceled: logCanceledCall(); break;
  }
}
...
int SBCCallLeg::filterSdp(AmMimeBody& body, const string& method)
{
  ...
  if (!(method == SIP_METH_INVITE || method == SIP_METH_UPDATE
        || method == SIP_METH_PRACK || method == SIP_METH_ACK))
    return 0;
  ...
  bool changed = false;
  bool prefer_existing_codecs =
      call_profile.codec_prefs.preferExistingCodecs(a_leg);

  bool needs_normalization = call_profile.codec_prefs.shouldOrderPayloads(a_leg)
                             || call_profile.transcoder.isActive()
  ...
  if (needs_normalization) {
    normalizeSDP(
        sdp, false,
        ""); // anonymization is done in the other leg to use correct IP address
    changed = true;
  }
...
void SBCCallLeg::appendTranscoderCodecs(AmSdp& sdp)
{
  ...
  const std::vector<SdpPayload>& transcoder_codecs =
  ...
  for (vector<SdpMedia>::iterator m = sdp.media.begin(); m != sdp.media.end();
    ...
    if (m->type == MT_AUDIO) {
      ...
      // if there are no such common payloads adding transcoder codecs can't
      // help because we won't be able to transcode later on! (we have to check
      // for each media stream independently)
      ...
      for (p = m->payloads.begin(); p != m->payloads.end(); ++p) {
        ...
        if (containsPayload(transcoder_codecs, *p, m->transport))
          transcodable = true;
      ...
      if (transcodable) {
        ...
        unsigned idx = 0;
        for (p = transcoder_codecs.begin(); p != transcoder_codecs.end();
             ++p, ++idx) {
        ...
        if (id > 128)
          ERROR("assigned too high payload type number (%d), see RFC 3551\n",
                id);
      }
      else {
        // no compatible codecs found
        TRACE("can not transcode stream %d - no compatible codecs with "
              "transcoder_codecs found\n",
              stream_idx + 1);
      }
...
bool SBCCallLeg::reinvite(const AmSdp& sdp, unsigned& request_cseq)
{
  ...
  string body_str;
  sdp.print(body_str);
  sdp_body->parse(SIP_APPLICATION_SDP, (const unsigned char*) body_str.c_str(),
                  body_str.length());
...
/**
 * initialize modules from @arg cc_module_list with DI interface instances from
 * @arg cc_module_di add sucessfull ext-API call control instances to cc_ext
 * @return true on success (all modules properly initialized)
 */
bool SBCCallLeg::initCCExtModules(const CCInterfaceListT&     cc_module_list,
                                  const vector<AmDynInvoke*>& cc_module_di)
{
  ...
  for (CCInterfaceListConstIteratorT cc_it = cc_module_list.begin();
       cc_it != cc_module_list.end(); cc_it++) {
    ...
    try {
      ...
      if (iface) {
        ...
        WARN("BUG: returned invalid extended CC interface by cc_module '%s'\n",
             cc_module.c_str());
    }
...
bool SBCCallLeg::initPendingCCExtModules()
{
  while (cc_module_queue.size()) {
    ...
    if (!::getCCInterfaces(_cc_mod_queue, _cc_mod_ifs)) return false;
...
static void alterHoldRequest(AmSdp&                                 sdp,
                             SBCCallProfile::HoldSettings::Activity a,
                             const string&                          ip)
{
...
void SBCCallLeg::alterHoldRequest(AmSdp& sdp)
{
  TRACE("altering B2B hold request(%s, %s, %s)\n",
        call_profile.hold_settings.alter_b2b(a_leg) ? "alter B2B"
                                                    : "do not alter B2B",
...
{
  if (new_session) {
    if (call_profile.log_rtp)
      new_session->setRtpLogger(logger);
    else
      new_session->setRtpLogger(NULL);
  }
...
void SBCCallLeg::computeRelayMask(const SdpMedia& m, bool& enable,
                                  PayloadMask& mask)
{
  if (call_profile.transcoder.isActive()) {
    ...
    SBCCallProfile::TranscoderSettings& transcoder_settings =
        call_profile.transcoder;
    PayloadMask m1, m2;
    bool        use_m1 = false;

    /* if "m" contains only "norelay" codecs, relay is enabled for them (main
     * idea of these codecs is to limit network bandwidth and it makes not much
     * sense to transcode between codecs 'which are better to avoid', right?)
     *
    ...
    vector<SdpPayload>& norelay_payloads =
        a_leg ? transcoder_settings.audio_codecs_norelay_aleg
              : transcoder_settings.audio_codecs_norelay;

    vector<SdpPayload>::const_iterator p;
    for (p = m.payloads.begin(); p != m.payloads.end(); ++p) {
      // do not mark telephone-event payload for relay (and do not use it for
      // transcoding as well)
      if (strcasecmp("telephone-event", p->encoding_name.c_str()) == 0)
        continue;
      ...
      if (!containsPayload(norelay_payloads, *p, m.transport)) {
        ...
        if (!use_m1
            && containsPayload(transcoder_settings.audio_codecs, *p,
                               m.transport)) {
          // the remote SDP contains transcodable codec which can be relayed
          // (i.e. the one with higher "priority" so we want to disable relaying
          // of the payloads which should not be ralyed if possible)
          use_m1 = true;
        }
      }
    }

                       ...
                       string& res)
{
  ...
  switch (s[p + 1]) {
    case 'u': { // URI
      ...
      if (!parsed.uri_port.empty()) res += ":" + parsed.uri_port;
    } break;
    case 'U':
      res += parsed.uri_user;
      break;    // User
    ...
    case 'P': { // Params
      if ((s.length() > p + 3) && (s[p + 2] == '(')) {
        ...
        string param;
        for (list<sip_avp*>::iterator it = params.begin(); it != params.end();
             it++) {
          if (lower_cmp_n((*it)->name.s, (*it)->name.len, param_name.c_str(),
                          param_name.length()))
            continue;
        ...
        switch (s[p]) {
          ...
          case 'd': { // destination (remote UAS)
            ...
            if (!call_profile->next_hop.empty()) {
              ...
              if (dest_list.size() == 0) {
                WARN("next-hop is not empty, but the resulting destination "
                     "list is\n");
                break;
              }
          ...
          case 'u': { // Reg-cached destination user
            ...
            if (!RegisterCache::instance()->findAliasEntry(alias,
                                                           alias_entry)) {
          ...
          case 'U': { // Reg-cached originating user
            ...
            if (s[p + 1] == 'a') { // $Ua originating AoR
              ...
              if (reg_cache->getAorAliasMap(aor, alias_map)
                  && !alias_map.empty()) {
                  ...
                  if (reg_cache->findAliasEntry(it->first, alias_entry)) {
                    if ((alias_entry.source_ip == req.remote_ip)
                      ...
                      DBG("matching entry for alias '%s' found (src=%s:%i)",
                          it->first.c_str(), alias_entry.source_ip.c_str(),
                    ...
                    // else {
                    //   DBG("alias '%s': source IP/port mismatch: %s:%i !=
                    //   %s:%i",
                    // 	it->first.c_str(),
...
#define case_HDR(pv_char, pv_name, hdr_name)                                   \
  case pv_char: {                                                              \
    ...
    if (s[p + 1] == 'i') {                                                     \
      ...
      if (!uri_parser.uri_port.empty()) res += ":" + uri_parser.uri_port;      \
    }                                                                          \
    else {                                                                     \
      skip_chars = replaceParsedParam(s, p, uri_parser, res);                  \
            ...
            case_HDR('a', "PAI",
                     SIP_HDR_P_ASSERTED_IDENTITY); // P-Asserted-Identity
            case_HDR('p', "PPI",
                     SIP_HDR_P_PREFERRED_IDENTITY); // P-Preferred-Identity
              ...
              if (it != call_profile->cc_vars.end()) {
                if (vn.empty()) {
                  ...
                  if (isArgStruct(it->second)) {
                    // recursive replacement for call variable name (defined via
                    // GUI)
                    if (vn.find('$') != string::npos)
                      vn = replaceParameters(
                          vn, "CallVar Subname", req, call_profile, app_param,
                          ruri_parser, from_parser, to_parser, rebuild_ruri,
                          rebuild_from, rebuild_to);
                    val = &it->second[vn];
                  }
                  else {
                    DBG("CC variable '%s' has wrong type: '%s'\n", vn.c_str(),
                        AmArg::print(it->second).c_str());
                  }
                }
                if (val != NULL) {
          ...
          case 'H': { // header
            ...
            string hdr_name =
                s.substr(p + name_offset, skip_p - p - name_offset);
            // DBG("param_name = '%s' (skip-p - p = %d)\n", param_name.c_str(),
            // skip_p-p);
            if (name_offset == 2) {
              // full header
          ...
          case 'M': { // regex map
            ...
            if (skip_p == s.length()) {
              WARN("Error parsing $M regex map replacement (unclosed "
                   "brackets)\n");
              skip_chars = skip_p - p;
              break;
            }
            ...
            if (spos == string::npos) {
              skip_chars = skip_p - p;
              WARN("Error parsing $M regex map replacement: no => found in "
                   "'%s'\n",
                   map_str.c_str());
              break;
            }
            ...
            string map_val_replaced = replaceParameters(
                map_val, r_type, req, call_profile, app_param, ruri_parser,
                from_parser, to_parser, rebuild_ruri, rebuild_from, rebuild_to);

            string mapping_name = map_str.substr(spos + 2);

            string map_res;
            if (SBCFactory::instance()->regex_mappings.mapRegex(
                    mapping_name, map_val_replaced.c_str(), map_res)) {
              DBG("matched regex mapping '%s' (orig '%s) in '%s'\n",
                  map_val_replaced.c_str(), map_val.c_str(),
                  mapping_name.c_str());
              res += map_res;
            }
            else {
          ...
          case '_': {                 // modify
            ...
            if (operation != 'U' && operation != 'l' && operation != 's'
                && operation != '5') {
              WARN(
                  "Error parsing $_%c string modifier: unknown operator '%c'\n",
                  operation, operation);
            }
            ...
            string br_str_replaced = replaceParameters(
                br_str, "$_*(...)", req, call_profile, app_param, ruri_parser,
                from_parser, to_parser, rebuild_ruri, rebuild_from, rebuild_to);
            ...
            switch (operation) {
              ...
              default:
                WARN("Error parsing $_%c string modifier: unknown operator "
                     "'%c'\n",
                     operation, operation);
                break;
            }
          ...
          case '#': { // URL encoding
            ...
            string expr_replaced = replaceParameters(
                expr_str, r_type, req, call_profile, app_param, ruri_parser,
                from_parser, to_parser, rebuild_ruri, rebuild_from, rebuild_to);

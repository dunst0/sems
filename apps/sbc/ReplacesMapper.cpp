...
void fixReplaces(string& req_hdrs, bool is_invite)
{
  ...
  if (is_invite) {
    ...
    refer_to =
        getHeader(req_hdrs, SIP_HDR_REFER_TO, SIP_HDR_REFER_TO_COMPACT, true);
    if (refer_to.empty()) {
    ...
    for (replaces_hdr_it = hdrs.begin(); replaces_hdr_it != hdrs.end();
         replaces_hdr_it++) {
      string      s            = URL_decode(*replaces_hdr_it);
      const char* Replaces_str = "Replaces";
      if ((s.length() >= 8) && !strncmp(Replaces_str, s.c_str(), 8)) {
        size_t pos = 8;
    ...
    if (replaces_hdr_it == hdrs.end()) {
      DBG("Replaces handler: no Replaces headers in Refer-To target, "
          "ignoring\n");
      return;
    }
  }
  ...
  while (cid_len < replaces.size() && replaces[cid_len] != ';') cid_len++;
  ...
  if (SBCCallRegistry::lookupCall(ttag, other_dlg)) {
    replaces = other_dlg.callid + ";from-tag=" + other_dlg.ltag
               + ";to-tag=" + other_dlg.rtag;
    if (early_only) replaces += ";early_only";
    ...
    if (is_invite) {
      ...
      req_hdrs +=
          SIP_HDR_COLSP(SIP_HDR_REFER_TO) + refer_target.nameaddr_str() + CRLF;
    }
  }
  else {

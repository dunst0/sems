...
SubscriptionDialog::SubscriptionDialog(SBCCallProfile&       profile,
    ...
    , subs(subscription)
{
  if (!subs) subs = new AmSipSubscription(this, this);
}
...
bool SubscriptionDialog::terminated() { return !(getUsages() > 0); }
...
{
  if (!subs->onRequestIn(req)) return;
...
{
  if (!subs->onReplyIn(req, reply)) return;

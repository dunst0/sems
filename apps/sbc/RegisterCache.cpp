...
AorEntry* AorBucket::get(const string& aor)
{
  ...
  if (it == elmts.end()) return NULL;

  return it->second;
}

void AorBucket::dump_elmt(const string& aor, const AorEntry* p_aor_entry) const
{
  DBG("'%s' ->", aor.c_str());
  ...
  for (AorEntry::const_iterator it = p_aor_entry->begin();
    ...
    if (it->second) {
      ...
      DBG("\t'%s' -> '%s'", it->first.c_str(), b ? b->alias.c_str() : "NULL");
    }
  }
}
...
struct RegCacheLogHandler : RegCacheStorageHandler
{
  void onDelete(const string& aor, const string& uri, const string& alias)
  {
    DBG("delete: aor='%s';uri='%s';alias='%s'", aor.c_str(), uri.c_str(),
        alias.c_str());
  }

  void onUpdate(const string& canon_aor, const string& alias, long int expires,
                const AliasEntry& alias_update)
  {
    DBG("update: aor='%s';alias='%s';expires=%li", canon_aor.c_str(),
        alias.c_str(), expires);
  }

  void onUpdate(const string& alias, long int ua_expires)
  {
    DBG("update: alias='%s';ua_expires=%li", alias.c_str(), ua_expires);
  }
};
...
void _RegisterCache::gbc(unsigned int bucket_id)
{
  ...
  for (list<string>::iterator it = alias_list.begin(); it != alias_list.end();
       it++) {
    removeAlias(*it, true);
  }
  bucket->unlock();
...
string _RegisterCache::compute_alias_hash(const string& aor,
                                          const string& contact_uri,
                            ...
                            const AliasEntry& alias_update)
{
  ...
  if (!binding) {
    ...
    DBG("inserted new binding: '%s' -> '%s'", uri.c_str(), alias.c_str());

    // inc stats
    active_regs.inc();

    ContactBucket* ct_bucket =
        getContactBucket(uri, alias_update.source_ip, alias_update.source_port);
    ct_bucket->lock();
    ct_bucket->insert(uri, alias_update.source_ip, alias_update.source_port,
                      alias);
    ct_bucket->unlock();
  }
  else {
    DBG("updating existing binding: '%s' -> '%s'", uri.c_str(),
        binding->alias.c_str());
  ...
  if (!alias_e) {
    DBG("inserting alias map entry: '%s' -> '%s'", alias.c_str(), uri.c_str());
...
void _RegisterCache::update(long int          reg_expires,
                            const AliasEntry& alias_update)
{
  string uri       = alias_update.contact_uri;
  string canon_aor = alias_update.aor;
    ...
    if (binding_it != aor_e->end()) {
      ...
      if (binding && (binding_it->first != idx)) {
        ...
        if (findAliasEntry(alias, ae)) {
          ...
          ct_bucket =
              getContactBucket(uri, public_ip, alias_update.source_port);
          ct_bucket->lock();
  ...
  if (!binding) {
    ...
    binding = new RegBinding();
    binding->alias =
        _RegisterCache::compute_alias_hash(canon_aor, uri, public_ip);
    ...
    string idx = uri + "/" + public_ip;
    aor_e->insert(AorEntry::value_type(idx, binding));
    DBG("inserted new binding: '%s' -> '%s'", idx.c_str(),
        binding->alias.c_str());
  ...
  // if no alias map entry, insert a new one
  if (!alias_e) {
    DBG("inserting alias map entry: '%s' -> '%s'", binding->alias.c_str(),
        uri.c_str());
    alias_e        = new AliasEntry(alias_update);
  ...
  if (storage_handler.get()) {
    storage_handler->onUpdate(canon_aor, binding->alias, reg_expires, *alias_e);
  }
...
bool _RegisterCache::updateAliasExpires(const string& alias,
                                        long int      ua_expires)
{
  bool         res          = false;
  AliasBucket* alias_bucket = getAliasBucket(alias);
...
void _RegisterCache::removeAlias(const string& alias, bool generate_event)
{
  ...
  if (ae) {
    ...
    storage_handler->onDelete(ae->aor, ae->contact_uri, ae->alias);
  }
                                    ...
                                    map<string, string>& alias_map)
{
  ...
  AorEntry* aor_e = bucket->get(canon_aor);
  if (aor_e) {
    for (AorEntry::iterator it = aor_e->begin(); it != aor_e->end(); ++it) {
      if (!it->second) continue;

      AliasEntry ae;
      if (!findAliasEntry(it->second->alias, ae)) continue;
...
bool _RegisterCache::findAliasEntry(const string& alias,
                                     ...
                                     AliasEntry&        ae)
{
  ...
  ContactBucket* ct_bucket =
      getContactBucket(contact_uri, remote_ip, remote_port);
  ct_bucket->lock();
...
int _RegisterCache::parseAoR(RegisterCacheCtx& ctx, const AmSipRequest& req,
                             msg_logger* logger)
{
  if (ctx.aor_parsed) return 0;
...
int _RegisterCache::parseContacts(RegisterCacheCtx&   ctx,
                                  const AmSipRequest& req, msg_logger* logger)
{
...
int _RegisterCache::parseExpires(RegisterCacheCtx& ctx, const AmSipRequest& req,
                                 msg_logger* logger)
{
                                      ...
                                      msg_logger*         logger)
{
  ...
  if ((parseAoR(ctx, req, logger) < 0) || (parseContacts(ctx, req, logger) < 0)
  ...
  for (vector<AmUriParser>::iterator contact_it = ctx.contacts.begin();
    ...
    alias_updates.push_back(make_pair(reg_binding.alias, contact_expires));
  }
  ...
  if (!updateAliasExpires(alias_update_it->first, alias_update_it->second)) {
    // alias not found ???
    return false; // fwd
  }
  it++;
                                       ...
                                       msg_logger*         logger)
{
  ...
  if (req.contact.empty()) {
    ...
    if (getAorAliasMap(ctx.from_aor, alias_map) && !alias_map.empty()) {
      ...
      if (findAliasEntry(alias_map.begin()->first, alias_entry)
        ...
        contact_hdr = SIP_HDR_COLSP(SIP_HDR_CONTACT) + alias_entry.contact_uri
                      + ";expires=" + int2str(exp) + CRLF;
      }
    }
  ...
  if (req.contact == "*") {
    ...
    AmBasicSipDialog::reply_error(
        req, 403, "Forbidden", "Warning: only one contact allowed\r\n", logger);
    return true; // error reply sent
  }
  else {
  ...
  if (ua_expires && ctx.max_ua_expires && (ua_expires > ctx.max_ua_expires)) {
  ...
  string contact_hdr = SIP_HDR_COLSP(SIP_HDR_CONTACT) + contact->print() + CRLF;

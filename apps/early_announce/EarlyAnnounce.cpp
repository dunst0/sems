                     ...
                     string domain, string language, string* audio_file)
{
  ...
  if (!user.empty()) {
    *audio_file = string("/tmp/") + application + "_" + message + "_" + domain
                  + "_" + user + ".wav";
    ...
    *audio_file =
        string("/tmp/") + application + "_" + message + "_" + language + ".wav";
...
void EarlyAnnounceDialog::onInvite(const AmSipRequest& req)
{
  ...
  if (dlg->reply(req, 183, "Session Progress", &sdp_body) != 0) {
                                          ...
                                          const map<string, string>& app_params)
{
  ...
  get_announce_msg(AnnounceApplication, AnnounceMessage, req.user, req.domain,
                   "", &announce_file);
  ...
  string announce_file = announce_path + req.domain + "/" + req.user + ".wav";

  DBG("trying '%s'\n", announce_file.c_str());
  if (file_exists(announce_file)) goto end;
...
void EarlyAnnounceDialog::process(AmEvent* event)
{
  ...
  if (audio_event && (audio_event->event_id == AmAudioEvent::cleared)) {
    ...
    if (EarlyAnnounceFactory::ContinueB2B == EarlyAnnounceFactory::Always) {
      continue_b2b = true;
    }
    ...
    if (!continue_b2b) {
      ...
      if (iptel_app_param.length()) {
        ...
        string h_reason =
            getHeader(invite_req.hdrs, "P-Final-Reply-Reason", true);
        if (h_reason.length()) {
          INFO(
              "Use of P-Final-Reply-Code/P-Final-Reply-Reason is deprecated. ");

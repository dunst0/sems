  ...
  if (language.empty()) {
    if (domain.empty()) {
      ...
      query_string = "select audio from " + string(DEFAULT_AUDIO_TABLE)
                     + " where application='" + APP_NAME + "' and message='"
    ...
    if (domain.empty()) {
      ...
      audio_file = "/tmp/" + domain + "_" APP_NAME "_" + message + "_"
                   + language + ".wav";
      query_string = "select audio from " + string(DOMAIN_AUDIO_TABLE)
                     + " where application='" + APP_NAME + "' and message='"
...
int ConferenceFactory::onLoad()
{
  ...
  try {
    ...
    if (!mysql_ca_cert.empty())
      Connection.set_option(new mysqlpp::SslOption(0, 0, mysql_ca_cert.c_str(),
                                                   "", "DHE-RSA-AES256-SHA"));
                                       ...
                                       const map<string, string>& app_params)
{
  ...
  if (UseRFC4240Rooms) {
    ...
    if (req.user.length() < 5) throw AmSession::Exception(404, "Not Found");
...
ConferenceDialog::~ConferenceDialog()
{
  ...
  for (vector<AmAudioFile*>::iterator it = TTSFiles.begin();
       it != TTSFiles.end(); it++) {
    delete *it;
  }
#endif
...
void ConferenceDialog::onInvite(const AmSipRequest& req)
{
  ...
  if (!language.empty()) {
    ...
    if (get_audio_file(LONELY_USER_MSG, req.domain, language, lonely_user_file)
      ...
      if (get_audio_file(LONELY_USER_MSG, "", language, lonely_user_file)
          && !lonely_user_file.empty()) {
        ConferenceFactory::LonelyUserFile = lonely_user_file;
      }
    }
...
void ConferenceDialog::onSessionStart()
{
  ...
  if (dialedout) {
    ...
    AmSessionContainer::instance()->postEvent(
        ...
        new DialoutConfEvent(DoConfConnect, dialout_channel->getConfID()));
  }
  ...
  if (dialout_channel.get()) {
    ...
    if (listen_only)
      play_list.addToPlaylist(
          new AmPlaylistItem(dialout_channel.get(), (AmAudio*) NULL));
    else
      play_list.addToPlaylist(
          new AmPlaylistItem(dialout_channel.get(), dialout_channel.get()));
  }
  else {
    ...
    if (listen_only) {
      play_list.addToPlaylist(
          new AmPlaylistItem(channel.get(), (AmAudio*) NULL));
    }
    else
...
{
  if (dialout_channel.get()) disconnectDialout();
  ...
  if (do_ev) {
    if (dialedout) {
      switch (do_ev->event_id) {
        case DoConfConnect: connectMainChannel(); break;
      ...
      switch (do_ev->event_id) {
        ...
        case DoConfError:
          ...
          DBG("adding error tone to the playlist (dialedout = %i)\n",
              dialedout);
          play_list.addToPlayListFront(
              new AmPlaylistItem(ErrorTone.get(), NULL));
          break;
      }
    }
  ...
  switch (state) {
    ...
    case CS_dialout_connected:
      ...
      if (event == 10) { // '*'
        ...
        AmSessionContainer::instance()->postEvent(
            dialout_id, new DialoutConfEvent(DoConfConnect, getLocalTag()));
...
void ConferenceDialog::createDialoutParticipant(const string& uri_user)
{
  ...
  dialout_channel.reset(AmConferenceStatus::getChannel(
      getLocalTag(), getLocalTag(), RTPStream()->getSampleRate()));

  dialout_id = AmSession::getNewId();

  ConferenceDialog* dialout_session = new ConferenceDialog(
      conf_id, AmConferenceStatus::getChannel(getLocalTag(), dialout_id,
                                              RTPStream()->getSampleRate()));
...
void ConferenceDialog::disconnectDialout()
{
  if (dialedout) {
    if (dialout_channel.get()) {
      AmSessionContainer::instance()->postEvent(
          dialout_channel->getConfID(),
          new DialoutConfEvent(DoConfDisconnect, dialout_channel->getConfID()));
    }
  }
  else {
    AmSessionContainer::instance()->postEvent(
        dialout_id, new DialoutConfEvent(DoConfDisconnect, getLocalTag()));
                                  ...
                                  AmBasicSipDialog::Status old_dlg_status)
{
  ...
  if (!dialedout /*&& !transfer_req.get()*/) return;
    ...
    switch (dlg->getStatus()) {
      ...
      case AmSipDialog::Disconnected:
        ...
        if (dialout_channel.get()) {
          ...
          AmSessionContainer::instance()->postEvent(
              dialout_channel->getConfID(),
              new DialoutConfEvent(DoConfError, dialout_channel->getConfID()));
        }
        setStopped();
...
#ifdef WITH_SAS_TTS
void ConferenceDialog::onZRTPEvent(zrtp_event_t       event,
    ...
    case ZRTP_EVENT_IS_SECURE: {
      ...
      if (ZRTP_SAS_BASE32 == session->sas_values.rendering) {
        ...
        DBG("Got SAS values SAS1 '%s' and SAS2 '%s'\n",
            session->sas_values.str1.buffer, session->sas_values.str2.buffer);

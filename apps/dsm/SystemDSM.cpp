...
SystemDSM::SystemDSM(const DSMScriptConfig& config, const string& startDiagName,
                     bool reload)
    ...
    , reload(reload)
{
  ...
  for (map<string, string>::const_iterator it = config.config_vars.begin();
       it != config.config_vars.end(); it++)
...
void SystemDSM::process(AmEvent* event)
{
  ...
  if (event->event_id == DSM_EVENT_ID) {
    ...
    if (dsm_event) {
      engine.runEvent(&dummy_session, this, DSMCondition::DSMEvent,
                      &dsm_event->params);
      return;
    }
  }
  ...
  if (jsonrpc_ev) {
    ...
    if (resp_ev) {
      ...
      engine.runEvent(&dummy_session, this, DSMCondition::JsonRpcResponse,
                      &params);
    ...
    if (req_ev) {
      ...
      engine.runEvent(&dummy_session, this, DSMCondition::JsonRpcRequest,
                      &params);
  ...
  if (event->event_id == E_SIP_SUBSCRIPTION) {
    ...
    if (sub_ev) {
      ...
      engine.runEvent(&dummy_session, this, DSMCondition::SIPSubscription,
                      &params);
...
void SystemDSM::transferOwnership(DSMDisposable* d) { gc_trash.insert(d); }

/** release ownership of object from this session instance */
void SystemDSM::releaseOwnership(DSMDisposable* d) { gc_trash.erase(d); }
...
NOT_IMPLEMENTED(playRingtone(int length, int on, int off, int f, int f2,
                             bool front));

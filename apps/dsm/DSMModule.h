...
class DSMModule
{
 public:
  ...
  virtual bool onInvite(const AmSipRequest& req, DSMSession* sess)
  {
    return true;
  }
...
#define EXPORT_SC_FACTORY(fctname, class_name, args...)                        \
  extern "C" void* fctname() { return new class_name(##args); }
#else
#define EXPORT_SC_FACTORY(fctname, class_name, ...)                            \
  extern "C" void* fctname() { return new class_name(__VA_ARGS__); }
#endif

#define SC_EXPORT(class_name) EXPORT_SC_FACTORY(SC_FACTORY_EXPORT, class_name)

class SCStrArgAction : public DSMAction
{
 protected:
  string arg;
...
#define DEF_ACTION_2P(CL_Name)                                                 \
  class CL_Name : public DSMAction                                             \
  {                                                                            \
    string par1;                                                               \
    string par2;                                                               \
                                                                               \
...
#define SPLIT_ARGS(sep, optional)                                              \
  ...
  while (p < arg.size()) {                                                     \
    if (quot) {                                                                \
      if (last_c != '\\' && arg[p] == quot_c) quot = false;                    \
    }                                                                          \
    else {                                                                     \
  ...
  if (par1.length() && par1[0] == '\'') {                                      \
    par1        = trim(par1, "\'");                                            \
    size_t rpos = 0;                                                           \
    while ((rpos = par1.find("\\\'")) != string::npos) par1.erase(rpos, 1);    \
  }                                                                            \
  ...
  if (par2.length() && par2[0] == '\'') {                                      \
    par2        = trim(par2, "\'");                                            \
    size_t rpos = 0;                                                           \
    while ((rpos = par2.find("\\\'")) != string::npos) par2.erase(rpos, 1);    \
  }                                                                            \
...
#define CONST_ACTION_2P(CL_name, _sep, _optional)                              \
  CL_name::CL_name(const string& arg) { SPLIT_ARGS(_sep, _optional); }
...
#define EXEC_ACTION_STOP return false;

string resolveVars(const string s, AmSession* sess, DSMSession* sc_sess,
                   map<string, string>* event_params, bool eval_ops = false);

void splitCmd(const string& from_str, string& cmd, string& params);

#define DEF_CMD(cmd_name, class_name)                                          \
                                                                               \
  if (cmd == cmd_name) {                                                       \
    class_name* a = new class_name(params);                                    \
    a->name       = from_str;                                                  \
    return a;                                                                  \
  }

#define DEF_SCCondition(cond_name)                                             \
  class cond_name : public DSMCondition                                        \
  {                                                                            \
    string arg;                                                                \
    bool   inv;                                                                \
                                                                               \
...
#define DECLARE_MODULE(mod_cls_name)                                           \
  class mod_cls_name : public DSMModule                                        \
  {                                                                            \
   public:                                                                     \
    mod_cls_name() {}                                                          \
    ~mod_cls_name() {}                                                         \

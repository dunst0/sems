...
extern "C" void* plugin_class_create() { return DSMFactory::instance(); }

DSMFactory* DSMFactory::_instance = 0;

DSMFactory* DSMFactory::instance()
{
  if (_instance == NULL) _instance = new DSMFactory(MOD_NAME);
  return _instance;
}
...
{
  for (map<string, AmPromptCollection*>::iterator it = prompt_sets.begin();
       it != prompt_sets.end(); it++)
    delete it->second;

  for (std::set<DSMStateDiagramCollection*>::iterator it = old_diags.begin();
       it != old_diags.end(); it++)
    delete *it;
...
int DSMFactory::onLoad()
{
  ...
  if (!loadPrompts(cfg)) return -1;

  if (!loadPromptSets(cfg)) return -1;

  if (!loadDiags(cfg, MainScriptConfig.diags)) return -1;

  vector<string> registered_apps;
  if (!registerApps(cfg, MainScriptConfig.diags, registered_apps)) return -1;
  ...
  MainScriptConfig.RunInviteEvent =
      cfg.getParameter("run_invite_event") == "yes";

  MainScriptConfig.SetParamVariables =
      cfg.getParameter("set_param_variables") == "yes";

  vector<string> system_dsms =
      explode(cfg.getParameter("run_system_dsms"), ",");
  for (vector<string>::iterator it = system_dsms.begin();
       it != system_dsms.end(); it++) {
    string status;
...
#ifdef USE_MONITORING
  string monitoring_full_callgraph =
      cfg.getParameter("monitoring_full_stategraph");
  ...
  string monitoring_full_transitions =
      cfg.getParameter("monitoring_full_transitions");
  ...
  string filters;
  for (vector<string>::iterator it = MonSelectFilters.begin();
       it != MonSelectFilters.end(); it++) {
    if (it != MonSelectFilters.begin()) filters += ", ";
    filters += *it;
  }
  ...
  if (!conf_d_dir.empty()) {
    if (conf_d_dir[conf_d_dir.length() - 1] != '/') conf_d_dir += '/';
    ...
    if (!dir) {
      ...
      while (((entry = readdir(dir)) != NULL) && (err == 0)) {
        ...
        if (!loadConfig(conf_file_name, conf_name, false, NULL)) return -1;
      }
      closedir(dir);
    }
  }
...
{
  vector<string> prompts_files = explode(cfg.getParameter("load_prompts"), ",");
  for (vector<string>::iterator it = prompts_files.begin();
       it != prompts_files.end(); it++) {
  ...
  if (!has_all_prompts) return false;
...
bool DSMFactory::loadPromptSets(AmConfigReader& cfg)
{
  ...
  for (vector<string>::iterator it = prompt_sets_names.begin();
       it != prompt_sets_names.end(); it++) {
...
bool DSMFactory::loadDiags(AmConfigReader&            cfg,
                           DSMStateDiagramCollection* m_diags)
{
  ...
  for (vector<string>::iterator it = diags_names.begin();
       it != diags_names.end(); it++) {
    if (!m_diags->loadFile(DiagPath + *it + ".dsm", *it, DiagPath, ModPath,
                           DebugDSM, CheckDSM)) {
      ERROR("loading %s from %s\n", it->c_str(),
            (DiagPath + *it + ".dsm").c_str());
      return false;
    }
  }
...
bool DSMFactory::loadConfig(const string& conf_file_name,
                            const string& conf_name, bool live_reload,
                            DSMStateDiagramCollection* m_diags)
{
  string script_name = conf_name.substr(0, conf_name.length() - 5); // - .conf
  DBG("loading %s from %s ...\n", script_name.c_str(), conf_file_name.c_str());
  AmConfigReader cfg;
  if (cfg.loadFile(conf_file_name)) return false;

  DSMScriptConfig script_config;
  script_config.RunInviteEvent = cfg.getParameter("run_invite_event") == "yes";
  ...
  if (live_reload) {
    ...
    if (!loadPrompts(cfg)) return false;

    if (!loadPromptSets(cfg)) return false;
  }
  ...
  if (!loadDiags(cfg, used_diags)) return false;

  vector<string> registered_apps;
  if (!registerApps(cfg, used_diags, registered_apps)) return false;

  ScriptConfigs_mut.lock();
  try {
    Name2ScriptConfig[script_name] = script_config;
    // set ScriptConfig to this for all registered apps' names
    for (vector<string>::iterator reg_app_it = registered_apps.begin();
         reg_app_it != registered_apps.end(); reg_app_it++) {
      ...
      if (it != ScriptConfigs.end()) {
        ...
        if (it->second.diags != NULL) old_diags.insert(it->second.diags);
      }
...
void AmArg2DSMStrMap(const AmArg& arg, map<string, string>& vars)
{
  for (AmArg::ValueStruct::const_iterator it = arg.begin(); it != arg.end();
       it++) {
    if (it->second.getType() == AmArg::CStr)
      ...
      for (size_t i = 0; i < it->second.size(); i++) {
        if (it->second.get(i).getType() == AmArg::CStr)
          vars[it->first + "_" + int2str((unsigned int) i)] =
              it->second.get(i).asCStr();
        else
          vars[it->first + "_" + int2str((unsigned int) i)] =
              AmArg::print(it->second.get(i));
      }
    }
    else {
...
#define FALLBACK_OR_EXCEPTION(code, reason)                                    \
  ...
  if ((ret.getType() != AmArg::Array) || !ret.size()) {
  ...
  if ((sess_params.getType() != AmArg::Array) || !sess_params.size()
                                ...
                                const map<string, string>& app_params)
{
  ...
  DSMCall* s =
      new DSMCall(call_config, &prompts, *call_config.diags, start_diag, NULL);
  ...
  if (call_config.SetParamVariables) addParams(s, req.hdrs);

  if (!vars.empty()) addVariables(s, "", vars);
                                ...
                                AmArg& session_params)
{
  ...
  DSMCall* s =
      new DSMCall(call_config, &prompts, *call_config.diags, start_diag, cred);
...
bool DSMFactory::createSystemDSM(const string& config_name,
                                 const string& start_diag, bool reload,
                                 string& status)
{
  bool res = true;
  ...
  if (config_name == "main")
    ...
    map<string, DSMScriptConfig>::iterator it =
        Name2ScriptConfig.find(config_name);
    if (it != Name2ScriptConfig.end()) script_config = &it->second;
  }
  if (script_config == NULL) {
    status = "Error: Script config '" + config_name + "' not found, in [";
    for (map<string, DSMScriptConfig>::iterator it = Name2ScriptConfig.begin();
         it != Name2ScriptConfig.end(); it++) {
      if (it != Name2ScriptConfig.begin()) status += ", ";
      status += it->first;
    }
    status += "]";
    res = false;
...
int DSMFactory::preloadModules(AmConfigReader& cfg, string& res,
                               const string& ModPath)
{
  ...
  if (preload_names.size()) {
    for (vector<string>::iterator it = preload_names.begin();
         it != preload_names.end(); it++) {
      DBG("preloading '%s'...\n", it->c_str());
...
void DSMFactory::loadDSM(const AmArg& args, AmArg& ret)
{
  ...
  try {
    if (MainScriptConfig.diags->hasDiagram(dsm_name)) {
      ret.push(400);
      ret.push("DSM named '" + dsm_name
               + "' already loaded (use reloadDSMs to reload all)");
    }
    else {
      if (!MainScriptConfig.diags->loadFile(dsm_file_name, dsm_name, DiagPath,
                                            ModPath, DebugDSM, CheckDSM)) {
        ret.push(500);
  ...
  try {
    if (MainScriptConfig.diags->hasDiagram(dsm_name)) {
      ...
      if (!MainScriptConfig.diags->loadFile(diag_path + dsm_name + ".dsm",
                                            dsm_name, diag_path, mod_path,
                                            DebugDSM, CheckDSM)) {
        ret.push(500);
        ret.push("error loading " + dsm_name + " from " + diag_path + dsm_name
                 + ".dsm");
      }
      else {
        ret.push(200);
        ret.push("loaded " + dsm_name + " from " + diag_path + dsm_name
                 + ".dsm");
      }
    }
  }
                                        ...
                                        bool& SetParamVariables)
{
  ...
  try {
    map<string, DSMScriptConfig>::iterator it =
        Name2ScriptConfig.find(config_set);
...
void DSMFactory::invoke(const string& method, const AmArg& args, AmArg& ret)
{
  if (method == "postDSMEvent") {
    ...
    args.assertArrayFmt("ss");
    string status;
    if (createSystemDSM(args.get(0).asCStr(), args.get(1).asCStr(), false,
                        status)) {
      ret.push(200);
      ret.push(status);
    }

...
string resolveVars(const string ts, AmSession* sess, DSMSession* sc_sess,
                   map<string, string>* event_params, bool eval_ops)
{
  string s = ts;
  if (s.length()) {
    if (eval_ops) {
      // remove all spaces
      string::size_type p;
      for (p = s.find(" ", 0); p != string::npos; p = s.find(" ", p)) {
        s.erase(p, 1);
      }
    ...
    switch (s[0]) {
      case '$': {
        ...
        if (it != sc_sess->var.end()) return it->second;
        return "";
      }
      case '#':
        if (s.substr(1, 1) == "#") return "#";
        if (event_params) {
          ...
          if (it != event_params->end()) return it->second;
          return "";
        }
        else
          return string();

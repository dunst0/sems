...
{
  DBG("adding state '%s'\n", state.name.c_str());
  for (vector<DSMElement*>::const_iterator it = state.pre_actions.begin();
       it != state.pre_actions.end(); it++) {
    DBG("   pre-action '%s'\n", (*it)->name.c_str());
  }
  for (vector<DSMElement*>::const_iterator it = state.post_actions.begin();
       it != state.post_actions.end(); it++) {
    DBG("   post-action '%s'\n", (*it)->name.c_str());
  }
...
bool DSMStateDiagram::addTransition(const DSMTransition& trans)
{
  DBG("adding Transition '%s' %s -(...)-> %s\n", trans.name.c_str(),
      trans.from_state.c_str(), trans.to_state.c_str());
  for (vector<DSMCondition*>::const_iterator it = trans.precond.begin();
       it != trans.precond.end(); it++) {
    DBG("       DSMCondition  %s'%s'\n", (*it)->invert ? "not " : "",
        (*it)->name.c_str());
  }
  for (vector<DSMElement*>::const_iterator it = trans.actions.begin();
       it != trans.actions.end(); it++) {
    DBG("       Action     '%s'\n", (*it)->name.c_str());
  }
  ...
  if (trans.from_state.find_first_of("(") != string::npos) {
    ...
    for (vector<string>::iterator it = from_states.begin();
      ...
      if (it->length() && (*it)[0] == ' ') *it = it->substr(1);
...
{
  // find target state
  for (vector<State>::iterator target_st = states.begin();
       target_st != states.end(); target_st++) {
    if (target_st->name == s_name) return &(*target_st);
  }
...
bool DSMStateDiagram::checkDestinationStates(string& report)
{
  ...
  for (vector<State>::iterator it = states.begin(); it != states.end(); it++) {
    for (vector<DSMTransition>::iterator t_it = it->transitions.begin();
         t_it != it->transitions.end(); t_it++) {
      if (NULL == getState(t_it->to_state)) {
        report += name + ": State '" + it->name + "' Transition '" + t_it->name
                  + "' : Destination state '" + t_it->to_state
                  + "' is not defined\n";
        res = false;
      }
    }
...
bool DSMStateDiagram::checkHangupHandled(string& report)
{
  ...
  for (vector<State>::iterator it = states.begin(); it != states.end(); it++) {
    ...
    for (vector<DSMTransition>::iterator t_it = it->transitions.begin();
         t_it != it->transitions.end(); t_it++) {
      for (vector<DSMCondition*>::iterator c_it = t_it->precond.begin();
           c_it != t_it->precond.end(); c_it++) {
  ...
  for (vector<DSMElement*>::iterator it = from; it != to; it++) {
    ...
    if (dsm_act) {
      ...
      if ((dsm_act)->execute(sess, sc_sess, event, event_params)) {
        string se_modifier;
        switch ((dsm_act)->getSEAction(se_modifier, sess, sc_sess, event,
                                       event_params)) {
          case DSMAction::Repost: is_consumed = false; break;
          case DSMAction::Jump:
            DBG("jumping to %s\n", se_modifier.c_str());
    ...
    if (cond_tree) {
      ...
      while (con != cond_tree->conditions.end()) {
        if (!(*con)->_match(sess, sc_sess, event, event_params)) break;
        con++;
      }
      if (con == cond_tree->conditions.end()) {
        DBG("condition tree matched.\n");
        if (runactions(cond_tree->run_if_true.begin(),
                       cond_tree->run_if_true.end(), sess, sc_sess, event,
                       event_params, is_consumed, do_break))
          return true;
      }
      else {
        if (runactions(cond_tree->run_if_false.begin(),
                       cond_tree->run_if_false.end(), sess, sc_sess, event,
                       event_params, is_consumed, do_break))
          return true;
      }
      continue;
    ...
    if (array_for) {
      if (array_for->for_type == DSMArrayFor::Range) {
        ...
        DBG("running for (%s%s in %s) {\n", array_for->k.c_str(),
            array_for->v.empty() ? "" : ("," + array_for->v).c_str(),
      ...
      if (array_for->for_type == DSMArrayFor::Struct) {
        ...
        while (lb != sc_sess->var.end()) {
          if ((lb->first.length() < array_name.length())
              || strncmp(lb->first.c_str(), array_name.c_str(),
                         array_name.length()))
            break;
        ...
        VarMapT::iterator v =
            sc_sess->var.lower_bound(array_name + "[" + int2str(a_index) + "]");
        ...
        if (!str2int(s_range, range[0])) {
          ...
          s_range =
              resolveVars(array_for->array_struct, sess, sc_sess, event_params);
      ...
      if (array_for->for_type == DSMArrayFor::Range) {
        ...
        while (cnt != range[1]) {
          ...
          runactions(array_for->actions.begin(), array_for->actions.end(), sess,
                     sc_sess, event, event_params, is_consumed, break_loop);
        ...
        for (vector<pair<string, string>>::iterator f_it = cnt_values.begin();
             f_it != cnt_values.end(); f_it++) {
          if (array_for->for_type == DSMArrayFor::Struct) {
...
{
  for (vector<DSMModule*>::iterator it = modules.begin(); it != modules.end();
       it++)
    mods.push_back(*it);
}
                          ...
                          map<string, string>* event_params)
{
  ...
  return invert ? (!match(sess, sc_sess, event, event_params))
                : match(sess, sc_sess, event, event_params);
}
  ...
  do {
    try {
      ...
      for (vector<DSMTransition>::iterator tr = current->transitions.begin();
        ...
        if (tr->is_exception != is_exception) continue;
        ...
        if (con == tr->precond.end()) {
          ...
          if (!target_st) {
            ERROR("script writer error: transition '%s' from "
                  ...
                  tr->name.c_str(), current->name.c_str(),
                  tr->to_state.c_str());
          }
          ...
          if (current->post_actions.size()) {
            ...
            if (runactions(current->post_actions.begin(),
                           current->post_actions.end(), sess, sc_sess,
                           active_event, active_params, is_consumed,
                           do_break)) {
              break;
            }
          ...
          if (tr->actions.size()) {
            ...
            bool do_break = false;
            if (runactions(tr->actions.begin(), tr->actions.end(), sess,
                           sc_sess, active_event, active_params, is_consumed,
                           do_break)) {
              break;
            }
            // further semantincs of break here?
          ...
          if (DSMFactory::MonitoringFullTransitions) {
            MONITORING_LOG_ADD(sess->getLocalTag().c_str(), "dsm_stategraph",
                               ("> " + tr->name + " >").c_str());
          }

          if (DSMFactory::MonitoringFullCallgraph) {
            MONITORING_LOG_ADD(
                sess->getLocalTag().c_str(), "dsm_stategraph",
          ...
          if (current->pre_actions.size()) {
            ...
            if (runactions(current->pre_actions.begin(),
                           current->pre_actions.end(), sess, sc_sess,
                           active_event, active_params, is_consumed,
                           do_break)) {
              break;
            }
                              ...
                              map<string, string>* event_params)
{
  for (vector<DSMStateDiagram*>::iterator it = diags.begin(); it != diags.end();
    ...
    if ((*it)->getName() == diag_name) {
      ...
      MONITORING_LOG2(sess->getLocalTag().c_str(), "dsm_diag",
                      diag_name.c_str(), "dsm_state", current->name.c_str());
      ...
      bool do_break    = false;
      runactions(current->pre_actions.begin(), current->pre_actions.end(), sess,
                 sc_sess, event, event_params, is_finished, do_break);
                                ...
                                map<string, string>* event_params)
{
  ...
  MONITORING_LOG2(sess->getLocalTag().c_str(), "dsm_diag",
                  current_diag->getName().c_str(), "dsm_state",
                  current->name.c_str());

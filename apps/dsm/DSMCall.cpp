...
DSMCall::~DSMCall()
{
  for (set<DSMDisposable*>::iterator it = gc_trash.begin();
       it != gc_trash.end(); it++)
    delete *it;
  ...
  for (set<AmPromptCollection*>::iterator it = used_prompt_sets.begin();
       it != used_prompt_sets.end(); it++)
...
void DSMCall::onInvite(const AmSipRequest& req)
{
  ...
  if (run_session_invite) AmB2BCallerSession::onInvite(req);
}
...
void DSMCall::onRinging(const AmSipReply& reply)
{
  ...
  params["has_body"] = reply.body.empty() ? "false" : "true";
...
void DSMCall::onEarlySessionStart()
{
  ...
  if (checkVar(DSM_CONNECT_EARLY_SESSION, DSM_CONNECT_EARLY_SESSION_FALSE)) {
    ...
    if (!getInput()) setInput(&playlist);

    if (!getOutput()) setOutput(&playlist);
...
void DSMCall::onSipReply(const AmSipRequest& req, const AmSipReply& reply,
...
void DSMCall::onRemoteDisappeared(const AmSipReply& reply)
{
  ...
  if (checkParam(DSM_PROCESSED, DSM_TRUE, &params)) {
    DBG("DSM script processed SIP onRemoteDisappeared reply '%u %s', "
        "returning\n",
...
#ifdef WITH_ZRTP
void DSMCall::onZRTPProtocolEvent(zrtp_protocol_event_t event,
...
void DSMCall::onZRTPSecurityEvent(zrtp_security_event_t event,
                                  zrtp_stream_t*        stream_ctx)
{
...
void DSMCall::process(AmEvent* event)
{
  ...
  if (jsonrpc_ev) {
    ...
    if (resp_ev) {
      ...
      params["is_error"] = resp_ev->response.is_error ? "true" : "false";

      // decode result for easy use from script
      varPrintArg(resp_ev->response.data, params,
                  resp_ev->response.is_error ? "error" : "result");
...
inline UACAuthCred* DSMCall::getCredentials() { return cred.get(); }

void DSMCall::playPrompt(const string& name, bool loop, bool front)
{
  DBG("playing prompt '%s'\n", name.c_str());
  if (prompts->addToPlaylist(name, (long) this, playlist, front, loop)) {
    if ((var["prompts.default_fallback"] != "yes")
        || default_prompts->addToPlaylist(name, (long) this, playlist, front,
                                          loop)) {
      DBG("checked [%p]\n", default_prompts);
...
void DSMCall::playRingtone(int length, int on, int off, int f, int f2,
                           bool front)
{
...
void DSMCall::addPromptSet(const string& name, AmPromptCollection* prompt_set)
{
  if (prompt_set) {
...
void DSMCall::setPromptSets(map<string, AmPromptCollection*>& new_prompt_sets)
{
...
{
  map<string, AmPromptCollection*>::iterator it = prompt_sets.find(name);
...
void DSMCall::B2BconnectCallee(const string& remote_party,
                               const string& remote_uri, bool relayed_invite)
{
...
AmB2BCalleeSession* DSMCall::newCalleeSession()
{
  ...
  if (!user.empty() && !pwd.empty()) {
    ...
    if (NULL == uac_auth_f) {
      INFO("uac_auth module not loaded. uac auth NOT enabled for B2B b leg in "
           "DSM.\n");
    }
    else {
...
void DSMCall::B2BclearHeaders() { invite_req.hdrs.clear(); }
...
UACAuthCred* DSMCallCalleeSession::getCredentials() { return cred.get(); }
...
void DSMCallCalleeSession::onSipReply(const AmSipRequest&      req,

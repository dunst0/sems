...
{
  while (pos < str.length() && is_wsp(str[pos])) pos++;
...
DSMFunction* DSMChartReader::functionFromToken(const string& str)
{
  ...
  for (vector<DSMFunction*>::iterator it = funcs.begin(); it != funcs.end();
       it++) {
    if ((*it)->name == cmd) {
...
{
  string forhdr = token;
  if (forhdr.length() < 2 || forhdr[0] != '('
      || forhdr[forhdr.length() - 1] != ')') {
  ...
  else if (forh_v[1].length() > 7 && forh_v[1].substr(0, 6) == "range("
  ...
  while (pos < chart.length()) {
    ...
    if (f) {
      if (f->name.length() == 0) {
        ...
        if (b_pos != string::npos) {
          ...
          ERROR(
              "Parse error -- function declarations must have a name followed "
    ...
    if (al) {
      ...
      if ((token == "}") || (token == "->")) {
        ...
        if (al->al_type == ActionList::AL_if
          ...
          DSMConditionTree* ct =
              dynamic_cast<DSMConditionTree*>(&(*stack.back()));
          if (!ct) {
      ...
      if (token == "else") {
        DBG(" ... else ...\n");
        DSMConditionTree* ct =
            dynamic_cast<DSMConditionTree*>(al->actions.back());
        if (NULL == ct) {
      ...
      if (token.substr(0, 3) == "for") {
        ...
        if (token.length() > 3) {
          if (!forFromToken(*af, token.substr(3))) return false;
        }
    ...
    if (af) {
      if (af->array_struct.length() || af->k.length()) {
        ...
        if (!forFromToken(*af, token)) return false;
      }

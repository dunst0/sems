...
static PyObject* IvrUAC_dialout(IvrUAC* self, PyObject* args)
{
  ...
  if (!PyArg_ParseTuple(args, "ssssss", &user, &app_name, &r_uri, &from,
                        &from_uri, &to))
    return NULL;

  string app_name_s(app_name);

  AmUAC::dialout(user, app_name_s, r_uri, from, from_uri, to);
...
static PyMethodDef IvrUAC_methods[] = {
    {"dialout", (PyCFunction) IvrUAC_dialout, METH_VARARGS, "place a new call"},
    {NULL} /* Sentinel */
};

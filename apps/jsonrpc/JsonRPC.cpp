...
int JsonRPCServerModule::onLoad() { return instance()->load(); }
...
int JsonRPCServerModule::load()
{
  ...
  if (cfg.loadFile(AmConfig::ModConfigPath + string(MOD_NAME ".conf"))) {
    ...
    threads =
        cfg.getParameterInt("server_threads", DEFAULT_JSONRPC_SERVER_THREADS);
  }
...
void JsonRPCServerModule::invoke(const string& method, const AmArg& args,
                                 AmArg& ret)
{
  if (method == "execRpc") {
    // todo: add connection id
    args.assertArrayFmt(
        "sssisis"); // evq_link, notificationReceiver, requestReceiver,
    ...
    args.assertArrayFmt(
        "sisss"); // conn_id, type, method, id, reply_sink, [params]
    if (args.size() > 5) {
    ...
    JsonRpcServer::execRpc(args.get(0).asCStr(), args.get(1).asCStr(),
                           args.get(2), ret);
...
void JsonRPCServerModule::execRpc(const AmArg& args, AmArg& ret)
{
  ...
  JsonRPCServerLoop::execRpc( // evq_link, notification_link, request_link
      ...
      args.get(3).asInt(),
      // host, port, method
      args.get(4).asCStr(), args.get(5).asInt(), args.get(6).asCStr(), params,
      udata, ret);
}

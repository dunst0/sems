...
int JsonRpcServer::createRequest(const string& evq_link, const string& method,
                                 ...
                                 const AmArg& udata, bool is_notification)
{
  AmArg rpc_params;
  ...
  DBG("RPC message: >>%.*s<<\n", (int) rpc_params_json.length(),
      rpc_params_json.c_str());
                                  ...
                                  JsonrpcPeerConnection* peer)
{
  ...
  bool is_request =
      (rpc_params.hasMember("method") && isArgCStr(rpc_params["method"]));
  if (!is_request) {
    ...
    std::map<std::string, std::pair<std::string, AmArg>>::iterator rep_recv_q =
    ...
    if (rpc_params.hasMember("result")) {
      resp_ev =
          new JsonRpcResponseEvent(false, id, rpc_params["result"], udata);
    }
    else {
    ...
    bool posted = AmEventDispatcher::instance()->post(event_queue_id, resp_ev);
    if (!posted) {
  ...
  if ((id.empty() && !peer->notificationReceiver.empty())
    ...
    *msg_size = 0;
    ...
    string dst_evqueue = id.empty() ? peer->notificationReceiver.c_str()
                                    : peer->requestReceiver.c_str();

    DBG("directly passing %s to event queue '%s'\n",
        id.empty() ? "notification" : "request", dst_evqueue.c_str());
    AmArg  none_params;
    ...
    bool posted = AmEventDispatcher::instance()->post(dst_evqueue, request_ev);
    ...
    if (!posted) {
      ...
      if (id.empty()
          && (peer->flags & JsonrpcPeerConnection::FL_CLOSE_NO_NOTIF_RECV)) {
        INFO("closing connection on missing notification receiver queue\n");
        return -1; // todo: reply error?
      }

      if (!id.empty()
          && (peer->flags & JsonrpcPeerConnection::FL_CLOSE_NO_REQUEST_RECV)) {
...
void JsonRpcServer::execRpc(const string& method, const string& id,
                            const AmArg& params, AmArg& rpc_res)
{
  try {
    ...
    try {
      ...
      throw JsonRpcError(-32000, "Server error", "unexpected Exception");
    }
                                  ...
                                  AmArg& res)
{
  if (method == "calls") {
    ...
    throw JsonRpcError(-32601, "Method not found", "function unknown in core");
  }
}

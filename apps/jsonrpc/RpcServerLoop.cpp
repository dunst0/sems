...
// ((struct JsonrpcClientConnection*) (((char*)w) - offsetof(struct
// JsonrpcClientConnection,ev_write)));
//   if (revents & EV_WRITE){
...
static void read_cb(struct ev_loop* loop, struct ev_io* w, int revents)
{
  struct JsonrpcNetstringsConnection* peer =
      ((struct JsonrpcNetstringsConnection*) (((char*) w)
                                              - offsetof(
  ...
  // char rbuff[1024];
  if (revents & EV_READ) {
    // read message - here in main server thread (more efficient for small
    // messages)
    ...
    switch (res) {
      ...
      case JsonrpcNetstringsConnection::DISPATCH: {
        ev_io_stop(EV_A_ w);
        JsonRPCServerLoop::dispatchServerEvent(
            new JsonServerEvent(peer, JsonServerEvent::StartReadLoop));
      }
        return;
    }
    return;
...
static void accept_cb(struct ev_loop* loop, struct ev_io* w, int revents)
{
  ...
  JsonrpcNetstringsConnection* peer =
      new JsonrpcNetstringsConnection(connection_id);
  peer->fd = client_fd;
...
void JsonRPCServerLoop::_processEvents() { instance()->processEvents(); }
  ...
  switch (server_event->event_id) {
    ...
    case JsonServerEvent::SendMessage: {
      ...
      if (ev_is_active(&peer->ev_read)) {
        ...
        JsonRPCServerLoop::dispatchServerEvent(
            new JsonServerSendMessageEvent(*snd_msg_ev, peer));
      }
      else {
        // peer is being processed by thread, save event for later sending
        pending_events_mut.lock();
        // (need to copy event here: original event is deleted when event
        // processed)
    ...
    default: ERROR("unknown server event type received\n"); return;
  }
}
...
void JsonRPCServerLoop::returnConnection(JsonrpcNetstringsConnection* conn)
{
  ...
  instance()->postEvent(
      new JsonServerEvent(conn, JsonServerEvent::StartReadLoop));
...
void JsonRPCServerLoop::execRpc(const string& evq_link,
                                ...
                                const string& method, const AmArg& params,
                                const AmArg& udata, AmArg& ret)
{
  ...
  JsonServerSendMessageEvent* send_message_event =
      new JsonServerSendMessageEvent(connection_id, false, method,
                                     "1" /* id - not empty */, params, udata,
                                     evq_link);

  JsonRPCServerLoop::dispatchServerEvent(send_message_event);

  // JsonRPCServerLoop::
  //   dispatchServerEvent(new JsonServerEvent(peer,
...
void JsonRPCServerLoop::sendMessage(const string& connection_id, int msg_type,
                                    const string& method, const string& id,
                                    const string& reply_sink,
                                    const AmArg& params, const AmArg& udata,
                                    AmArg& ret)
{
  ...
  JsonServerSendMessageEvent* ev = new JsonServerSendMessageEvent(
      connection_id, msg_type != JSONRPC_MSG_REQUEST, method, id, params, udata,
      reply_sink);
...
string JsonRPCServerLoop::newConnectionId() { return AmSession::getNewId(); }

bool JsonRPCServerLoop::registerConnection(JsonrpcPeerConnection* peer,
                                           const string&          id)
{
  bool res = false;

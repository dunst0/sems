...
int RegistrationAgentFactory::onLoad()
{
  ...
  do {
    ...
    if (!ri.domain.length() || !ri.user.length()) {
      ...
      DBG("no mandatory config parameters '" CFG_PARAM_DOMAIN
          "' and '" CFG_PARAM_USER
          "' provided for entry #%d; configuration halted.\n",
          i);
      break;
    }
...
void RegThread::add_reg(const RegInfo& ri) { registrations.push_back(ri); }

void RegThread::create_registration(RegInfo& ri)
{
  AmDynInvokeFactory* di_f =
      AmPlugIn::instance()->getFactory4Di("registrar_client");
  if (di_f == NULL) {
    ...
    if (registrar_client_i != NULL) {
      ...
      if (reg_handle.size()) ri.handle = reg_handle.get(0).asCStr();
    }
  }
}
...
{
  if (!ri.handle.length()) return false;
  AmDynInvokeFactory* di_f =
      AmPlugIn::instance()->getFactory4Di("registrar_client");
  if (di_f == NULL) {
    ERROR("unable to get a registrar_client\n");
    ...
    if (registrar_client_i != NULL) {
      ...
      if (res.size()) {
        if (!res.get(0).asInt()) return false; // does not exist
...
void RegThread::on_stop() { DBG("not stopping...\n"); }

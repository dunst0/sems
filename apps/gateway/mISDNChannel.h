...
class mISDNChannel : public AmAudio
{
 public:
  ...
  // this buffer is feeded by isdn in 128byte chunks, while media processor eats
  // 160bytes each time.
  ...
  int placeCall(const AmSipRequest& req, std::string tonumber,
                std::string fromnumber); /* place a call */
  int call();

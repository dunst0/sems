...
int GatewayFactory::onLoad()
{
  ...
  if (gwconf.loadFile(AmConfig::ModConfigPath + string(MODULE_NAME)
                      + ".conf")) {
    //  if (gwconf.loadFile(AmConfig::ModConfigPath +"gateway.conf")) {
    DBG("cant load conf file %s/%s.conf\n", AmConfig::ModConfigPath.c_str(),
        MODULE_NAME);
    exit(-1);
  }
...
AmSession* GatewayFactory::onInvite(const AmSipRequest& req)
{
  ...
  //        }
  DBG("calling (mISDNStack::instance())->placeCall(req, session, tonumber, "
      "fromnumber);\n");
  int ret =
      (mISDNStack::instance())->placeCall(req, session, req.user, req.from);
  if (ret == FAIL) {
...
// call actually starts in isdn module as CC_SETUP, and there
// GWSession::CallFromOutSide is executed
// which calls AmUac::dialout which in turn calls this function
// session pointer is returned back so we can finish filling object with data
// there
AmSession* GatewayFactory::onInvite(const AmSipRequest& req,

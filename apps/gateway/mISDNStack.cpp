...
{
  std::map<int, mISDNChannel*>::iterator iter =
      BC_map.find(frame->addr & STACK_ID_MASK);
  ;
  if (iter != BC_map.end()) {
...
int mISDNStack::placeCall(const AmSipRequest& req, GWSession* session,
                          ...
                          const std::string& fromnumber)
{
  ...
  DBG("calling ((mISDNChannel*)m_pstndevice)->placeCall(m_req, tonumber, "
      "fromnumber);\n");
...
int mISDNStack::GetPortInfo()
{
  ...
  if ((device = mISDN_open()) < 0) {
    ERROR("mISDNStack::mISDNStack: mISDN_open() failed: ret=%d errno=%d (%s) "
          "Check for mISDN modules and device.\n",
  ...
  while (i <= num_cards) {
    ...
    if (err <= 0) {
      ERROR("mISDN_get_stack_info() failed: port=%d err=%d\n", i, err);
      break;
    }
    ...
    switch (stinf->pid.protocol[0] & ~ISDN_PID_FEATURE_MASK) {
      case ISDN_PID_L0_TE_S0:
        INFO("Port %2d: TE-mode BRI S/T interface line (for phone lines)\n", i);
        break;
      case ISDN_PID_L0_NT_S0:
        nt = 1;
        INFO("Port %2d: NT-mode BRI S/T interface port (for phones)\n", i);
        break;
      case ISDN_PID_L0_TE_E1:
        pri = 1;
        INFO("Port %2d: TE-mode PRI E1  interface line (for phone lines)\n", i);
        break;
      case ISDN_PID_L0_NT_E1:
        nt  = 1;
        pri = 1;
        INFO("Port %2d: NT-mode PRI E1  interface port (for phones)\n", i);
        break;
      default:
        useable = 0;
        ERROR("unknown type 0x%08x\n", stinf->pid.protocol[0]);
    }
    if (nt) {
      if (stinf->pid.protocol[1] == 0) {
        useable = 0;
        INFO(" -> Missing layer 1 NT-mode protocol.\n");
      }
      p = 2;
      while (p <= MAX_LAYER_NR) {
        if (stinf->pid.protocol[p]) {
          useable = 0;
          INFO(" -> Layer %d protocol 0x%08x is detected, but not allowed for "
               "NT.\n",
      ...
      if (useable) {
        if (pri)
          INFO(" -> Interface is Point-To-Point (PRI).\n");
        else
          INFO(" -> Interface can be Poin-To-Point/Multipoint.\n");
      }
    }
    else {
      if (stinf->pid.protocol[1] == 0) {
        useable = 0;
        INFO(" -> Missing layer 1 protocol.\n");
      }
      if (stinf->pid.protocol[2] == 0) {
        useable = 0;
        INFO(" -> Missing layer 2 protocol.\n");
      }
      if (stinf->pid.protocol[2] & ISDN_PID_L2_DF_PTP) {
        INFO(" -> Interface is Poin-To-Point.\n");
      }
      if (stinf->pid.protocol[3] == 0) {
        useable = 0;
        INFO(" -> Missing layer 3 protocol.\n");
      }
      else {
        switch (stinf->pid.protocol[3] & ~ISDN_PID_FEATURE_MASK) {
          case ISDN_PID_L3_DSS1USER:
            INFO(" -> Protocol: DSS1 (Euro ISDN)\n");
            break;
          default:
            useable = 0;
            INFO(" -> Protocol: unknown protocol 0x%08x\n",
    ...
    if (!useable)
      ERROR(
          " * Port %2d  NOT useable. (maybe somethind is using it already?)\n",
          i);
    i++;
  }
  /* close mISDN */
...
int mISDNStack::init()
{
  ...
  if ((m_mISDNdevice = mISDN_open()) < 0) {
    ERROR("mISDNStack::init: mISDN_open() failed: ret=%d errno=%d (%s) Check "
          "for mISDN modules and device.\n",
          m_mISDNdevice, errno, strerror(errno));
    return FAIL;
  }
  DBG("mISDNStack::init: mISDN_opened %d\n", m_mISDNdevice);
  /* create entity for layer 3 TE-mode */
  mISDN_write_frame(m_mISDNdevice, buff, 0, MGR_NEWENTITY | REQUEST, 0, 0, NULL,
                    TIMEOUT_1SEC);
  ret = mISDN_read_frame(m_mISDNdevice, frm, sizeof(iframe_t), 0,
                         MGR_NEWENTITY | CONFIRM, TIMEOUT_1SEC);
  if (ret < (int) mISDN_HEADER_LEN) {
    ERROR("Cannot request MGR_NEWENTITY from mISDN. header too small.\n");
    return FAIL;
  }
  m_entity = frm->dinfo & 0xffff;
  if (!m_entity) {
    ERROR("Cannot request MGR_NEWENTITY from mISDN. Exitting due to software "
          "bug.\n");
    return FAIL;
  }
  DBG("our entity for l3-processes is 0x%08x.\n", m_entity);
  m_crcount =
      1; // entity and crcount is used to generate uniqe addr for outgoing calls
  ...
  for (port = 1; port <= cnt; port++) {
    ...
    if (ret < 0) {
      ERROR("Cannot get stack info for port %d (ret=%d)\n", port, ret);
    }
    stinf = (stack_info_t*) &frm->data.p;
    switch (stinf->pid.protocol[0] & ~ISDN_PID_FEATURE_MASK) {
      case ISDN_PID_L0_TE_S0:
        DBG("TE-mode BRI S/T interface line\n");
        pri = 0;
        nt  = 0;
        break;
      case ISDN_PID_L0_NT_S0:
        DBG("NT-mode BRI S/T interface port\n");
        pri = 0;
        nt  = 1;
        break;
      case ISDN_PID_L0_TE_E1:
        DBG("TE-mode PRI E1  interface line\n");
        pri = 1;
        nt  = 0;
        break;
      case ISDN_PID_L0_NT_E1:
        DBG("LT-mode PRI E1  interface port\n");
        pri = 1;
        nt  = 1;
        break;
      default:
    ...
    if (nt) {
      DBG("Port %d (nt)  proto1=0x%08x proto2=0x%08x\n", port,
          stinf->pid.protocol[1], stinf->pid.protocol[2]);
      if (stinf->pid.protocol[1] == 0) {
        ERROR("Given port %d: Missing layer 1 NT-mode protocol.\n", port);
      }
      if (stinf->pid.protocol[2]) {
        ERROR("Given port %d: Layer 2 protocol 0x%08x is detected, but not "
              "allowed for NT.\n",
      ...
      DBG("Port %d (te)  proto1=0x%08x proto2=0x%08x proto3=0x%08x "
          "proto4=0x%08x (nul proto4 is good here)\n",
          port, stinf->pid.protocol[1], stinf->pid.protocol[2],
          stinf->pid.protocol[3], stinf->pid.protocol[4]);
      if (stinf->pid.protocol[1] == 0) {
        ERROR("Given port %d: Missing layer 1 protocol.\n", port);
      }
      if (stinf->pid.protocol[2] == 0) {
        ERROR("Given port %d: Missing layer 2 protocol.\n", port);
      }
      if (stinf->pid.protocol[2] & ISDN_PID_L2_DF_PTP) {
        ptp = 1;
        DBG("Port %d is point-to-point.\n", port);
      }
      else {
        ptp = 0;
      }
      if (stinf->pid.protocol[3] == 0) {
        ERROR("Given port %d: Missing layer 3 protocol.\n", port);
      }
      else {
        switch (stinf->pid.protocol[3] & ~ISDN_PID_FEATURE_MASK) {
          case ISDN_PID_L3_DSS1USER: break;
          default:
            ERROR("Given port %d: own protocol 0x%08x", port,
                  stinf->pid.protocol[3]);
        }
      }
      if (stinf->pid.protocol[4]) {
        ERROR("Given port %d: Layer 4 protocol not allowed.\n", port);
      }
    }
    ...
    *mISDNportp = mISDNport;
    ...
    if (ret) {
      ERROR("Cannot add layer4 of port %d (ret %d)\n", port, ret);
      //	                mISDNport_close(mISDNport);
      return FAIL;
    }
    mISDNport->upper_id = li.id;
    ret                 = mISDN_register_layer(m_mISDNdevice, mISDNport->d_stid,
                               mISDNport->upper_id);
    if (ret) {
      ERROR("Cannot register layer4 of port %d\n", port);
      //	                mISDNport_close(mISDNport);
      return FAIL;
    }
    mISDNport->lower_id =
        mISDN_get_layerid(m_mISDNdevice, mISDNport->d_stid, 3);
    if (mISDNport->lower_id < 0) {
      ERROR("Cannot get layer(%d) id of port %d\n", nt ? 1 : 3, port);
      //	                mISDNport_close(mISDNport);
      return FAIL;
    }
    mISDNport->upper_id =
        mISDN_get_layerid(m_mISDNdevice, mISDNport->d_stid, 4);
    if (mISDNport->upper_id < 0) {
      ERROR("Cannot get layer4 id of port %d\n", port);
      //	                mISDNport_close(mISDNport);
      return FAIL;
    }
    DBG("Layer 4 of port %d added (0x%08x) lower_id=0x%08x.\n", port,
        mISDNport->upper_id, mISDNport->lower_id);
...
void mISDNStack::on_stop(void)
{
  ...
  if (m_mISDNdevice >= 0) {
    mISDN_write_frame(m_mISDNdevice, buff, 0, MGR_DELENTITY | REQUEST, m_entity,
                      0, NULL, TIMEOUT_1SEC);
    mISDN_close(m_mISDNdevice);
    m_mISDNdevice = -1;
...
void mISDNStack::run()
{
  ...
  while (true) {
    //		DBG("tick\n");
    msg_buf_s =
        mISDN_read(m_mISDNdevice, &msg_buf, MAX_MSG_SIZE, TIMEOUT_10SEC);
    ;
    if (msg_buf_s == -1) {
      switch (errno) {
        case EINTR:
    ...
    if (frame->dinfo == (signed long) 0xffffffff
        && frame->prim == (PH_DATA | CONFIRM)) {
      ERROR(
    ...
    while (port) {
      if ((frame->addr & MASTER_ID_MASK)
          == (unsigned int) (port->upper_id & MASTER_ID_MASK))
        break;
      port = port->next;
    }
    if (!port) {
      ERROR("message belongs to no mISDNport: prim(0x%x) addr(0x%x) "
            "msg->len(%d)\n",
    ...
    if (frame->addr & FLG_CHILD_STACK) { /* child stack */
      ...
      if (channel == NULL) {
        DBG("b-channel is not associated to an ISDNPort (address 0x%x), "
            "ignoring.\n",
            frame->addr);
        continue;
      }
      ...
      if (msg_buf_s > 16)
        DBG("IE: %s", mISDNStack::dumpIE(msg_buf, msg_buf_s).c_str());
      ...
      switch (frame->prim) {
        ...
        case CC_RELEASE_CR | INDICATION:
          ...
          if (channel != NULL) {
            DBG("should delete channel=%p but we will leave it to have media "
                "procesor happy just unregister CR from map\n",
                channel);
            channel->unregister_CR();
            //	                   		delete(channel);
          }
          else
            ERROR("Channel not found for CC_RELEASE_CR | INDICATION %s (%d) "
                  "prim(0x%x) addr(0x%x) msg->len(%d) \n",
                  port->name, port->portnum, frame->prim, frame->addr,
                  msg_buf_s);
          continue;
          break;
        ...
        case MGR_SHORTSTATUS | CONFIRM:
          DBG("MGR_SHORTSTATUS ind or confirm for %s (%d) prim(0x%x) "
              "addr(0x%x) msg->len(%d) \n",
              port->name, port->portnum, frame->prim, frame->addr, msg_buf_s);
          switch (frame->dinfo) {
            case SSTATUS_L1_ACTIVATED:
              port->l1link = 1;
              DBG("MGR_SHORTSTATUS->SSTATUS_L1_ACTIVATED for %s (%d) "
                  "prim(0x%x) addr(0x%x) msg->len(%d) \n",
                  port->name, port->portnum, frame->prim, frame->addr,
                  msg_buf_s);
              break;
            case SSTATUS_L1_DEACTIVATED:
              port->l1link = 0;
              DBG("MGR_SHORTSTATUS->SSTATUS_L1_DEACTIVATED for %s (%d) "
                  "prim(0x%x) addr(0x%x) msg->len(%d) \n",
                  port->name, port->portnum, frame->prim, frame->addr,
                  msg_buf_s);
              break;
            case SSTATUS_L2_ESTABLISHED:
              port->l2link = 1;
              DBG("MGR_SHORTSTATUS->SSTATUS_L2_ESTABLISHED for %s (%d) "
                  "prim(0x%x) addr(0x%x) msg->len(%d) \n",
                  port->name, port->portnum, frame->prim, frame->addr,
                  msg_buf_s);
              break;
            case SSTATUS_L2_RELEASED:
              port->l2link = 0;
              DBG("MGR_SHORTSTATUS->SSTATUS_L2_RELEASED for %s (%d) prim(0x%x) "
                  "addr(0x%x) msg->len(%d) \n",
                  port->name, port->portnum, frame->prim, frame->addr,
                  msg_buf_s);
              break;
        ...
        case PH_ACTIVATE | INDICATION:
          DBG("PH_ACTIVATE ind or confirm for %s (%d) prim(0x%x) addr(0x%x) "
              "msg->len(%d) \n",
              port->name, port->portnum, frame->prim, frame->addr, msg_buf_s);
          port->l1link = 1;
          continue;
        ...
        case PH_DEACTIVATE | INDICATION:
          DBG("PH_DEACTIVATE ind or confirm for %s (%d) prim(0x%x) addr(0x%x) "
              "msg->len(%d) \n",
              port->name, port->portnum, frame->prim, frame->addr, msg_buf_s);
          port->l1link = 0;
          continue;
        ...
        case DL_ESTABLISH | CONFIRM:
          DBG("DL_ESTABLISH ind or confirm for %s (%d) prim(0x%x) addr(0x%x) "
              "msg->len(%d) \n",
              port->name, port->portnum, frame->prim, frame->addr, msg_buf_s);
          port->l2link = 1;
          continue;
        ...
        case DL_RELEASE | CONFIRM:
          DBG("DL_RELEASE ind or confirm for %s (%d) prim(0x%x) addr(0x%x) "
              "msg->len(%d) \n",
              port->name, port->portnum, frame->prim, frame->addr, msg_buf_s);
          port->l2link = 0;
          continue;
        ...
        default:
          DBG("GOT d-msg from %s port %d prim 0x%x dinfo 0x%x addr 0x%x\n",
              (port->ntmode) ? "NT" : "TE", port->portnum, frame->prim,
              frame->dinfo, frame->addr);
      }
      /* d-message */
      if (port->ntmode) {
        ERROR("NT mode not supported yet\n");
...
////////////////////////////////////////////////////////////////
void mISDNStack::l1l2l3_trace_header(struct mISDNport* mISDNport, int port,
                                     unsigned long prim, int direction)
{
  ...
  /* add direction */
  if (direction && (prim & 0xffffff00) != CC_NEW_CR
      && (prim & 0xffffff00) != CC_RELEASE_CR) {
    if (mISDNport) {
      if (mISDNport->ntmode) {
...
#define QI_DUMP(_x_, _y_)                                                      \
  sprintf(x, " %25s off=0x%04x ridx=0x%04x res1=0x%04x cs_flg=0x%04x", _y_,    \
          qi->_x_.off, qi->_x_.ridx, qi->_x_.res1, qi->_x_.cs_flg);            \
  ret.append(x);                                                               \
  if (qi->_x_.off > 0) {                                                       \
    sprintf(x, " 0x%02hhx 0x%02hhx 0x%02hhx 0x%02hhx\n", y[qi->_x_.off],       \
            y[qi->_x_.off + 1], y[qi->_x_.off + 2], y[qi->_x_.off + 3]);       \
    ret.append(x);                                                             \
  }                                                                            \
  else                                                                         \
    ret.append(" \n");
...
std::string mISDNStack::dumpIE(char* buf, int len)
{
  ...
  for (i = 0; i < 37; i++) {
    if (ie[i].off > 0) {
      sprintf(x, " %25s off=0x%04x ridx=0x%04x res1=0x%04x cs_flg=0x%04x",
              mISDNNames::IE_Names[i], ie[i].off, ie[i].ridx, ie[i].res1,
              ie[i].cs_flg);
      ret.append(x);
      tmp_a.assign("");
      tmp_b.assign(" ");
      for (j = 0; j < y[ie[i].off + 1]; j++) {
        sprintf(x, " 0x%02hhx", y[ie[i].off + 2 + j]);
        tmp_a.append(x);
        sprintf(x, "%c",
                (y[ie[i].off + 2 + j] > 32) ? y[ie[i].off + 2 + j] : '.');
        tmp_b.append(x);
      }
      ret.append(tmp_a);
      ret.append(tmp_b);
      ...
      if ((ie[i].repeated > 0) || (ie[i].ridx > 0)) {
        ...
        sprintf(x,
                " extinfo[%d]:               cs.codeset=0x%04x "
                "cs.locked=0x%04x cs.res1=0x%04x cs.len=0x%04x | "
                "v.codeset=0x%04x v.res1=0x%04x v.val=0x%04x\n",
                i, qi->ext[i].cs.codeset, qi->ext[i].cs.locked,
                qi->ext[i].cs.res1, qi->ext[i].cs.len, qi->ext[i].v.codeset,
                qi->ext[i].v.res1, qi->ext[i].v.val);
        ret.append(x);
      }
    }
  }
  ...
  for (i = 0; i <= (len - mISDN_HEADER_LEN) / 2; i++) {
    sprintf(x, " 0x%04x (%c %c),", arr[i], (p[2 * i] >= 32) ? p[2 * i] : '.',
            ((p[2 * i + 1]) >= 32) ? p[2 * i + 1] : '.');
    ret.append(x);
  }
  ret.append("\n");
  if (len > (mISDN_HEADER_LEN + L3_EXTRA_SIZE)) {
    ...
    for (i = 0; i <= (len - (mISDN_HEADER_LEN + L3_EXTRA_SIZE)); i++) {
      sprintf(x, " 0x%02hhx (%c),", y[i], (y[i] > 32) ? y[i] : '.');
      ret.append(x);
    }
    ret.append("\n");
  }

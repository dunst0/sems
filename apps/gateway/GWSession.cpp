...
GWSession::GWSession(const string& auth_realm, const string& auth_user,
                     const string& auth_pwd)
    : credentials(auth_realm, auth_user, auth_pwd)
{
  DBG("new GWSession@%p\n", this);
}
UACAuthCred* GWSession::getCredentials() { return &credentials; }

void GWSession::setOtherLeg(AmAudio* otherleg) { m_OtherLeg = otherleg; }
...
void GWSession::onSessionStart(const AmSipRequest& req)
{
  ...
  try {
    ...
    if (dlg.reply(req, 200, "OK Isdn side state is: CONNECTED",
                  "application/sdp", sdp_reply)
        != 0)
...
GWSession::~GWSession() { INFO("destroying GWSession!\n"); }

// we just need a hack this function for INVITE as orginal executes
// onSessionStart imediately after OnInvite
                           ...
                           AmSipDialog::Status old_dlg_status)
{
  ...
  if ((dlg.getStatus() == AmSipDialog::Pending) && (reply.code == 183)) {
    onProgress(reply);
  }
...
GWSession* GWSession::CallFromOutside(std::string& fromnumber,
                                      ...
                                      AmAudio* chan)
{
  ...
  r_uri.insert(4, tonumber);
  r_uri.append(gwconf.getParameter("calleddomain", ""));
  std::string from = "sip:@";
  from.insert(4, fromnumber);
  from.append(gwconf.getParameter("callerdomain", ""));
  std::string from_uri = "sip:@";
  from_uri.insert(4, fromnumber);
  from_uri.append(gwconf.getParameter("callerdomain", ""));
  std::string to = "sip:@";
  to.insert(4, tonumber);
  to.append(gwconf.getParameter("calleddomain", ""));
  ...
  // this is static function so we dont that 'this' pointer yet as GWcall object
  // is created in factory::onInvite. so we use pointer returned by dialout

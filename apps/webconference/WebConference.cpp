...
string
       WebConferenceFactory::participant_id_paramname; // default: param not used
string WebConferenceFactory::participant_id_hdr =
    "X-ParticipantID"; // default header

bool         WebConferenceFactory::room_pin_split     = false;
unsigned int WebConferenceFactory::room_pin_split_pos = 0;

int WebConferenceFactory::onLoad() { return getInstance()->load(); }
...
int WebConferenceFactory::load()
{
  ...
  // get prompts
  AM_PROMPT_START;
  AM_PROMPT_ADD(FIRST_PARTICIPANT,
                WEBCONF_ANNOUNCE_PATH "first_paricipant.wav");
  ...
  AM_PROMPT_ADD(ENTERING_CONFERENCE,
                WEBCONF_ANNOUNCE_PATH "entering_conference.wav");
  ...
  if (!feedback_filename.empty()) {
    feedback_file.open(feedback_filename.c_str(),
  ...
  if (cfg.getParameter("participants_expire") == "no") {
    ...
    // default: 10s
    ParticipantExpiredDelay =
        cfg.getParameterInt("participants_expire_delay", 10);
  }
  ...
  if (cfg.getParameter("rooms_expire") == "no") {
    ...
    RoomExpiredDelay =
        cfg.getParameterInt("rooms_expire_delay", 7200); // default: 2h
  }
  ...
  for (vector<string>::iterator it = predefined_rooms.begin();
       it != predefined_rooms.end(); it++) {
  ...
  if (cfg.getParameter("private_rooms") == "yes") PrivateRoomsMode = true;
...
{
  if (!PrivateRoomsMode) return true;

  bool res = false;
  rooms_mut.lock();
  map<string, ConferenceRoom>::iterator room = rooms.find(conf_id);
  if (room != rooms.end()) {
    if (!participant_id.size()
        || room->second.hasInvitedParticipant(participant_id)) {
                                          ...
                                          bool          check_exisiting)
{
  ...
  if (PrivateRoomsMode) {
    ...
    if (check_exisiting && room_pin_split
        && !room->second.hasInvitedParticipant(participant_id)) {
      DBG("participant with ID '%s' not listed in invited participants for "
          "'%s'\n",
...
void WebConferenceFactory::updateStatus(
    const string& conf_id, const string& localtag,
    ConferenceRoomParticipant::ParticipantStatus status, const string& reason)
{
  rooms_mut.lock();
                                              ...
                                              bool ignore_adminpin = false)
{
  ...
  if (it == rooms.end()) {
    ...
    if ((!ignore_pin) && (!ignore_adminpin) && (!it->second.adminpin.empty())
...
string WebConferenceFactory::getAccessUri(const string& room)
{
  ...
  if (!WebConferenceFactory::urlbase.empty()) {
    ...
    if (!room.empty()) {
      ...
      if (!adminpin.empty()) res += "&roomAdminPassword=" + adminpin;
    }
  }
  return res;
}
...
{
  if (NULL != session_timer_f) {
    if (!session_timer_f->onInvite(req, cfg)) return NULL;
  }
  WebConferenceDialog* w;

  map<string, string>::const_iterator r_it = app_params.find("room");
  map<string, string>::const_iterator enter_room_it =
      app_params.find("enter_room");
  if (enter_room_it != app_params.end() && enter_room_it->second == "true") {
    // enter the room
    DBG("creating new Webconference call, room name to be entered via "
        "keypad\n");
  ...
  else if (use_direct_room
           && !regexec(&direct_room_re, req.user.c_str(), 0, 0, 0)) {
    // regegex match
...
void WebConferenceFactory::invoke(const string& method, const AmArg& args,
                                  AmArg& ret)
{
...
{
  string res = "Server: " DEFAULT_SIGNATURE " calls: "
               + int2str(AmSession::getSessionNum()) + " active";
...
{
  string res;
  for (int i = 0; i < 6; i++) res += (char) ('0' + random() % 10);
  return res;
}
...
void WebConferenceFactory::roomDelete(const string& room,
                                      const string& adminpin, AmArg& ret,
                                      bool ignore_adminpin = false)
{
  rooms_mut.lock();
  ...
  postAllConfEvent(room, adminpin, ret, WebConferenceEvent::Kick,
                   ignore_adminpin);
...
void WebConferenceFactory::closeExpiredRooms()
{
  ...
  rooms_mut.lock();
  for (map<string, ConferenceRoom>::iterator it = rooms.begin();
       it != rooms.end(); it++) {
    if (it->second.hard_expired(now)) expired_rooms.push_back(it->first);
  }
  rooms_mut.unlock();

  for (vector<string>::iterator it = expired_rooms.begin();
       it != expired_rooms.end(); it++) {
...
void WebConferenceFactory::dialout(const AmArg& args, AmArg& ret)
{
  ...
  DBG("dialout webconference room '%s', from '%s', to '%s'", room.c_str(),
      from.c_str(), to.c_str());
  ...
  string localtag = AmUAC::dialout(room.c_str(), app_name, to, "<" + from + ">",
                                   from, "<" + to + ">",
                                   string(""), // callid
                                   headers,    // headers
                                   a);
  if (!localtag.empty()) {
    ...
    updateStatus(room, localtag, ConferenceRoomParticipant::Connecting,
                 "INVITE");
  }
  else {
    ret.push(1);
...
void WebConferenceFactory::postAllConfEvent(const string& room,
                                            const string& adminpin, AmArg& ret,
                                            int  id,
                                            bool ignore_adminpin = false)
{
  ...
  for (vector<string>::iterator it = ltags.begin(); it != ltags.end(); it++) {
    AmSessionContainer::instance()->postEvent(*it, new WebConferenceEvent(id));
  }
...
void WebConferenceFactory::postConfEvent(const AmArg& args, AmArg& ret, int id,
                                         int mute)
{
  string room     = args.get(0).asCStr();
  string adminpin = args.get(1).asCStr();
  ...
  if (p_exists && (mute >= 0)) r->setMuted(call_tag, mute);
  rooms_mut.unlock();
...
void WebConferenceFactory::getRoomPassword(const AmArg& args, AmArg& ret)
{
  ...
  if ((!MasterPassword.length()) || pwd != MasterPassword) {
    ret.push(403);
...
void WebConferenceFactory::findParticipant(const AmArg& args, AmArg& ret)
{
  ...
  for (map<string, ConferenceRoom>::iterator it = rooms.begin();
    ...
    for (list<ConferenceRoomParticipant>::iterator p_it =
             it->second.participants.begin();
         p_it != it->second.participants.end(); p_it++) {
...
void WebConferenceFactory::vqRoomFeedback(const AmArg& args, AmArg& ret)
{
  ...
  saveFeedback(string("RO " + room + "|||" + adminpin + "|||" + int2str(opinion)
                      + "|||" + int2str((unsigned int) time(NULL)) + "|||\n"));
...
void WebConferenceCleaner::on_stop() { is_stopped.set(true); }

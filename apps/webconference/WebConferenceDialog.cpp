...
WebConferenceDialog::~WebConferenceDialog()
{
  ...
  if (is_dialout || (InConference == state)) {
    factory->updateStatus(is_dialout ? dlg->getUser() : conf_id, getLocalTag(),
...
void WebConferenceDialog::connectConference(const string& room)
{
  ...
  if (channel.get() == NULL)
    channel.reset(AmConferenceStatus::getChannel(conf_id, getLocalTag(),
                                                 RTPStream()->getSampleRate()));
  else
    AmConferenceStatus::postConferenceEvent(conf_id, ConfNewParticipant,
                                            getLocalTag());
...
void WebConferenceDialog::onInvite(const AmSipRequest& req)
{
  if (state == None) {
    if (WebConferenceFactory::participant_id_paramname.length()) {
      ...
      if (appparams.length()) {
        participant_id = get_header_param(
            appparams, WebConferenceFactory::participant_id_paramname);
      }
    }
    else if (WebConferenceFactory::participant_id_hdr.length()) {
      participant_id =
          getHeader(req.hdrs, WebConferenceFactory::participant_id_hdr);
    }
      ...
      if (conf_id.empty()) {
        ...
        if (WebConferenceFactory::room_pin_split) {
          if (pin_str.length() <= WebConferenceFactory::room_pin_split_pos) {
            DBG("short conference room/pin combination ('%s', want at least "
                "%d)\n",
          ...
          participant_id =
              pin_str.substr(WebConferenceFactory::room_pin_split_pos);
        ...
        if (!factory->newParticipant(conf_id, getLocalTag(),
                                     dlg->getRemoteParty(), participant_id)) {
          ...
          factory->updateStatus(conf_id, getLocalTag(),
                                     ...
                                     AmBasicSipDialog::Status old_dlg_status)
{
  ...
  DBG("reply: %u %s, old_dlg_status = %s, status = %s\n", reply.code,
      reply.reason.c_str(), AmBasicSipDialog::getStatusStr(old_dlg_status),
      dlg->getStatusStr());
  ...
  if (is_dialout) {
    ...
    switch (dlg->getStatus()) {
      ...
      case AmSipDialog::Connected:
        rep_st = ConferenceRoomParticipant::Connected;
        break;
      case AmSipDialog::Cancelling:
      case AmSipDialog::Disconnecting:
        rep_st = ConferenceRoomParticipant::Disconnecting;
        break;
      case AmSipDialog::Disconnected:
        rep_st = ConferenceRoomParticipant::Finished;
        break;
      default: break;
    }
    DBG("is dialout: updateing status\n");
    factory->updateStatus(dlg->getUser(), getLocalTag(), rep_st,
                          int2str(reply.code) + " " + reply.reason);
  }
}
...
void WebConferenceDialog::onBye(const AmSipRequest& req)
{
  if (InConference == state) {
    factory->updateStatus(conf_id, getLocalTag(),
                          ConferenceRoomParticipant::Disconnecting, req.method);
  }
...
void WebConferenceDialog::process(AmEvent* ev)
{
    ...
    switch (ce->event_id) {
      case ConfNewParticipant: {
        ...
        if (ce->participants == 1) {
          prompts.addToPlaylist(
              FIRST_PARTICIPANT, (long) this, play_list, true,
          ...
          if (WebConferenceFactory::LoopFirstParticipantPrompt) {
            ...
            // get a channel from the status
            if (channel.get() == NULL)
              channel.reset(AmConferenceStatus::getChannel(
                  conf_id, getLocalTag(), RTPStream()->getSampleRate()));

            // clear the playlist
            play_list.flush();

            // add the channel to our playlist
            play_list.addToPlaylist(
                new AmPlaylistItem(channel.get(), channel.get()));
          }
  ...
  AmPlaylistSeparatorEvent* sep_ev =
      dynamic_cast<AmPlaylistSeparatorEvent*>(ev);
  if (NULL != sep_ev) {
    ...
    if (EnteringConference == state) {
      ...
      if (!factory->newParticipant(pin_str, getLocalTag(),
                                   dlg->getRemoteParty(), participant_id)) {
      ...
      factory->updateStatus(pin_str, getLocalTag(),
                            ConferenceRoomParticipant::Connected, "entered");
    }
  }
  ...
  if (NULL != webconf_ev) {
    switch (webconf_ev->event_id) {
      ...
      default: {
        WARN("ignoring unknown webconference event %d\n", webconf_ev->event_id);
      } break;
    }
  }
...
void WebConferenceDialog::onDtmf(int event, int duration)
{
  ...
  if (EnteringPin == state) {
    ...
    if (event < 10) {
      ...
      if (!factory->isValidConference(
              pin_str,
              WebConferenceFactory::room_pin_split ? participant_id : "")) {
        setInOut(&play_list, &play_list);
        play_list.flush();
        prompts.addToPlaylist(WRONG_PIN, (long) this, play_list);
        pin_str = "";
...
void WebConferenceDialog::onKicked()
{
  ...
  disconnectConference();
  factory->updateStatus(conf_id, getLocalTag(),
                        ConferenceRoomParticipant::Disconnecting, "disconnect");
}
...
int WebConferenceDialog::readStreams(unsigned long long ts,

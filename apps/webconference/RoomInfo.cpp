...
{
  if (Finished != status) return false;

  if (WebConferenceFactory::ParticipantExpiredDelay < 0) return false;

  struct timeval diff;
  timersub(&now, &last_access_time, &diff);
  return (diff.tv_sec > 0)
         && (unsigned int) diff.tv_sec
                > (unsigned int) WebConferenceFactory::ParticipantExpiredDelay;
}
...
void ConferenceRoomParticipant::setMuted(int mute) { muted = mute; }

void ConferenceRoomParticipant::updateStatus(
    ConferenceRoomParticipant::ParticipantStatus new_status,
    const string& reason, struct timeval& now)
{
  status      = new_status;
  last_reason = reason;
  updateAccess(now);
...
void ConferenceRoom::cleanExpired()
{
  ...
  if (is_updated) memcpy(&last_access_time, &now, sizeof(struct timeval));
}
...
bool ConferenceRoom::hasParticipant(const string& localtag)
{
  for (list<ConferenceRoomParticipant>::iterator it = participants.begin();
       it != participants.end(); it++) {
    if (it->localtag == localtag) return true;
  }
...
bool ConferenceRoom::hasInvitedParticipant(const string& participant_id)
{
  for (list<ConferenceRoomParticipant>::iterator it = participants.begin();
       it != participants.end(); it++) {
    if (it->participant_id == participant_id) return true;
  }
  return false;
}
...
bool ConferenceRoom::updateStatus(
    const string&                                part_tag,
...
bool ConferenceRoom::expired(const struct timeval& now)
{
  ...
  if (WebConferenceFactory::RoomExpiredDelay < 0) return false;

  struct timeval diff;
  timersub(&now, &last_access_time, &diff);
  return (diff.tv_sec > 0)
         && (unsigned int) diff.tv_sec
                > (unsigned int) WebConferenceFactory::RoomExpiredDelay;
}

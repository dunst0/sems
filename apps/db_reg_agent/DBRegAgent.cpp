...
int DBRegAgent::onLoad()
{
  ...
  if (cfg.hasParameter("minimum_reregister_interval")) {
    ...
    minimum_reregister_interval =
        atof(cfg.getParameter("minimum_reregister_interval").c_str());
    if (minimum_reregister_interval <= 0 || minimum_reregister_interval > 1) {
      ERROR("configuration value 'minimum_reregister_interval' could not be "
            "read. "
  ...
  if (enable_ratelimiting) {
    if (!cfg.hasParameter("ratelimit_rate")
        || !cfg.hasParameter("ratelimit_per")) {
      ERROR("if ratelimiting is enabled, ratelimit_rate and ratelimit_per must "
            "be set\n");
      return -1;
    }
  ...
  save_contacts = cfg.getParameter("save_contacts", "yes") == "yes";

  db_read_contact = cfg.getParameter("db_read_contact", "no") == "yes";

  save_auth_replies = cfg.getParameter("save_auth_replies", "no") == "yes";
  ...
  try {
    ...
    // matched instead of changed rows in result, so we know when to create DB
    // entry
...
void DBRegAgent::onUnload()
{
  if (running) {
    ...
    for (int i = 0; i < 400; i++) {
      if (shutdown_finished && registration_scheduler._shutdown_finished) break;
      usleep(2000); // 2ms
    }

    if (!shutdown_finished || !registration_scheduler._shutdown_finished) {
      WARN("processing thread could not be stopped, process will probably "
           "crash\n");
    }
  }
  ...
  try {
    ...
    while (mysqlpp::Row row = res.fetch_row()) {
      ...
      if (row[COLNAME_STATUS] != mysqlpp::null)
        ...
        DBG("registration status entry for id %ld does not exist, "
            "creating...\n",
            subscriber_id);
      ...
      switch (status) {
        ...
        {
          createRegistration(subscriber_id, (string) row[COLNAME_USER],
                             (string) row[COLNAME_PASS],
                             (string) row[COLNAME_REALM], contact_uri);
        ...
        case REG_STATUS_ACTIVE: {
          ...
          if (row[COLNAME_REGISTRATION_TS] != mysqlpp::null) {
            dt_registration_ts =
                (time_t)((mysqlpp::DateTime) row[COLNAME_REGISTRATION_TS]);
          }
          ...
          if (dt_registration_ts > now_time) {
            WARN("needed to sanitize last_registration timestamp TS from the "
                 "%ld (now %ld) - "
          ...
          // if expired add to pending registrations, else schedule
          // re-regstration
        ...
        case REG_STATUS_REMOVED: {
          DBG("ignoring removed registration %ld %s@%s", subscriber_id,
              ((string) row[COLNAME_USER]).c_str(),
              ((string) row[COLNAME_REALM]).c_str());
        } break;
...
void DBRegAgent::createRegistration(long subscriber_id, const string& user,
                                    ...
                                    const string& contact)
{
  ...
  try {
    ...
    AmSIPRegistration* reg =
        new AmSIPRegistration(handle, reg_info, "" /*MOD_NAME*/);
...
void DBRegAgent::updateRegistration(long subscriber_id, const string& user,
                                    const string& pass, const string& realm,
                                    const string& contact)
{
  registrations_mut.lock();
  ...
  if (need_reregister) {
    DBG("user/realm for registration %ld changed (%s@%s -> %s@%s). "
        ...
        subscriber_id, old_user.c_str(), old_realm.c_str(), user.c_str(),
        realm.c_str());
    scheduleRegistration(subscriber_id);
  }
}
...
{
  if (enable_ratelimiting) {
    registration_processor.postEvent(new RegistrationActionEvent(
...
void DBRegAgent::scheduleDeregistration(long subscriber_id)
{
  if (enable_ratelimiting) {
    registration_processor.postEvent(new RegistrationActionEvent(
        RegistrationActionEvent::Deregister, subscriber_id));
  }
  else {
    // use our own thread
...
void DBRegAgent::onRegistrationActionEvent(
    RegistrationActionEvent* reg_action_ev)
{
      ...
      if (it == registrations.end()) {
        ...
        if (!it->second->doRegistration()) {
          updateDBRegistration(
              ProcessorDBConnection, reg_action_ev->subscriber_id, 480,
              ERR_REASON_UNABLE_TO_SEND_REQUEST, true, REG_STATUS_FAILED);
          if (error_retry_interval) {
            // schedule register-refresh after error_retry_interval
            setRegistrationTimer(reg_action_ev->subscriber_id,
        ...
        if (!it->second->doUnregister()) {
          if (delete_removed_registrations && delete_failed_deregistrations) {
            ...
            DBG("failed sending de-register, updating DB with "
                "REG_STATUS_TO_BE_REMOVED " ERR_REASON_UNABLE_TO_SEND_REQUEST
                "for subscriber %ld\n",
                reg_action_ev->subscriber_id);
            updateDBRegistration(ProcessorDBConnection,
                                 reg_action_ev->subscriber_id, 480,
                                 ERR_REASON_UNABLE_TO_SEND_REQUEST, true,
...
void DBRegAgent::createDBRegistration(long                 subscriber_id,
                                      mysqlpp::Connection& conn)
{
  string insert_query = "insert into " + registrations_table
                        + " (subscriber_id) values (" + long2str(subscriber_id)
                        + ");";
...
void DBRegAgent::deleteDBRegistration(long                 subscriber_id,
                                      mysqlpp::Connection& conn)
{
...
void DBRegAgent::updateDBRegistration(
    mysqlpp::Connection& db_connection, long subscriber_id, int last_code,
    const string& last_reason, bool update_status, int status, bool update_ts,
    unsigned int expiry, bool update_contacts, const string& contacts)
{
  try {
    mysqlpp::Query query = db_connection.query();

    query << "update " + registrations_table
                 + " set last_code=" + int2str(last_code) + ", ";
    query << "last_reason=";
      ...
      if (!delete_status) {
        if (auth_pending && !save_auth_replies) {
          ...
          DBG("update DB with reply %u %s\n", ev->reply.code,
              ev->reply.reason.c_str());
          updateDBRegistration(MainDBConnection, subscriber_id, ev->reply.code,
                               ev->reply.reason, update_status, status,
                               update_ts, expiry, save_contacts,
                               ev->reply.contact);
        }
      }
      else {
...
void DBRegAgent::setRegistrationTimer(
    long subscriber_id, unsigned int timeout,
    RegistrationActionEvent::RegAction reg_action)
{
  DBG("setting Register timer for subscription %ld, timeout %u, reg_action "
      "%u\n",
...
void DBRegAgent::setRegistrationTimer(long subscriber_id, time_t expiry,
                                      time_t reg_start_ts, time_t now_time)
{
  ...
  if (minimum_reregister_interval > 0.0) {
    ...
    if (t_expiry_min > t_expiry_max) t_expiry_min = t_expiry_max;
    ...
    if (t_expiry_max == now_time) {
      ...
      DBG("calculated re-registration at TS <now> (%ld)"
          ...
          t_expiry_max, reg_start_ts, expiry, reregister_interval,
          minimum_reregister_interval);
      registration_scheduler.insert_timer(timer);
    }
    else {
                                      ...
                                      const string& contact, AmArg& ret)
{
  DBG("DI method: createRegistration(%i, %s, %s, %s, %s)\n", subscriber_id,
      user.c_str(), pass.c_str(), realm.c_str(), contact.c_str());
...
void DBRegAgent::invoke(const string& method, const AmArg& args, AmArg& ret)
{
  if (method == "createRegistration") {
    ...
    DIcreateRegistration(args.get(0).asInt(), args.get(1).asCStr(),
                         args.get(2).asCStr(), args.get(3).asCStr(), contact,
                         ret);
  }
...
void DBRegAgentProcessorThread::rateLimitWait()
{
  ...
  double seconds_passed =
      (double) time_passed.tv_sec + (double) time_passed.tv_usec / 1000000.0;
  allowance += seconds_passed * (double) DBRegAgent::ratelimit_rate
               / (double) DBRegAgent::ratelimit_per;

  if (allowance > (double) DBRegAgent::ratelimit_rate)
    allowance = (double)
        DBRegAgent::ratelimit_rate; // enough time passed, but limit to max
  if (allowance < 1.0) {
    useconds_t sleep_time = 1000000.0 * (1.0 - allowance)
                            * ((double) DBRegAgent::ratelimit_per
                               / (double) DBRegAgent::ratelimit_rate);

...
{
  if (dia_conn) {
    if (tls_shutdown) tcp_tls_shutdown(dia_conn);
...
int ServerConnection::init(const string& _server_name, int _server_port,
                           const string& _ca_file, const string& _cert_file,
                           const string& _origin_host,
                           const string& _origin_realm,
                           const string& _origin_ip, AAAApplicationId _app_id,
                           unsigned int _vendorID, const string& _product_name,
...
void ServerConnection::openConnection()
{
  ...
  if (addOrigin(cer)
      || addDataAVP(cer, AVP_Host_IP_Address, origin_ip_address,
                    sizeof(origin_ip_address))
      || addDataAVP(cer, AVP_Vendor_Id, (char*) &vendorID, sizeof(vendorID))
      || addDataAVP(cer, AVP_Supported_Vendor_Id, (char*) &vendorID,
                    sizeof(vendorID))
  ...
  AAA_AVP* vs_appid;
  if ((vs_appid = AAACreateAVP(AVP_Vendor_Specific_Application_Id,
                               (AAA_AVPFlag) AAA_AVP_FLAG_NONE, 0, 0, 0,
                               AVP_DONT_FREE_DATA))
      == 0) {
  ...
  // feels like c coding...
  if (addGroupedAVP(vs_appid, AVP_Auth_Application_Id, (char*) &app_id,
                    sizeof(app_id))
      || addGroupedAVP(vs_appid, AVP_Vendor_Id, (char*) &vendorID,
                       sizeof(vendorID))
  ...
  while (true) {
    int res =
        tcp_recv_msg(conn.dia_conn, &conn.rb, CONNECT_CEA_REPLY_TIMEOUT, 0);
...
AAAMessage* ServerConnection::ReqEvent2AAAMessage(DiameterRequestEvent* re)
{
  ...
  for (int i = re->val.size() - 1; i >= 0; i--) {
    ...
    AAA_AVP* avp;
    if ((avp = AAACreateAVP(avp_id, (AAA_AVPFlag) flags, vendor,
                            (const char*) data->data, data->len,
                            AVP_DUPLICATE_DATA))
        == 0) {
...
int ServerConnection::addStringAVP(AAAMessage* msg, AAA_AVPCode avp_code,
                                   string& val, bool attail)
{
  AAA_AVP* avp;
  if ((avp = AAACreateAVP(avp_code, (AAA_AVPFlag) AAA_AVP_FLAG_NONE, 0,
                          val.c_str(), val.length(), AVP_DUPLICATE_DATA))
      == 0) {
...
int ServerConnection::addDataAVP(AAAMessage* msg, AAA_AVPCode avp_code,
                                 char* val, unsigned int len)
{
  AAA_AVP* avp;
  if ((avp = AAACreateAVP(avp_code, (AAA_AVPFlag) AAA_AVP_FLAG_NONE, 0, val,
                          len, AVP_DUPLICATE_DATA))
      == 0) {
    ERROR(M_NAME ": addDataAVP() no more free memory!\n");
    return -1;
  }
...
int ServerConnection::handleRequest(AAAMessage* req)
{
  switch (req->commandCode) {
    case AAA_CC_DWR: { // Device-Watchdog-Request
      ...
      AAAMessage* reply;
      if ((reply = AAAInMessage(AAA_CC_DWA, AAA_APP_DIAMETER_COMMON_MSG))
          == NULL) {
    ...
    case AAA_CC_DPR: { // Disconnect-Peer-Request
      ...
      AAAMessage* reply;
      if ((reply = AAAInMessage(AAA_CC_DPA, AAA_APP_DIAMETER_COMMON_MSG))
          == NULL) {
        ERROR(M_NAME ":handleRequest(): can't create new "
                     "DPA message!\n");
        return AAA_ERROR_MESSAGE;
...
int ServerConnection::handleReply(AAAMessage* rep)
{
  ...
  if (!sess_link.empty()) {
    DiameterReplyEvent* r_ev = new DiameterReplyEvent(
        (unsigned int) rep->commandCode, (unsigned int) rep->applicationId,
...
void ServerConnection::on_stop() { DBG("todo: stop connection.\n"); }

void ServerConnection::receive()
{
  int res = tcp_recv_msg(conn.dia_conn, &conn.rb, 0, CONN_WAIT_USECS);
...
{
  if ((++timeout_check_cntr) % CHECK_TIMEOUT_INTERVAL) return;
  ...
  for (DReqMap::iterator it = req_map.begin(); it != req_map.end();) {
    struct timeval diff;
    ...
    if (diff.tv_sec * 1000 + diff.tv_usec / 1000 > request_timeout) {
      ...
      DBG("notify session '%s' of diameter request timeout\n",
          sess_link.c_str());
      DiameterTimeoutEvent* r_ev = new DiameterTimeoutEvent(it->first);
      if (!AmSessionContainer::instance()->postEvent(sess_link, r_ev)) {
...
void ServerConnection::shutdownConnection()
{
  ...
  DBG("shutdown: posting timeout to %zd pending requests....\n",
      req_map.size());
  for (DReqMap::iterator it = req_map.begin(); it != req_map.end(); it++) {
    string&               sess_link = it->second.first;
    DiameterTimeoutEvent* r_ev      = new DiameterTimeoutEvent(it->first);
    if (!AmSessionContainer::instance()->postEvent(sess_link, r_ev)) {
      DBG("unhandled timout event.\n");
...
{
  if (open) conn.terminate(tls_shutdown);
  open = false;
}

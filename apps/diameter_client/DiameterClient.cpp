...
void DiameterClient::newConnection(const AmArg& args, AmArg& ret)
{
  ...
  sc->init(server_ip, server_port, ca_file, cert_file, origin_host,
           origin_realm, origin_ip, app_id, vendor_id, product_name,
           req_timeout);
...
void DiameterClient::sendRequest(const AmArg& args, AmArg& ret)
{
  string app_name     = args.get(0).asCStr();
  ...
  for (multimap<string, ServerConnection*>::iterator it =
    ...
    if (it->second->is_open()) scs.push_back(it->second);
  }
  conn_mut.unlock();
  ...
  scs[pos]->postEvent(
      new DiameterRequestEvent(command_code, app_id, val, sess_link));
  ret.push(0);
  ret.push("request sent");
  return;
}

void DiameterClient::invoke(const string& method, const AmArg& args, AmArg& ret)
{

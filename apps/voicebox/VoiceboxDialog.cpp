...
VoiceboxDialog::VoiceboxDialog(const string& user, const string& domain,
                               const string& pin, AmPromptCollection* prompts,
...
void VoiceboxDialog::openMailbox()
{
  ...
  if (MSG_OK != ecode) {
    ERROR("userdir_open for user '%s' domain '%s': %s\n", user.c_str(),
          domain.c_str(), MsgStrError(ret.get(0).asInt()));
    closeMailbox();
    return;
  }

  if ((ret.size() < 2) || (!isArgArray(ret.get(1)))) {
    ERROR("userdir_open for user '%s' domain '%s'"
  ...
  for (size_t i = 0; i < ret.get(1).size(); i++) {
    ...
    if (!isArgArray(elem) || elem.size() != 3) {
  ...
  DBG("Got %zd new and %zd saved messages for user '%s' domain '%s'\n",
      new_msgs.size(), saved_msgs.size(), user.c_str(), domain.c_str());
...
{
  if (!userdir_open) return;

  AmArg di_args, ret;
  di_args.push(domain.c_str()); // domain
  di_args.push(user.c_str());   // user
  msg_storage->invoke("userdir_close", di_args, ret);
  if (ret.size() && isArgInt(ret.get(0)) && ret.get(0).asInt() != MSG_OK) {
...
FILE* VoiceboxDialog::getCurrentMessage()
{
  ...
  int ecode = ret.get(0).asInt();
  if (MSG_OK != ecode) {
    ERROR("msg_get for user '%s' domain '%s' message '%s': %s", user.c_str(),
          domain.c_str(), msgname.c_str(), MsgStrError(ret.get(0).asInt()));
    return NULL;
  }

  if ((ret.size() < 2) || (!isArgAObject(ret.get(1)))) {
    ERROR("msg_get for user '%s' domain '%s' message '%s': invalid return "
          "value\n",
          user.c_str(), domain.c_str(), msgname.c_str());
    return NULL;
  }
...
void VoiceboxDialog::doListOverview()
{
  ...
  if (!new_msgs.empty()) {
    if (prompt_options.has_digits && (new_msgs.size() == 1)) {
      // one new message
      enqueueBack("new_msg");
    }
    else {
      // five
      if (prompt_options.has_digits) enqueueCount(new_msgs.size());
      // new messages
      enqueueBack("new_msgs");
    }
    if (!saved_msgs.empty()) enqueueBack("and");
  }

  if (!saved_msgs.empty()) {
    if (prompt_options.has_digits && (saved_msgs.size() == 1)) {
      // one saved message
      enqueueBack("saved_msg");
    }
    else {
      // fifteen
      if (prompt_options.has_digits) enqueueCount(saved_msgs.size());
      // saved messages
...
{
  if (!in_saved_msgs) {
    if (cur_msg != new_msgs.end()) cur_msg++;
    ...
    if (cur_msg != saved_msgs.end()) cur_msg++;
  }
}
...
void VoiceboxDialog::curMsgOP(const char* op)
{
  if (!isAtEnd()) {
    ...
    if ((ret.size() < 1) || !isArgInt(ret.get(0))) {
...
{
  if (do_save_cur_msg) curMsgOP("msg_markread");
  do_save_cur_msg = false;
}

void VoiceboxDialog::deleteCurMessage() { curMsgOP("msg_delete"); }

bool VoiceboxDialog::isAtLastMsg()
{
  if (in_saved_msgs) {
    if (saved_msgs.empty()) return true;
    return cur_msg->name == saved_msgs.back().name;
  }
  else {
    if (!saved_msgs.empty() || (new_msgs.empty())) return false;
...
{
  bool res = (in_saved_msgs && (cur_msg == saved_msgs.end()))
             || (!in_saved_msgs && (cur_msg == new_msgs.end()));
  return res;
}

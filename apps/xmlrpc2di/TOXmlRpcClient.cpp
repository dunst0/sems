                             ...
                             XmlRpcValue& result, double timeout)
{
  ...
  // This is not a thread-safe operation, if you want to do multithreading, use
  // separate clients for each thread. If you want to protect yourself from
  // multiple threads accessing the same client, replace this code with a real
  // mutex.
  ...
  if (_connectionState != IDLE || !parseResponse(result)) return false;

...
void MultithreadXmlRpcServer::acceptConnection()
{
  ...
  if (s < 0) {
    if (errno != EAGAIN && errno != EWOULDBLOCK) {
      ...
      if ((errno == EMFILE) || (errno == ENFILE)) {
        // would cause fast loop because we don't get the connection from the
        // listening socket
        usleep(500000);
      }
    }
       ...
       // in work()
  {
    ...
    while (!thr) {
      if (!have_waiting.get()) have_waiting.wait_for();
      thr = getIdleThread();
    }
    ...
    //       // Just close the connection so that the rejected client would try
    //       again. XmlRpcSocket::close(s);
    //       XmlRpcUtil::error("MultithreadXmlRpcServer::acceptConnection: All
    //       threads are busy. Rejected a client");
  }
}

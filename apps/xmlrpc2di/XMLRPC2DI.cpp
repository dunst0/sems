...
int XMLRPC2DI::load()
{
  ...
  server =
      new XMLRPC2DIServer(XMLRPCPort, bind_ip, export_di, direct_export, s);
...
XMLRPCServerEntry* XMLRPC2DI::getServer(const string& app_name)
{
  ...
  for (multimap<string, XMLRPCServerEntry*>::iterator it =
    ...
    if (it->second->is_active()) scs.push_back(it->second);
  }
  server_mut.unlock();
...
void XMLRPC2DI::invoke(const string& method, const AmArg& args, AmArg& ret)
{
...
XMLRPC2DIServer::XMLRPC2DIServer(unsigned int port, const string& bind_ip,
                                 bool di_export, string direct_export,
                                 XmlRpcServer* s)
    : AmEventQueue(this)
    , port(port)
    , bind_ip(bind_ip)
  ...
  try {
    ...
    for (unsigned int i = 0; i < fct_list.size(); i++) {
      ...
      INFO("XMLRPC Server: enabling method '%s.%s'\n", iface.c_str(),
           method.c_str());
      DIMethodProxy* mp = new DIMethodProxy(iface + "." + method, method, di_f);
      s->addMethod(mp);
    }
  }
...
void XMLRPC2DIServerGetLoglevelMethod::execute(XmlRpcValue& params,
                                               XmlRpcValue& result)
{
  int res = log_level;
...
void XMLRPC2DIServerSetLoglevelMethod::execute(XmlRpcValue& params,
                                               XmlRpcValue& result)
{
  log_level = params[0];
...
void XMLRPC2DIServerGetShutdownmodeMethod::execute(XmlRpcValue& params,
                                                   XmlRpcValue& result)
{
...
void XMLRPC2DIServerSetShutdownmodeMethod::execute(XmlRpcValue& params,
                                                   XmlRpcValue& result)
{
...
void XMLRPC2DIServerGetCPSLimitMethod::execute(XmlRpcValue& params,
                                               XmlRpcValue& result)
{
  pair<unsigned int, unsigned int> l =
      AmSessionContainer::instance()->getCPSLimit();
  DBG("XMLRPC2DI: get_cpslimit returns %d and %d\n", l.first, l.second);
  result = int2str(l.first) + " " + int2str(l.second);
}

void XMLRPC2DIServerSetCPSLimitMethod::execute(XmlRpcValue& params,
                                               XmlRpcValue& result)
{
...
void XMLRPC2DIServerGetCpsavgMethod::execute(XmlRpcValue& params,
                                             XmlRpcValue& result)
{
...
void XMLRPC2DIServerGetCpsmaxMethod::execute(XmlRpcValue& params,
                                             XmlRpcValue& result)
{
...
XMLMETH_EXEC(XMLRPC2DIServerGetSessionCount, getSessionCount,
             "get_sessioncount");
XMLMETH_EXEC(XMLRPC2DIServerGetCallsavgMethod, getAvgSessionNum,
             "get_callsavg");
XMLMETH_EXEC(XMLRPC2DIServerGetCallsmaxMethod, getMaxSessionNum,
             "get_callsmax");
#undef XMLMETH_EXEC
...
void XMLRPC2DIServerDIMethod::execute(XmlRpcValue& params, XmlRpcValue& result)
{
  try {
    ...
    throw XmlRpcException("Exception: AmDynInvoke::NotImplemented: " + e.what,
                          504);
  }
  catch (const AmArg::OutOfBoundsException& e) {
    throw XmlRpcException(
        "Exception: AmArg out of bounds - parameter number mismatch.", 300);
  }
  ...
  if (v.valid()) {
    switch (v.getType()) {
      ...
      case XmlRpcValue::TypeStruct: {
        ...
        for (XmlRpc::XmlRpcValue::ValueStruct::const_iterator it = xvs.begin();
             it != xvs.end(); ++it) {
      ...
      case XmlRpcValue::TypeBase64: {
        ...
        int i                             = 0;
        for (XmlRpcValue::BinaryData::const_iterator it = bd.begin();
             it != bd.end(); ++it) {
          ((char*) ab.data)[i] = *it;
          ++i;
        }
...
void XMLRPC2DIServer::amarg2xmlrpcval(const AmArg& a, XmlRpcValue& result)
{
  switch (a.getType()) {
    ...
    case AmArg::Bool: result = a.asBool(); break;

    case AmArg::CStr:
      //    DBG("a->X CSTR\n");
      result = string(a.asCStr());
      break;
    ...
    case AmArg::Struct:
      ...
      for (AmArg::ValueStruct::const_iterator it = a.begin(); it != a.end();
           it++) {
        // duh... recursion...
    ...
    default: {
      WARN("unsupported return value type %d\n", a.getType());
    } break;

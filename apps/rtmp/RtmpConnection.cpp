...
int RtmpConnection::processPacket(RTMPPacket* packet)
{
  ...
  switch (packet->m_packetType) {
    ...
    default: DBG("unknown packet type (0x%02x)", packet->m_packetType); break;
  }
...
int RtmpConnection::invoke(RTMPPacket* packet, unsigned int offset)
{
  const char*  body;
  ...
  if (AVMATCH(&method, &av_connect)) {
    ...
    if (rtmp.Link.tcUrl.av_len) {
      DBG("tcUrl = <%.*s>\n", rtmp.Link.tcUrl.av_len, rtmp.Link.tcUrl.av_val);
    ...
    unsigned int stream_id =
        (unsigned int) AMFProp_GetNumber(AMF_GetProp(&obj, NULL, 3));
    ...
    if (registered) {
      if (di_reg_client && !rtmp_cfg->ImplicitRegistrar.empty()) {
        createRegistration(rtmp_cfg->ImplicitRegistrar, ident,
                           rtmp_cfg->FromName);
      }
    }
  }
...
void RtmpConnection::HandleCtrl(const RTMPPacket* packet)
{
  ...
  DBG("received ctrl. type: %d, len: %d", nType, packet->m_nBodySize);
  ...
  if (packet->m_nBodySize >= 6) {
    switch (nType) {
      ...
      case 1:
        ...
        if (rtmp.m_pausing == 1) rtmp.m_pausing = 2;
        break;
      ...
      case 31:
        ...
        if (!(rtmp.Link.lFlags & RTMP_LF_BUFX)) break;
        if (!rtmp.m_pausing) {
  ...
  if (nType == 0x1A) {
    ...
    /* respond with HMAC SHA256 of decompressed SWF, key is the 30byte player
     * key, also the last 30 bytes of the server handshake are applied */
...
RtmpSession* RtmpConnection::startSession(const char* uri)
{
  ...
  string local_party = "\"" + rtmp_cfg->FromName + "\" <sip:" + ident + "@";
  ...
  if (!rtmp_cfg->FromDomain.empty()) {
    ...
    if (AmConfig::SIP_Ifs[out_if].LocalPort != 5060)
      local_party += ":" + int2str(AmConfig::SIP_Ifs[out_if].LocalPort);
  }
  ...
  switch (
      AmSessionContainer::instance()->addSession(dialout_id, n_session.get())) {
    ...
    case AmSessionContainer::Inserted: break;
  }

...
{
  if (fds_num) {
    for (unsigned int i = 0; i < fds_num; i++) ::close(fds[i].fd);
  }
}
...
int _RtmpServer::listen(const char* addr, unsigned short port)
{
  ...
  int onoff = 1;
  if (setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &onoff, sizeof(onoff))
      < 0) {
  ...
  if (bind(listen_fd, (const sockaddr*) &listen_addr,
...
void _RtmpServer::on_stop() { ERROR("not yet supported!\n"); }

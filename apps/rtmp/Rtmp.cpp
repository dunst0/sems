...
int RtmpFactory::onLoad()
{
  ...
  if (cfg_file.loadPluginConf(MOD_NAME) < 0) {
    ...
    if (cfg_file.hasParameter("listen_address")) {
      cfg.ListenAddress =
          fixIface2IP(cfg_file.getParameter("listen_address"), false);
    }

    if (cfg_file.hasParameter("listen_port")) {
      string listen_port_str = cfg_file.getParameter("listen_port");
      if (sscanf(listen_port_str.c_str(), "%u", &(cfg.ListenPort)) != 1) {
  ...
  AmDynInvokeFactory* di_reg_client_f =
      AmPlugIn::instance()->getFactory4Di("registrar_client");
  if (di_reg_client_f) di_reg_client = di_reg_client_f->getInstance();

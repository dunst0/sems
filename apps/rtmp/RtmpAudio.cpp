...
static void dump_audio(unsigned char* buffer, unsigned int size)
{
  ...
  if (dump_fd == 0) {
    dump_fd = open("pcm_in.raw", O_WRONLY | O_CREAT | O_TRUNC,
                   S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
    if (dump_fd < 0) ERROR("could not open pcm_in.raw: %s\n", strerror(errno));
  }
  if (dump_fd < 0) return;
...
int RtmpAudio::send(unsigned int user_ts, unsigned int size)
{
  ...
  packet.m_headerType =
      send_offset_i ? RTMP_PACKET_SIZE_MEDIUM : RTMP_PACKET_SIZE_LARGE;
  ...
  unsigned int rtmp_ts =
      (user_ts - send_rtmp_offset) / (SPEEX_WB_SAMPLE_RATE / 1000);
...
void RtmpAudio::process_recv_queue(unsigned int ref_ts)
{
  ...
  while (!q_recv.empty()) {
    ...
    if (p.m_nBodySize <= (unsigned int) AUDIO_BUFFER_SIZE) {
      ...
      unsigned int rtp_ts =
          (p.m_nTimeStamp - recv_rtmp_offset) * (SPEEX_WB_SAMPLE_RATE / 1000);

      playout_buffer.write(ref_ts, rtp_ts,
...
void RtmpAudio::add_to_history(int16_t*, unsigned int) { return; }

unsigned int RtmpAudio::conceal_loss(unsigned int, unsigned char*) { return 0; }

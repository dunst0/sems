...
void SIPRegistrarClient::run()
{
  ...
  if (uac_auth_f == NULL) {
    DBG("unable to get a uac_auth factory. registrations will not be "
        "authenticated.\n");
...
void SIPRegistrarClient::process(AmEvent* ev)
{
  ...
  SIPRemoveRegistrationEvent* rem_reg =
      dynamic_cast<SIPRemoveRegistrationEvent*>(ev);
  if (rem_reg) {
...
void SIPRegistrarClient::onNewRegistration(SIPNewRegistrationEvent* new_reg)
{
  AmSIPRegistration* reg =
      new AmSIPRegistration(new_reg->handle, new_reg->info, new_reg->sess_link);
...
void SIPRegistrarClient::onRemoveRegistration(
    SIPRemoveRegistrationEvent* new_reg)
{
  AmSIPRegistration* reg = get_reg(new_reg->handle);
  if (reg) reg->doUnregister();
}
...
void SIPRegistrarClient::add_reg(const string&      reg_id,
                                 AmSIPRegistration* new_reg)
{
  ...
  if (reg != NULL) delete reg; // old one with the same ltag
}

// API
string SIPRegistrarClient::createRegistration(
    const string& domain, const string& user, const string& name,
    const string& auth_user, const string& pwd, const string& sess_link,
    const string& proxy, const string& contact, const string& handle)
{
  string l_handle = handle.empty() ? AmSession::getNewId() : handle;
  instance()->postEvent(new SIPNewRegistrationEvent(
      SIPRegistrationInfo(domain, user, name, auth_user, pwd, proxy, contact),
      l_handle, sess_link));
                                ...
                                AmArg& ret)
{
  if (method == "createRegistration") {
    ...
    ret.push(createRegistration(args.get(0).asCStr(), args.get(1).asCStr(),
                                ...
                                args.get(4).asCStr(), args.get(5).asCStr(),
    ...
    if (instance()->getRegistrationState(args.get(0).asCStr(), state,
                                         expires)) {
      ret.push(1);
      ret.push((int) state);

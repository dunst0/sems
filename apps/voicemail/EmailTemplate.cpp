...
string EmailTemplate::replaceVars(const string&        str,
                                  const EmailTmplDict& dict) const
{
...
{
  try {
    return AmMail(replaceVars(from, dict), replaceVars(subject, dict),
                  replaceVars(to, dict), replaceVars(body, dict),
                  replaceVars(header, dict));
  }
  catch (const string& err) {
    throw string("EmailTemplate: error in template '" + tmpl_file
                 + "': " + err);
  }
}

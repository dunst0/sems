                                      ...
                                      map<string, string>& params)
{
  ...
  // checks
  if (user.empty()) throw AmSession::Exception(500, MOD_NAME ": user missing");

  string announce_file =
      add2path(AnnouncePath, 2, domain.c_str(), (user + ".wav").c_str());
  ...
  announce_file =
      add2path(AnnouncePath, 2, domain.c_str(), DefaultAnnounce.c_str());
  if (file_exists(announce_file)) goto announce_found;
...
AnnRecorderDialog::~AnnRecorderDialog()
{
  ...
  if (msg_filename.length()) unlink(msg_filename.c_str());
}
...
void AnnRecorderDialog::process(AmEvent* event)
{
  ...
  if (pl_ev) {
    if ((pl_ev->event_id == SEP_MSG_BEGIN) && (S_WAIT_START == state)) {
      // start timer
      setTimer(TIMERID_START_TIMER, START_RECORDING_TIMEOUT);
      return;
    }

    if ((pl_ev->event_id == SEP_CONFIRM_BEGIN) && (S_CONFIRM == state)) {
      // start timer
...
inline UACAuthCred* AnnRecorderDialog::getCredentials() { return cred.get(); }

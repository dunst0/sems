...
void MsgStorage::invoke(const string& method, const AmArg& args, AmArg& ret)
{
  if (method == "msg_new") {
    MessageDataFile* f = dynamic_cast<MessageDataFile*>(args.get(3).asObject());
    if (NULL == f) {
      throw(string("message data is not a file ptr."));
    }
    ret.push(msg_new(args.get(0).asCStr(), args.get(1).asCStr(),
                     args.get(2).asCStr(), f->fp));
  }
  else if (method == "msg_get") {
    msg_get(args.get(0).asCStr(), args.get(1).asCStr(), args.get(2).asCStr(),
            ret);
  }
  else if (method == "msg_markread") {
    ret.push(msg_markread(args.get(0).asCStr(), args.get(1).asCStr(),
                          args.get(2).asCStr()));
  }
  else if (method == "msg_delete") {
    ret.push(msg_delete(args.get(0).asCStr(), args.get(1).asCStr(),
                        args.get(2).asCStr()));
  }
  else if (method == "userdir_open") {
    userdir_open(args.get(0).asCStr(), args.get(1).asCStr(), ret);
  }
  else if (method == "userdir_close") {
    ret.push(userdir_close(args.get(0).asCStr(), args.get(1).asCStr()));
  }
  else if (method == "userdir_getcount") {
    userdir_getcount(args.get(0).asCStr(), args.get(1).asCStr(), ret);
  }
  else if (method == "events_subscribe") {
    events_subscribe(args.get(0).asDynInv(), args.get(1).asCStr());
  }
...
int MsgStorage::msg_new(string domain, string user, string msg_name, FILE* data)
{
...
void MsgStorage::event_notify(const string& domain, const string& user,
                              const string& event)
{
  AmArg args, ret;
  args.push(domain);
  ...
  for (Listeners::iterator it = listeners.begin(); it != listeners.end();
       ++it) {
    try {
...
void MsgStorage::filecopy(FILE* ifp, FILE* ofp)
{
  ...
  while (!feof(ifp)) {
    ...
    if (fwrite(buf, 1, nread, ofp) != nread) break;
  }
}

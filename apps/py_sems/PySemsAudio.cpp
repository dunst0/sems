...
static PyObject* PySemsAudioFile_open(PySemsAudioFile* self, PyObject* args)
{
  ...
  if ((py_is_tmp == NULL) || (py_is_tmp == Py_False))
    ...
    PyErr_SetString(PyExc_TypeError,
                    "third parameter should be of type PyBool");
    return NULL;
  }
...
static PyObject* PySemsAudioFile_tts(PyObject* cls, PyObject* args)
{
  ...
  if (tts_file == NULL) {
    ...
    PyErr_SetString(PyExc_RuntimeError,
                    "could not create new PySemsAudioFile object");
    return NULL;
  }
  ...
  *self->filename =
      string(TTS_CACHE_PATH) + AmSession::getNewId() + string(".wav");
  self->del_file = true;
...
static PyObject* PySemsAudioFile_setRecordTime(PySemsAudioFile* self,
                                               PyObject*        args)
{
  int rec_time;
  if (!PyArg_ParseTuple(args, "i", &rec_time)) return NULL;
...
{
  if (self->af->getMode() == AmAudioFile::Write) self->af->on_close();
...
static int PySemsAudioFile_setloop(PySemsAudioFile* self, PyObject* value,
                                   void*)
{
  if (value == NULL) {
...
static PyGetSetDef PySemsAudioFile_getseters[] = {
    {"loop", (getter) PySemsAudioFile_getloop, (setter) PySemsAudioFile_setloop,
     "repeat mode", NULL},
    {NULL} /* Sentinel */
};

...
{
  if (_instance == NULL) _instance = new Monitor(MOD_NAME);
  return _instance;
}
...
void Monitor::invoke(const string& method, const AmArg& args, AmArg& ret)
{
...
void Monitor::inc(const AmArg& args, AmArg& ret) { add(args, ret, 1); }

void Monitor::dec(const AmArg& args, AmArg& ret) { add(args, ret, -1); }
...
void Monitor::addSample(const AmArg& args, AmArg& ret)
{
  ...
  if ((!sample_list.empty()) && timercmp(&sample_list.front().time, &now, >=)) {
    // sample list time stamps needs to be monotonically increasing - clear if
    // resyncing WARN("clock drift backwards - clearing %zd items\n",
    // sample_list.size());
    sample_list.clear();
  }
...
void Monitor::truncate_samples(list<SampleInfo::time_cnt>& v,
                               struct timeval              now)
{
  struct timeval cliff = now;
  cliff.tv_sec -= retain_samples_s;
  while ((!v.empty()) && timercmp(&cliff, &(v.back().time), >=)) v.pop_back();
}
...
void Monitor::getCount(const AmArg& args, AmArg& ret)
{
  ...
  map<string, SampleInfo>::iterator it = bucket.samples.find(args[0].asCStr());
  if (it != bucket.samples.end()) {
    ...
    if (s_it != it->second.sample.end()) {
      ...
      while (v_it != v.end() && timercmp(&(v_it->time), &to, >)) v_it++;
      if (v_it != v.end()) {
...
void Monitor::getAllCounts(const AmArg& args, AmArg& ret)
{
  ...
  if (it != bucket.samples.end()) {
    for (map<string, list<SampleInfo::time_cnt>>::iterator s_it =
             it->second.sample.begin();
         s_it != it->second.sample.end(); s_it++) {
      list<SampleInfo::time_cnt>& v = s_it->second;
      truncate_samples(v, now);
      list<SampleInfo::time_cnt>::iterator v_it = v.begin();
      ...
      while (timercmp(&(v_it->time), &to, >) && v_it != v.end()) v_it++;
      if (v_it != v.end()) {
        while (timercmp(&(v_it->time), &from, >=) && v_it != v.end()) {
          res += v_it->counter;
          v_it++;
...
void Monitor::clearFinished()
{
  ...
  for (int i = 0; i < NUM_LOG_BUCKETS; i++) {
    ...
    while (it != logs[i].log.end()) {
      if (it->second.finished && it->second.finished <= now) {
...
void Monitor::get(const AmArg& args, AmArg& ret)
{
  ...
  if (it != bucket.log.end()) ret.push(it->second.info);
  bucket.log_lock.unlock();
}
...
void Monitor::getSingle(const AmArg& args, AmArg& ret)
{
  ...
  DBG("getSingle(%s,%s)", args[0].asCStr(), args[1].asCStr());
...
void Monitor::getAttribute(const AmArg& args, AmArg& ret)
{
  ...
  for (int i = 0; i < NUM_LOG_BUCKETS; i++) {
    logs[i].log_lock.lock();
    for (std::map<string, LogInfo>::iterator it = logs[i].log.begin();
         it != logs[i].log.end(); it++) {
      ret.push(AmArg());
...
DEF_GET_ATTRIB_FUNC(getAttributeActive,
                    (!(it->second.finished && it->second.finished <= now)))
DEF_GET_ATTRIB_FUNC(getAttributeFinished,
                    (it->second.finished && it->second.finished <= now))
    ...
    try {
      for (std::map<string, LogInfo>::iterator it = logs[i].log.begin();
        ...
        if (!it->second.info.hasMember(args[0].asCStr())
            ...
            || regexec(&attr_reg, it->second.info[args[0].asCStr()].asCStr(), 0,
                       0, 0))
          continue;
...
{
  if (call_id.empty()) return logs[0];
  char c =
      '\0'; // some distribution...bad luck if all callid start with 00000...
  for (size_t i = 0; i < 5 && i < call_id.length(); i++) c = c ^ call_id[i];

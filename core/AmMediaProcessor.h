...
class AmMediaSession
{
 ...
 public:
   ...
   *
   * \param buffer multi-purpose space given from outside
   * (AmMediaProcessorThread)
   *
   * Buffer given as parametr is usable for anything, originally was intended
   * for data read from one stream before putting to another stream.
   *
...
class AmMediaProcessor
{
 ...
 public:
   ...
   * SoftRemoveSession : remove the session from the processor but leave it
   * attached ClearSession      : remove the session from processor and clear
   * audio
   */
  enum
  {

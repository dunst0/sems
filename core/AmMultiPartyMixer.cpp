...
void DEBUG_MIXER_BUFFER_STATE(const MixerBufferState& mbs,
                              const string&           context)
{
  ...
  for (MixerBufferState::ChannelMap::const_iterator it = mbs.channels.begin();
       it != mbs.channels.end(); it++) {
                                         ...
                                         unsigned int           size)
{
  ...
  if ((channel = bstate->get_channel(channel_id)) != 0) {
    ...
    bstate->last_ts =
        put_ts
        + (samples * (WALLCLOCK_RATE / 100) / (GetCurrentSampleRate() / 100));
  }
  else {
    /*
                                         ...
                                         unsigned int& output_sample_rate)
{
  ...
  SampleArrayShort* channel = 0;
  if (bstate != buffer_state.end()
      && (channel = bstate->get_channel(channel_id)) != 0) {
...
void AmMultiPartyMixer::cleanupBufferStates(unsigned int last_ts)
{
  while (!buffer_state.empty()
         && (buffer_state.front().last_ts != 0
             && buffer_state.front().last_ts < last_ts)
         && (unsigned int) GetCurrentSampleRate()
                != buffer_state.front().sample_rate) {
    // DEBUG_MIXER_BUFFER_STATE(buffer_state.front(), "freed in
    // cleanupBufferStates");
...
void AmMultiPartyMixer::lock() { audio_mut.lock(); }

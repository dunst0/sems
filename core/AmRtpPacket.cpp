...
unsigned char* AmRtpPacket::getData() { return &buffer[data_offset]; }

unsigned char* AmRtpPacket::getBuffer() { return &buffer[0]; }
...
{
  int err = ::sendto(sd, buffer, b_size, 0, (const struct sockaddr*) &addr,
                     SA_len(&addr));
...
int AmRtpPacket::recv(int sd)
{
  ...
  int ret = recvfrom(sd, buffer, sizeof(buffer), 0, (struct sockaddr*) &addr,
                     &recv_addr_len);

  if (ret > 0) {
    if (ret > 4096) return -1;
...
void AmRtpPacket::logReceived(msg_logger*              logger,

...
const char* __timer_id_str[2] = {"RFC6665_TIMER_N", "SUBSCRIPTION_EXPIRE"};

const char* __sub_state_str[] = {"init", "notify_wait", "pending", "active",
                                 "terminated"};
...
{
  DBG("[%p] tag=%s;role=%s timer_id = %s\n", this, dlg()->getLocalTag().c_str(),
      role ? "Notifier" : "Subscriber", __timer_id_str[timer_id]);
  ...
  switch (timer_id) {
    ...
    case SUBSCRIPTION_EXPIRE:
      if (subs->ev_q) {
        AmEvent* ev =
            new SingleSubTimeoutEvent(subs->dlg->getLocalTag(), timer_id, this);
...
void SingleSubscription::terminate() { setState(SubState_terminated); }
                                                        ...
                                                        bool                uac)
{
  SingleSubscription::Role role =
      uac ? SingleSubscription::Subscriber : SingleSubscription::Notifier;
...
{
  if ((req.method == SIP_METH_SUBSCRIBE) || (req.method == SIP_METH_REFER)) {
...
void SingleSubscription::replyFSM(const AmSipRequest& req,
                                  const AmSipReply&   reply)
{
  ...
  if ((req.method == SIP_METH_SUBSCRIBE) || (req.method == SIP_METH_REFER)) {
      ...
      if (!expires_txt.empty() && str2int(expires_txt, sub_expires)) {
        if (sub_expires) {
          DBG("setTimer(%s,SUBSCRIPTION_EXPIRE)\n",
              dlg()->getLocalTag().c_str());
          AmAppTimer::instance()->setTimer(&timer_expires,
                                           (double) sub_expires);
    ...
    string sub_state_txt =
        getHeader(req.hdrs, SIP_HDR_SUBSCRIPTION_STATE, true);
    string expires_txt   = get_header_param(sub_state_txt, "expires");
    int    notify_expire = 0;

    if (!expires_txt.empty()) str2int(expires_txt, notify_expire);
...
void SingleSubscription::setState(unsigned int st)
{
  ...
  if (sub_state == SubState_terminated) return;
...
{
  return "[" + str2json(event) + "," + str2json(id) + ","
...
bool AmSipSubscription::isActive()
{
  for (Subscriptions::iterator it = subs.begin(); it != subs.end(); it++) {
    if ((*it)->getState() == SingleSubscription::SubState_active) return true;
  }
                                    ...
                                    const string& event, const string& id)
{
  ...
  DBG("searching for event='%s'; id='%s'; no_id=%i", event.c_str(), id.c_str(),
      no_id);

  for (Subscriptions::iterator it = subs.begin(); it != subs.end(); it++) {
    SingleSubscription* sub = *it;

    DBG("role='%s';event='%s';id='%s'", sub->role ? "Notifier" : "Subscriber",
        sub->event.c_str(), sub->id.c_str());

    if ((sub->role == role) && (sub->event == event)
        && (no_id || (sub->id == id))) {
      match = it;
      DBG("\tmatched!");
      break;
  ...
  for (CSeqMap::iterator i = uas_cseq_map.begin(); i != uas_cseq_map.end();) {
    if (i->second == sub) {
      ...
      if (req) {
        ...
        DBG("request not found: could not reply 481 to pending UAS "
            "transaction");
      }
...
void AmSipSubscription::onRequestSent(const AmSipRequest& req)
{
  ...
  if (sub_it == subs.end()) {
    if ((req.method == SIP_METH_NOTIFY) && allow_subless_notify) {
      return;
    }
                                    ...
                                    const AmSipReply&   reply)
{
  ...
  if (cseq_it == uas_cseq_map.end()) return;
...
void AmSipSubscription::onTimeout(int timer_id, SingleSubscription* sub)
{
  ...
  if (it == subs.end()) return; // no match...
...
AmSipSubscriptionDialog::AmSipSubscriptionDialog(
...
int AmSipSubscriptionDialog::subscribe(int expires)
{
  ...
  if (!event.empty()) {
    ...
    if (!event_id.empty()) hdrs += ";id=" + event_id;
    hdrs += CRLF;
  }
                                       ...
                                       SingleSubscription* sub)
{
  ...
  SIPSubscriptionEvent* sub_ev = new SIPSubscriptionEvent(
  ...
  if (!req.body.empty()) sub_ev->notify_body.reset(new AmMimeBody(req.body));
...
void AmSipSubscriptionDialog::onTimeout(int timer_id, SingleSubscription* sub)
{
  ...
  SIPSubscriptionEvent* sub_ev = new SIPSubscriptionEvent(
      SIPSubscriptionEvent::SubscriptionTimeout, local_tag);

...
class PayloadMask
{
 ...
 public:
  ...
  {
    if (payload_id < 128) bits[payload_id / 8] |= 1 << (payload_id % 8);
  }
...
class AmRtpStream : public AmObject
{
 ...
 public:
  /**
   * Set whether RTP stream will receive RTP packets internally (received
   * packets will be dropped or not).
   */
  ...
  int compile_and_send(const int payload, bool marker, unsigned int ts,
  ...
  /**
   * Generate an answer for the given SDP media based on the stream
   * capabilities.
   * @param index index of the SDP media within the SDP.
   * @param offer the remote offer.
   * @param answer the local answer to be filled/completed.
   */
  virtual void getSdpAnswer(unsigned int index, const SdpMedia& offer,
                            SdpMedia& answer);

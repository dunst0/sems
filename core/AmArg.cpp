...
AmArg& AmArg::operator=(const AmArg& v)
{
  if (this != &v) {
    ...
    switch (type) {
      ...
      case Array: {
        v_array = new ValueArray(*v.v_array);
      } break;
      case Struct: {
...
AmArg::AmArg(map<string, string>& v)
    : type(Undef)
{
  assertStruct();
...
AmArg::AmArg(map<string, AmArg>& v)
    : type(Undef)
{
  assertStruct();
  for (map<string, AmArg>::iterator it = v.begin(); it != v.end(); it++) {
    (*v_struct)[it->first] = it->second;
  }
}

AmArg::AmArg(vector<string>& v)
    : type(Array)
{
  assertArray(0);
...
{
  if (Array != type) throw TypeMismatchException();
}
...
void AmArg::assertArray(size_t s)
{
  ...
  if (v_array->size() < s) v_array->resize(s);
}
...
{
  if (Struct != type) throw TypeMismatchException();
}
...
void AmArg::pop(AmArg& a)
{
  assertArray();
  if (!size()) {
    if (a.getType() == AmArg::Undef) return;
    a = AmArg();
    return;
  }
  a = v_array->front();
...
{
  if (Array == type) return v_array->size();

  if (Struct == type) return v_struct->size();
...
{
  assertArray();
  if (!v_array->size()) throw OutOfBoundsException();
...
{
  assertArray();
  if (idx >= v_array->size()) throw OutOfBoundsException();
...
{
  if (idx < 0) throw OutOfBoundsException();
...
AmArg& AmArg::operator[](int idx) const
{
  ...
  assertArray();
  if ((size_t) idx >= v_array->size()) throw OutOfBoundsException();

  return (*v_array)[idx];
}

AmArg& AmArg::operator[](string key)
{
  assertStruct();
  return (*v_struct)[key];
}

AmArg& AmArg::operator[](string key) const
{
  assertStruct();
  return (*v_struct)[key];
}
...
bool operator==(const AmArg& lhs, const AmArg& rhs)
{
  ...
  switch (lhs.type) {
    case AmArg::Int: {
      return lhs.v_int == rhs.v_int;
    } break;
    case AmArg::LongLong: {
      return lhs.v_long == rhs.v_long;
    } break;
    case AmArg::Bool: {
      return lhs.v_bool == rhs.v_bool;
    } break;
    case AmArg::Double: {
      return lhs.v_double == rhs.v_double;
    } break;
    case AmArg::CStr: {
      return !strcmp(lhs.v_cstr, rhs.v_cstr);
    } break;
    case AmArg::AObject: {
      return lhs.v_obj == rhs.v_obj;
    } break;
    case AmArg::ADynInv: {
      return lhs.v_inv == rhs.v_inv;
    } break;
    case AmArg::Array: {
      return lhs.v_array == rhs.v_array;
    } break;
    case AmArg::Struct: {
      return lhs.v_struct == rhs.v_struct;
    } break;
    case AmArg::Blob: {
...
vector<string> AmArg::enumerateKeys() const
{
  assertStruct();
  vector<string> res;

  for (ValueStruct::iterator it = v_struct->begin(); it != v_struct->end();
       it++) {
...
void AmArg::erase(const string& name)
{
  assertStruct();
  v_struct->erase(name);
}
    ...
    for (size_t i = 0; i < fmt_len; i++) {
      switch (format[i]) {
        ...
        case 's':
          ...
          break;
        case 'o':
          assertArgAObject(get(i));
          got += 'o';
          break;
        case 'd':
          assertArgADynInv(get(i));
          got += 'd';
          break;
        case 'a':
          assertArgArray(get(i));
          got += 'a';
          break;
        case 'b':
          assertArgBlob(get(i));
          ...
          break;
        case 'u':
          assertArgStruct(get(i));
          got += 'u';
          break;
        default:
          got += '?';
          ERROR("ignoring unknown format type '%c'\n", format[i]);
          break;
      }
    }
  }
...
#define VECTOR_GETTER(type, name, getter)                                      \
  ...
  {                                                                            \
    vector<type> res;                                                          \
    for (size_t i = 0; i < size(); i++) res.push_back(get(i).getter());        \
    return res;                                                                \
  }
...
{
  vector<ArgBlob> res;
  for (size_t i = 0; i < size(); i++) res.push_back(*get(i).asBlob());
  return res;
}

void AmArg::clear() { invalidate(); }
  ...
  switch (a.getType()) {
    ...
    case Int:
      return a.asInt() < 0 ? "-" + int2str(abs(a.asInt()))
                           : int2str(abs(a.asInt()));
    case LongLong: return longlong2str(a.asLongLong());
    case Bool: return a.asBool() ? "true" : "false";
    case Double: return double2str(a.asDouble());
    case CStr: return "'" + string(a.asCStr()) + "'";
    case AObject: return "<Object>";
    case ADynInv: return "<DynInv>";
    case Blob: s = "<Blob of size:" + int2str(a.asBlob()->len) + ">";
    case Array:
      s = "[";
      for (size_t i = 0; i < a.size(); i++) s += print(a[i]) + ", ";
...
int arg2int(const AmArg& a)
{
  ...
  if (isArgCStr(a)) {
    ...
    if (!str2int(a.asCStr(), res)) {
      throw string("can't convert arg to int: " + string(a.asCStr()));
    }
    return res;
  }

  throw string("can't convert arg to int");
}
...
string arg2str(const AmArg& a)
{
  ...
  throw string("can't convert arg to string");
}

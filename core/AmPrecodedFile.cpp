...
amci_codec_t _codec_precoded = {PRECODED_CODEC_ID,
                                NULL,
                                NULL,
                                NULL,
                                NULL,
...
AmPrecodedRtpFormat::AmPrecodedRtpFormat(precoded_payload_t& precoded_payload)
    : AmAudioRtpFormat()
    , precoded_payload(precoded_payload)
{
...
int AmPrecodedFile::open(const string& filename)
{
  ...
  while (ifs.good() && !ifs.eof()) {
    ...
    if (!codec_line.length() || codec_line[0] == '#') continue;
...
#undef get_uint_item
    ...
    DBG("inserting codec '%s' file '%s' and id %d\n", pl.name,
        pl.filename.c_str(), pl.payload_id);
...
int AmPrecodedFile::getDynPayload(const string& name, int rate,

...
bool AmSipDialog::onRxReqSanity(const AmSipRequest& req)
{
  if (req.method == SIP_METH_ACK) {
    if (onRxReqStatus(req) && hdl) hdl->onSipRequest(req);
    return false;
  }
  ...
  if (!AmBasicSipDialog::onRxReqSanity(req)) return false;
...
bool AmSipDialog::onRxReqStatus(const AmSipRequest& req)
{
  switch (status) {
    case Disconnected:
      if (req.method == SIP_METH_INVITE) setStatus(Trying);
      break;
    case Connected:
      if (req.method == SIP_METH_BYE) setStatus(Disconnecting);
      break;
...
AmOfferAnswer::OAState AmSipDialog::getOAState() { return oa.getState(); }

void AmSipDialog::setOAState(AmOfferAnswer::OAState n_st) { oa.setState(n_st); }
...
int AmSipDialog::onTxRequest(AmSipRequest& req, int& flags)
{
  ...
  if (offeranswer_enabled && oa.onRequestOut(req)) return -1;

  if (AmBasicSipDialog::onTxRequest(req, flags) < 0) return -1;
...
int AmSipDialog::onTxReply(const AmSipRequest& req, AmSipReply& reply,
                           int& flags)
{
  if (offeranswer_enabled) {
    if (oa.onReplyOut(reply) < 0) return -1;
  }
  ...
  switch (status) {
    ...
    case Disconnecting:
      if (reply.cseq_method == SIP_METH_BYE) {
        ...
        if (reply.code >= 200) setStatus(Disconnected);
      }
      break;
...
void AmSipDialog::onReplyTxed(const AmSipRequest& req, const AmSipReply& reply)
{
  ...
  if (reply.code >= 200) {
    if (reply.cseq_method == SIP_METH_INVITE) pending_invites--;
  }
}
...
bool AmSipDialog::getUACInvTransPending()
{
  for (TransMap::iterator it = uac_trans.begin(); it != uac_trans.end(); it++) {
    if (it->second.method == SIP_METH_INVITE) return true;
  }
  return false;
}
...
{
  for (TransMap::iterator it = uas_trans.begin(); it != uas_trans.end(); it++) {
    if (it->second.method == SIP_METH_INVITE) return &(it->second);
  }
  return NULL;
}
  ...
  switch (status) {
    ...
    case Early:
      if (getUACInvTransPending())
        ...
        for (TransMap::iterator it = uas_trans.begin(); it != uas_trans.end();
             it++) {
          if (it->second.method == SIP_METH_INVITE) {
    ...
    case Cancelling:
      ...
      // missing AmSipRequest to be able
      // to send the reply on behalf of the app.
      DBG("ignoring bye() in %s state: no UAS transaction to reply",
          getStatusStr());
      setStatus(Disconnected);
      return 0;
...
int AmSipDialog::reinvite(const string& hdrs, const AmMimeBody* body, int flags)
{
...
int AmSipDialog::refer(const string& refer_to, int expires,
...
int AmSipDialog::prack(const AmSipReply& reply1xx, const AmMimeBody* body,
                       const string& hdrs)
{
  switch (getStatus()) {
    case Trying:
    ...
    case Disconnecting:
      ...
      return -1;
    default: ERROR("BUG: unexpected dialog state '%d'.\n", status); return -1;
  }
  string h = hdrs + SIP_HDR_COLSP(SIP_HDR_RACK) + int2str(reply1xx.rseq) + " "
             + int2str(reply1xx.cseq) + " " + reply1xx.cseq_method + CRLF;
  return sendRequest(SIP_METH_PRACK, body, h);
}

int AmSipDialog::cancel()
{
  for (TransMap::reverse_iterator t = uac_trans.rbegin(); t != uac_trans.rend();
       t++) {
    if (t->second.method == SIP_METH_INVITE) {
      if (getStatus() != Cancelling) {
        setStatus(Cancelling);
        return SipCtrlInterface::cancel(&t->second.tt, local_tag, t->first,
                                        t->second.hdrs);
      }
      else {
...
int AmSipDialog::send_200_ack(unsigned int inv_cseq, const AmMimeBody* body,
                              const string& hdrs, int flags)
{
  ...
  if (onTxRequest(req, flags) < 0) return -1;

  if (!(flags & SIP_FLAGS_VERBATIM)) {
    // add Signature
    if (AmConfig::Signature.length())
      req.hdrs +=
          SIP_HDR_COLSP(SIP_HDR_USER_AGENT) + AmConfig::Signature + CRLF;
  }

  int res = SipCtrlInterface::send(
      req, local_tag, remote_tag.empty() || !next_hop_1st_req ? next_hop : "",

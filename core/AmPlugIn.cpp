...
amci_codec_t _codec_pcm16 = {
    CODEC_PCM16,         NULL, NULL, NULL, NULL, NULL, pcm16_bytes2samples,
    pcm16_samples2bytes, NULL};

amci_codec_t _codec_tevent = {
    CODEC_TELEPHONE_EVENT, NULL, NULL, NULL, NULL, NULL, tevent_bytes2samples,
    tevent_samples2bytes,  NULL};

amci_payload_t _payload_tevent = {-1,
                                  "telephone-event",
...
{
  if (!_instance) _instance = new AmPlugIn();
...
void AmPlugIn::init()
{
  vector<string> excluded_payloads_v = explode(AmConfig::ExcludePayloads, ";");
  for (vector<string>::iterator it = excluded_payloads_v.begin();
...
int AmPlugIn::load(const string& directory, const string& plugins)
{
  ...
  if (!plugins.length()) {
    ...
    while (((entry = readdir(dir)) != NULL) && (err == 0)) {
      ...
      if (excluded_plugins_s.find(
              plugin_name.substr(0, plugin_name.length() - 3))
  ...
  for (vector<AmPluginFactory*>::iterator it = loaded_plugins.begin();
                         ...
                         vector<AmPluginFactory*>& plugins)
{
  ...
  if (!strcmp(bname, "dsm.so") || !strcmp(bname, "ivr.so")
      ...
      || !strcmp(bname, "diameter_client.so")
      || !strcmp(bname, "registrar_client.so") || !strcmp(bname, "uac_auth.so")
      || !strcmp(bname, "msg_storage.so")) {
  ...
  bool has_sym = false;
  if (exports) {
    if (loadAudioPlugIn(exports)) goto error;
    goto end;
  }

  if ((fc = (FactoryCreate) dlsym(h_dl, FACTORY_SESSION_EXPORT_STR)) != NULL) {
    plugin = (AmPluginFactory*) fc();
    if (loadAppPlugIn(plugin)) goto error;
    has_sym = true;
    if (NULL != plugin) plugins.push_back(plugin);
  }
  if ((fc = (FactoryCreate) dlsym(h_dl,
                                  FACTORY_SESSION_EVENT_HANDLER_EXPORT_STR))
      != NULL) {
    plugin = (AmPluginFactory*) fc();
    if (loadSehPlugIn(plugin)) goto error;
    has_sym = true;
    if (NULL != plugin) plugins.push_back(plugin);
  }
  if ((fc = (FactoryCreate) dlsym(h_dl, FACTORY_PLUGIN_EXPORT_STR)) != NULL) {
    plugin = (AmPluginFactory*) fc();
    if (loadBasePlugIn(plugin)) goto error;
    has_sym = true;
    if (NULL != plugin) plugins.push_back(plugin);
  }
  if ((fc = (FactoryCreate) dlsym(h_dl, FACTORY_PLUGIN_CLASS_EXPORT_STR))
      != NULL) {
    plugin = (AmPluginFactory*) fc();
    if (loadDiPlugIn(plugin)) goto error;
    has_sym = true;
    if (NULL != plugin) plugins.push_back(plugin);
  }
  ...
  if ((fc = (FactoryCreate) dlsym(h_dl, FACTORY_LOG_FACILITY_EXPORT_STR))
      != NULL) {
    plugin = (AmPluginFactory*) fc();
    if (loadLogFacPlugIn(plugin)) goto error;
    has_sym = true;
    if (NULL != plugin) plugins.push_back(plugin);
  }
...
string AmPlugIn::getSdpFormatParameters(int codec_id, bool is_offer,
                                        const string& fmt_params_in)
{
  amci_codec_t* c = codec(codec_id);
  if (NULL == c) return ""; // empty for unsupported codec

  if (NULL == c->negotiate_fmt) return ""; // empty if codec doesn't know either

  char out_fmt[256] = {'\0'};
  if ((c->negotiate_fmt)(is_offer ? 1 : 0, fmt_params_in.c_str(), out_fmt, 256)
      >= 0)
...
int AmPlugIn::getDynPayload(const string& name, int rate,
  ...
  for (std::map<int, int>::const_iterator it = payload_order.begin();
    ...
    if (pl_it != payloads.end()) {
      ...
      pl_vec.push_back(SdpPayload(pl_it->first, pl_it->second->name,
                                  pl_it->second->advertised_sample_rate,
...
{
  AmSessionEventHandlerFactory* sf =
      dynamic_cast<AmSessionEventHandlerFactory*>(f);
  if (!sf) {
...
AmSessionFactory* AmPlugIn::findSessionFactory(const AmSipRequest& req,
                                               string&             app_name)
{
  string m_app_name;

  switch (AmConfig::AppSelect) {
    case AmConfig::App_RURIUSER: m_app_name = req.user; break;
    ...
    case AmConfig::App_MAPPING:
      ...
      break;
    case AmConfig::App_SPECIFIED: m_app_name = AmConfig::Application; break;
  }
...
bool AmPlugIn::registerLoggingFacility(const string&      lf_name,
                                       AmLoggingFacility* f)
{

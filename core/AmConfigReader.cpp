...
static int fifo_get_line(FILE* fifo_stream, char* str, size_t len)
{
  ...
  while (l && (c = fgetc(fifo_stream)) && !ferror(fifo_stream) && c != EOF
         && c != '\n') {
    if (c != '\r') {
      *(s++) = c;
      l--;
    ...
    if ((key_beg < key_end) && (val_beg <= val_end)) {
      ...
      if (hasParameter(keyname)) {
        WARN("while loading '%s': overwriting configuration "
             "'%s' value '%s' with  '%s'\n",
             path.c_str(), keyname.c_str(), getParameter(keyname).c_str(),
             val.c_str());
      }
      ...
      if (keyname == "plugin_config_path") AmConfig::ModConfigPath = val;
    }
    else
      goto syntax_error;
  }
...
static int str_get_line(const char** c, const char* end, char* line_buf,
                        size_t line_buf_len)
{
  enum
  {
    ...
    if ((key_beg < key_end) && (val_beg <= val_end)) {
      ...
      if (hasParameter(keyname)) {
        WARN("while loading string: overwriting configuration "
             "'%s' value '%s' with  '%s'\n",
             keyname.c_str(), getParameter(keyname).c_str(), val.c_str());
      }
...
void AmConfigReader::eraseParameter(const string& param) { keys.erase(param); }
...
{
  for (map<string, string>::iterator it = keys.begin(); it != keys.end();
       it++) {

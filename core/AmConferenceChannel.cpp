...
{
  if (own_channel) AmConferenceStatus::releaseChannel(conf_id, channel_id);
                             ...
                             unsigned int size)
{
  ...
  mixer->PutChannelPacket(channel_id, system_ts, (unsigned char*) samples,
                          size);
  mixer->unlock();
  return size;
}
                             ...
                             unsigned int nb_samples)
{
  ...
  unsigned int size = output_sample_rate
                          ? PCM16_S2B(nb_samples * mixer->GetCurrentSampleRate()
                                      / output_sample_rate)
                          : 0;
  unsigned int mixer_sample_rate = 0;
  mixer->GetChannelPacket(channel_id, system_ts, buffer, size,
                          mixer_sample_rate);

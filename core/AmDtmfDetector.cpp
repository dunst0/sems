...
void AmSipDtmfEvent::parseLine(const string& line)
{
  ...
  if (line.length() > KeySignal.length()
    ...
    switch (event.c_str()[0]) {
      case '*': m_event = 10; break;
      case '#': m_event = 11; break;
      case 'A':
      case 'a': m_event = 12; break;
      case 'B':
      case 'b': m_event = 13; break;
      case 'C':
    ...
    m_duration_msec =
        atol(line.substr(KeyDuration.length(), string::npos).c_str());
    ...
    , m_rtpEventReceived(false)
{
  ...
  //  m_session->RTPStream()->getSampleRate());
  //#else
                                       ...
                                       int                      sample_rate)
{
  ...
  if ((t != m_inband_type) || (!m_inbandDetector.get())) {
    if (t == Dtmf::SEMSInternal) {
      ...
      m_inbandDetector.reset(
          new AmSpanDSPInbandDtmfDetector(this, sample_rate));
    }
    m_inband_type = t;
  }
#endif
...
void AmDtmfDetector::process(AmEvent* evt)
{
  ...
  switch (event->event_id) {
    ...
    //            m_audioDetector.process(dynamic_cast<AmAudioDtmfEvent
    //            *>(event)); break;
                                         ...
                                         unsigned int          event_id)
{
  ...
  if ((m_eventPending && m_currentEvent != event)
      || (m_eventPending && has_eventid && m_current_eventid_i
          && (event_id != m_current_eventid))) {
#ifdef EXCESSIVE_DTMF_DEBUGINFO
...
{
#ifdef EXCESSIVE_DTMF_DEBUGINFO
  DBG("registerKeyPressed(%d, .., %s, %u); m_eventPending=%s, "
      "m_currentEvent=%d, "
      "m_current_eventid=%u,m_current_eventid_i=%s\n",
      event, has_eventid ? "true" : "false", event_id,
      m_eventPending ? "true" : "false", m_currentEvent, m_current_eventid,
      m_current_eventid_i ? "true" : "false");
#endif
  struct timeval tm;
  ...
  if (!m_eventPending) {
    ...
    if ((m_currentEvent != event)
        || (has_eventid && m_current_eventid_i
            && (event_id != m_current_eventid))) {
#ifdef EXCESSIVE_DTMF_DEBUGINFO
...
void AmRtpDtmfDetector::process(AmRtpDtmfEvent* evt)
{
  if (evt && evt->volume() < 55) // From RFC 2833:
                                 // The range of valid DTMF is from 0 to -36
                                 // dBm0 (must accept); lower than -55 dBm0 must
                                 // be rejected (TR-TSY-000181, ITU-T Q.24A)
  {
...
static dtmf_t dtmf_tones[8] = {{697, LOGRP},  {770, LOGRP},  {852, LOGRP},
                               {941, LOGRP},  {1209, HIGRP}, {1336, HIGRP},
                               {1477, HIGRP}, {1633, HIGRP}};

static char dtmf_matrix[4][4] = {{'1', '2', '3', 'A'},
                                 {'4', '5', '6', 'B'},
                                 {'7', '8', '9', 'C'},
                                 {'*', '0', '#', 'D'}};
...
void AmSemsInbandDtmfDetector::isdn_audio_goertzel_relative()
{
  ...
  for (int k = 0; k < REL_NCOEFF; k++) {
    // like m_buf, sk..sk2 are in (32-REL_AMP_BITS).REL_AMP_BITS fixed-point
    // format
    sk = sk1 = sk2 = 0;
...
void AmSemsInbandDtmfDetector::isdn_audio_eval_dtmf_relative()
{
  ...
  for (int i = 0; i < REL_NCOEFF; i++) {
    if (m_result[i] > REL_DTMF_TRESH) {
      if (m_result[i] > thresh) thresh = m_result[i];
    }
  ...
  if (silence == REL_NCOEFF)
    ...
    if (thresh > 0) {
      ...
      for (int i = 0; i < REL_NCOEFF; i++) {
        if (m_result[i] < thresh) continue; /* ignore */
      ...
      if ((grp[LOGRP] >= 0) && (grp[HIGRP] >= 0)) {
        ...
        if (what != m_last) {
          ...
          m_startTime.tv_usec =
              ((m_last_ts * 10000) / (SAMPLERATE / 100)) % 1000000;
        }
      }
      else
        what = '.';
                                                    ...
                                                    int len, unsigned int ts)
{
  ...
  while (len) {
    c = (len < ((int) NELEMSOF(m_buf) - m_idx)) ? len
                                                : (NELEMSOF(m_buf) - m_idx);
...
AmSpanDSPInbandDtmfDetector::AmSpanDSPInbandDtmfDetector(
    AmKeyPressSink* keysink, int sample_rate)
    : AmInbandDtmfDetector(keysink)
{
...
void AmSpanDSPInbandDtmfDetector::tone_report_f(int code, int level, int delay)
{
  ...
  if (code) { // key pressed
    ...
    m_keysink->registerKeyReleased(char2int(m_lastCode), Dtmf::SOURCE_INBAND,
                                   key_start, now);
  }
}
...
int AmSpanDSPInbandDtmfDetector::char2int(char code)
{
  ...
  if (code == '#') return IVR_DTMF_HASH;
  if (code == '*') return IVR_DTMF_ASTERISK;

  if (code >= 'A' && code <= 'D') return code - 'A';
  return code;
}

// unused - we use realtime reporting functions instead
// void AmSpanDSPInbandDtmfDetector::dtmf_rx_callback(void* user_data, const
// char* digits, int len) {
//   AmSpanDSPInbandDtmfDetector* o = (AmSpanDSPInbandDtmfDetector*)user_data;

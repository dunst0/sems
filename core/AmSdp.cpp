...
{
  return user == other.user && sessId == other.sessId && sessV == other.sessV
         && conn == other.conn;
}
...
string SdpMedia::type2str(int type) { return media_t_2_str(type); }
...
bool SdpAttribute::operator==(const SdpAttribute& other) const
{
  return attribute == other.attribute
         && (value.empty() || (value == other.value));
}

bool SdpMedia::operator==(const SdpMedia& other) const
{
  if (payloads.empty()) {
    if (!other.payloads.empty()) return false;
  }
  ...
  return type == other.type && port == other.port && nports == other.nports
         && transport == other.transport && conn == other.conn
         && dir == other.dir && send == other.send && recv == other.recv;
}
...
bool RtcpAddress::parse(const string& src)
{
  ...
  for (int i = 0; i < len; ++i) {
    switch (s) {
      ...
      case ADDR: address += src[i]; break;
    }
  }
  return s == PORT || (s == ADDR && !address.empty());
...
string RtcpAddress::print()
{
  ...
  if (!address.empty()) s += " IN " + addrtype + " " + address;
  return s;
}

RtcpAddress::RtcpAddress(const string& attr_value)
    : port(0)
{
  if (!parse(attr_value))
...
void AmSdp::print(string& body) const
{
  string out_buf = "v=" + int2str(version)
                   + "\r\n"
                     "o="
                   + origin.user + " " + int2str(origin.sessId) + " "
  ...
  for (vector<SdpMedia>::const_iterator media_it = media.begin();
    ...
    out_buf += "m=" + media_t_2_str(media_it->type) + " "
               ...
               + transport_p_2_str(media_it->transport);

    string options;

    if (media_it->transport == TP_RTPAVP || media_it->transport == TP_RTPAVPF
...
{
  if (attributes.empty()) {
    if (!other.attributes.empty()) return false;
  }
  ...
  if (media.empty()) {
    if (!other.media.empty()) return false;
  }
  ...
  return version == other.version && origin == other.origin
         && sessionName == other.sessionName && uri == other.uri
         && conn == other.conn;
}
                          ...
                          SdpMedia&                answer) const
{
  ...
  for (; it != payloads.end(); ++it) {
    ...
    if (a_pl) {
      answer.payloads.push_back(SdpPayload(a_pl->payload_id, a_pl->name,
                                           a_pl->advertised_sample_rate, 0));
    }
    else {
      ...
      int int_pt = payload_prov->getDynPayload(
...
{
  if (!s)
    return true; // SDP can't be empty, return error (true really used for
                 // failure?)
...
static char* parse_sdp_connection(AmSdp* sdp_msg, char* s, char t)
{
  ...
  while (parsing) {
    switch (state) {
      ...
      case ADDR_TYPE: {
        ...
        std::transform(addr_type_uc.begin(), addr_type_uc.end(),
                       addr_type_uc.begin(), toupper);
        connection_line += 4;
        if (addr_type_uc == "IP4") {
          c.addrType = AT_V4;
          state      = IP4;
  ...
  // DBG("parse_sdp_line_ex: parse_sdp_connection: done parsing sdp
  // connection\n");
  return next_line;
}
    ...
    switch (state) {
      ...
      case FMT: {
        if (m.transport == TP_RTPAVP || m.transport == TP_RTPAVPF
            || m.transport == TP_RTPSAVP || m.transport == TP_RTPSAVPF
...
static void parse_session_attr(AmSdp* sdp_msg, char* s, char** next)
{
  ...
  *next           = skip_till_next_line(s, line_len);
  ...
  while (attr_end >= s && ((*attr_end == LF) || (*attr_end == CR))) attr_end--;
  ...
  if (col == attr_end) {
    ...
    sdp_msg->attributes.push_back(
        SdpAttribute(string(s, col - s - 1), string(col, attr_end - col + 1)));
    // DBG("got session attribute '%.*s:%.*s'\n", (int)(col-s-1), s,
    // (int)(attr_end-col+1), col);
  }
}
...
static char* parse_sdp_attr(AmSdp* sdp_msg, char* s)
{
  ...
  if (attr == "rtpmap") {
    ...
    if (pl_it != media.payloads.end()) {
      *pl_it = SdpPayload(int(payload_type), encoding_name, int(clock_rate),
                          int(encoding_param));
    }
  }
  else if (attr == "fmtp") {
    while (parsing) {
      switch (fmtp_st) { // fixme
        ...
        case FORMAT_PARAM: {
          ...
          while (is_wsp(*param_end)) param_end--;
    ...
    if (pl_it != media.payloads.end()) pl_it->sdp_format_parameters = params;
  }
...
static TransProt transport_type(string transport)
{
  ...
  std::transform(transport_uc.begin(), transport_uc.end(), transport_uc.begin(),
                 toupper);

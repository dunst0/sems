...
{
  if (reliable_1xx == REL100_IGNORED) return 1;
  ...
  if (100 < reply.code && reply.code < 200
    ...
    switch (reliable_1xx) {
      ...
      case REL100_REQUIRE:
        if (!key_in_list(getHeader(reply.hdrs, SIP_HDR_REQUIRE), SIP_EXT_100REL)
            || !reply.rseq) {
          ERROR("'" SIP_EXT_100REL "' not supported or no positive RSeq value "
                "in (reliable) 1xx.\n");
          dlg->bye();
          if (hdl) hdl->onFailure();
        }
    ...
    if (300 <= reply.code) {
      ...
      if (hdl) ((AmSipDialogEventHandler*) hdl)->onPrack2xx(reply);
    }
    else {
...
{
  if (reliable_1xx == REL100_IGNORED) return;

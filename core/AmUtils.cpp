...
string int2hex(unsigned int val, bool lowercase)
{
  ...
  for (i = 0; i < int(2 * sizeof(int)); i++) {
    ...
    val         = val << 4;
    buffer[j++] = lowercase ? _int2hex_lookup_l[(unsigned char) digit]
                            : _int2hex_lookup[(unsigned char) digit];
  }
...
bool reverse_hex2int(const string& str, unsigned int& result)
{
  ...
  for (string::const_reverse_iterator pc = str.rbegin(); pc != str.rend();
       ++pc) {
    result <<= 4;
    mychar = *pc;
                            ...
                            sockaddr_storage* local)
{
  ...
  if (connect(temp_sock, (sockaddr*) remote_ip,
              remote_ip->ss_family == AF_INET ? sizeof(sockaddr_in)
                                              : sizeof(sockaddr_in6))
      == -1) {
...
int get_local_addr_for_dest(const string& remote_ip, string& local)
{
  ...
  int err = inet_pton(AF_INET, remote_ip.c_str(),
                      &((sockaddr_in*) &remote_ip_ss)->sin_addr);
  if (err == 1) {
    remote_ip_ss.ss_family = AF_INET;
  }
  else if (err == 0) {
    err = inet_pton(AF_INET6, remote_ip.c_str(),
                    &((sockaddr_in6*) &remote_ip_ss)->sin6_addr);
    if (err == 1) {
  ...
  if (err == 0) {
    ...
    dns_handle dh;
    err = resolver::instance()->resolve_name(remote_ip.c_str(), &dh,
                                             &remote_ip_ss, IPv4);
  }
...
#endif
    ((sockaddr_in6*) &remote_ip_ss)->sin6_port =
        htons(5060); // fake port number
  }
...
string extract_tag(const string& addr)
{
  ...
  while (p_end < addr.length()) {
    if (addr[p_end] == '>' || addr[p_end] == ';') break;
    p_end++;
  }
  ...
  while (pos < s_list.length()) {
    ...
    while ((pos2 > 0)
           && ((s_list[pos2] == ' ') || (s_list[pos2] == list_delim)
               || (s_list[pos2] == '\n')))
      pos2--;
    if (s_list.substr(pos, pos2 - pos + 1) == key) return true;
    if (pos_n == string::npos) return false;
    while ((pos_n < s_list.length())
           && ((s_list[pos_n] == ' ') || (s_list[pos_n] == list_delim)
               || (s_list[pos_n] == '\n')))
      pos_n++;
    if (pos_n == s_list.length()) return false;
    pos = pos_n;
  }
  return false;
}
...
string strip_header_params(const string& hdr_string)
{
  ...
  for (; (val_begin < hdr_string.length()) && hdr_string[val_begin] == ' ';
       val_begin++)
    ;
  // strip parameters
...
#define ST_VAL_ESC 7
  ...
  size_t p = 0, k_begin = 0, corr = 0, v_begin = 0, v_end = 0;
  ...
  if (!v_end && (st == ST_VALUE || st == ST_VAL_ESC)) v_end = p;

  if (v_begin && v_end) {
    if ((v_end - v_begin > 1) && (param_hdr[v_begin] == param_hdr[v_end - 1])
        && ((param_hdr[v_begin] == '\'') || (param_hdr[v_begin] == '"')))
      return param_hdr.substr(v_begin + 1,
                              v_end - v_begin - 2); // whole value quoted
...
size_t skip_to_end_of_brackets(const string& s, size_t start)
{
  ...
  char   last_c       = ' ';
  int    num_brackets = 0;
  while (res < s.size() && (s[res] != ')' || num_brackets || last_c == '\\')) {
    if (last_c != '\\') {
                        ...
                        const char* dbg_type, RegexMappingVector& result)
{
  ...
  if (!appcfg.good()) {
    ...
    while (!appcfg.eof()) {
      ...
      if (non_wsp_pos != string::npos && entry[non_wsp_pos] == '#') continue;
  ...
  for (RegexMappingVector::const_iterator it = mapping.begin();
    ...
    if (!regexec(&it->first, test_s, MAX_GROUPS, groups, 0)) {
      ...
      for (g = 1; g < MAX_GROUPS; g++) {
        ...
        DBG("group %u: [%2u-%2u]: %.*s\n", g, (unsigned int) groups[g].rm_so,
            (unsigned int) groups[g].rm_eo,
            (int) (groups[g].rm_eo - groups[g].rm_so),
            test_s + groups[g].rm_so);

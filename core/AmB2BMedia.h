...
class AudioStreamData
{
 ...
 public:
  ...
  {
    if (dtmf_queue) dtmf_queue->processEvents();
  }
  ...
  {
    if (stream) stream->getSdpOffer(media_idx, m);
  }
  ...
  {
    if (stream) stream->setLogger(logger);
  }

  void debug();
};

/** \brief Class for control over media relaying and transcoding in a B2B
 * session.
 *
 ...
 *
 *  - Is non-transparent SSRC & seq. no needed if some payloads can be
 * transcoded and some relayed? Couldn't be confusing to have transparent ones
 * for relayed but our own SSRC & seq. no for transcoded payloads? [wireshark
 * seems to be confused] => disable transparent SSRC/seq.no if there are
 * payloads for transcoding?
 *
 *    Note that forcing our own SSRC can break things if the incomming RTP
 * stream comes from a source mixing audio from different sources - in that case
 * we should prefer to propagate SSRC (i.e. use transparent SSRC)!
 *
 *  - we should use our seq. numbers if transcoding is possible but propagate
 *    lost packets (i.e. remember the difference between received seq. numbers
 * and sent ones and for the transcoding purpose use seq. number = max. already
 *    used number + 1)
 *
 *  - configurable playout buffer type (from a test with transcoding PCMA ->
 * PCMU between SPA 942 and 941 it seems that at simulated 20% packet loss is
 * the audio quality better with ADAPTIVE_PLAYOUT in comparison with
 * SIMPLE_PLAYOUT but can't say it is really big differece)
 *
...
class AmB2BMedia : public AmMediaSession
{
 private:
  ...
  struct AudioStreamPair
  {
    AudioStreamData a, b;
    int             media_idx;
    AudioStreamPair(AmB2BSession* _a, AmB2BSession* _b, int _media_idx)
        : a(_a)
        , b(_b)
 ...
 public:
  ...
  // void updateRelayPayloads(bool a_leg, const AmSdp &local_sdp, const AmSdp
  // &remote_sdp);

  /**
   * Adds a reference.
   *
   * Both AmB2BSessions and AmMediaProcessor uses refcnt to this class;
   * B2BSession in case of RTP relay, AmMediaProcessor in case of local media
   * processing.
   *
  ...
  void updateStreams(bool a_leg, const AmSdp& local_sdp,
  ...
  void createHoldAnswer(bool a_leg, const AmSdp& offer, AmSdp& answer,
                        bool use_zero_con);

                 ...
                 const string& compact_hdr_name, bool single)
{
  ...
  if (!res.length()) return getHeader(hdrs, compact_hdr_name, single);
  return res;
}
                ...
                size_t& pos1, size_t& pos2, size_t& hdr_start)
{
  ...
  while (hdrs_c != hdrs_end) {
    ...
    if (hdr_c == hdr_end) {
      ...
      while (*srccol == ' ' || *srccol == '\t') srccol++;
      if (*srccol == ':') break; // was end of current hdr
  ...
  if (hdr_c == hdr_end) {
    ...
    while ((hdrs_c != hdrs_end) && (*hdrs_c == ' ')) hdrs_c++;
    ...
    if ((hdrs_c != hdrs_end) && (*hdrs_c == ':')) {
      ...
      while (p_end < hdrs.size() && hdrs[p_end] != '\r' && hdrs[p_end] != '\n')
        p_end++;
      free(hdr);
...
bool removeHeader(string& hdrs, const string& hdr_name)
{
  ...
  while (findHeader(hdrs, hdr_name, 0, pos1, pos2, hdr_start)) {
    while (pos2 < hdrs.length() && (hdrs[pos2] == '\r' || hdrs[pos2] == '\n'))
      pos2++;
...
void removeOptionTag(string& hdrs, const string& hdr_name, const string& tag)
{
  ...
  for (vector<string>::iterator it = options_v.begin(); it != options_v.end();
    ...
    if (it != options_v.begin()) o_hdr = ", ";
    o_hdr += *it;
  }
  if (!found) return;

  removeHeader(hdrs, hdr_name);
  if (o_hdr.empty()) return;
...
#define _PM(member, name)                                                      \
  do {                                                                         \
    if (!member.empty()) buf += string(name) + ":" + member + ";";             \
  } while (0)
...
void AmSipRequest::log(msg_logger* logger) const
{
  ...
  if (t) {
    sip_msg* msg = t->msg;
    logger->log(msg->buf, msg->len, &msg->remote_ip, &msg->local_ip,

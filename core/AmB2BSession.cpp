...
static void errCode2RelayedReply(AmSipReply& reply, int err_code,
                                 unsigned int default_code = 500)
{
...
void AmB2BSession::clear_other() { setOtherId(""); }
...
void AmB2BSession::onB2BEvent(B2BEvent* ev)
{
  ...
  switch (ev->event_id) {
    case B2BSipRequest: {
      ...
      if (req_ev->forward) {
        ...
        if (req_ev->req.max_forwards == 0) {
          relayError(req_ev->req.method, req_ev->req.cseq, true, 483,
                     SIP_REPLY_TOO_MANY_HOPS);
          return;
        }
        ...
        if (req_ev->req.method == SIP_METH_INVITE
          ...
          relayError(req_ev->req.method, req_ev->req.cseq, true, 491,
                     SIP_REPLY_PENDING);
          return;
        }
    ...
    case B2BSipReply: {
      ...
      if (reply_ev->forward) {
        map<int, AmSipRequest>::iterator t_req =
        ...
        if (t_req != recvd_req.end()) {
          if ((reply_ev->reply.code >= 300) && (reply_ev->reply.code <= 305)
            ...
            n_reply.hdrs +=
                SIP_HDR_COLSP(SIP_HDR_CONTACT) + reply_ev->reply.contact + CRLF;
...
void AmB2BSession::onSipRequest(const AmSipRequest& req)
{
  ...
  if (((req.method == SIP_METH_SUBSCRIBE) || (req.method == SIP_METH_NOTIFY)
...
void AmB2BSession::updateLocalSdp(AmSdp& sdp)
{
  ...
  if (!media_session) {
    // report missing media session (here we get for rtp_relay_mode ==
    // RTP_Relay)
...
void AmB2BSession::updateUACTransCSeq(unsigned int old_cseq,
                                      unsigned int new_cseq)
{
  if (old_cseq == new_cseq) return;
                              ...
                              AmBasicSipDialog::Status old_dlg_status)
{
  ...
  if (!dlg->getRemoteTag().empty() && dlg->getRemoteTag() != reply.to_tag) {
    DBG("sess %p received %i reply with != to-tag: %s (remote-tag:%s)", this,
        reply.code, reply.to_tag.c_str(), dlg->getRemoteTag().c_str());
    return; // drop packet
  }
  ...
  if (fwd) {
    ...
    relayEvent(
        new B2BSipReplyEvent(n_reply, true, t->second.method, getLocalTag()));
    ...
    if (reply.code >= 200) {
      if ((reply.code < 300) && (t->second.method == SIP_METH_INVITE)) {
        ...
        if ((reply.code < 300) && (reply.cseq_method == SIP_METH_REFER)) {
          if (subs->subscriptionExists(SingleSubscription::Subscriber, "refer",
                                       int2str(reply.cseq))) {
    ...
    DBG("relaying B2B SIP reply %u %s\n", n_reply.code, n_reply.reason.c_str());
    relayEvent(
        new B2BSipReplyEvent(n_reply, false, reply.cseq_method, getLocalTag()));
  }
}
...
int AmB2BSession::onSdpCompleted(const AmSdp& local_sdp,
...
{
  if (!other_id.empty()) relayEvent(new B2BEvent(B2BTerminateLeg));
}
...
bool AmB2BSession::refresh(int flags)
{
  ...
  if (dlg->getStatus() != AmSipDialog::Connected) return false;

  DBG(" AmB2BSession::refresh: refreshing session\n");
  // not in B2B mode      UPDATE as refresh handled like normal session
  if (other_id.empty() || refresh_method == REFRESH_UPDATE) {
...
int AmB2BSession::relaySip(const AmSipRequest& req)
{
  ...
  if (req.method != "ACK") {
    ...
    if (req.method == SIP_METH_PRACK && req.rseq) {
      ...
      for (t = relayed_req.begin(); t != relayed_req.end(); t++) {
        if (t->second.cseq == req.rack_cseq) {
          m_hdrs = req.hdrs + SIP_HDR_COLSP(SIP_HDR_RACK) + int2str(req.rseq)
    ...
    while (t != relayed_req.end()) {
      if (t->second.cseq == req.cseq) break;
      t++;
    }
    if (t == relayed_req.end()) {
...
int AmB2BSession::relaySip(const AmSipRequest& orig, const AmSipReply& reply)
{
  ...
  if (reply.rseq != 0) {
    m_hdrs =
        reply.hdrs + SIP_HDR_COLSP(SIP_HDR_RSEQ) + int2str(reply.rseq) + CRLF;
    hdrs = &m_hdrs;
  }

  AmMimeBody body(reply.body);
  if ((orig.method == SIP_METH_INVITE || orig.method == SIP_METH_UPDATE
  ...
  if (reply.to_tag.empty()) flags |= SIP_FLAGS_NOTAG;
...
void AmB2BSession::computeRelayMask(const SdpMedia& m, bool& enable,
                                    PayloadMask& mask)
{
  int te_pl = -1;
  enable    = false;
  ...
  for (vector<SdpPayload>::const_iterator i = m.payloads.begin();
       i != m.payloads.end(); i++) {
...
void AmB2BCallerSession::terminateLeg() { AmB2BSession::terminateLeg(); }
...
void AmB2BCallerSession::onB2BEvent(B2BEvent* ev)
{
  ...
  if (ev->event_id == B2BSipReply) {
    ...
    if (getOtherId().empty()) {
      ...
      DBG("B2BSipReply: other_id empty ("
          ...
          "to_tag=%s; cseq=%i)\n",
          reply.code, reply.cseq_method.c_str(), reply.callid.c_str(),
          reply.from_tag.c_str(), reply.to_tag.c_str(), reply.cseq);
      // return;
    }
  ...
  if (!processed) AmB2BSession::onB2BEvent(ev);
}
                                       ...
                                       bool          relayed_invite)
{
  ...
  if (relayed_invite) {
    ...
    recvd_req.insert(make_pair(invite_req.cseq, invite_req));
...
{
  return dlg->sendRequest(SIP_METH_INVITE, &callee_reply.body, "" /* hdrs */,
                          SIP_FLAGS_VERBATIM);
}
...
void AmB2BCallerSession::createCalleeSession()
{
  ...
  MONITORING_LOG4(getOtherId().c_str(), "dir", "out", "from",
...
void AmB2BCallerSession::initializeRTPRelay(AmB2BCalleeSession* callee_session)
{
  ...
  if ((rtp_relay_mode == RTP_Relay) || (rtp_relay_mode == RTP_Transcoding)) {
    // we need to add our reference
    setMediaSession(new AmB2BMedia(this, callee_session));
...
void AmB2BCalleeSession::onB2BEvent(B2BEvent* ev)
{
  if (ev->event_id == B2BConnectLeg) {
    ...
    MONITORING_LOG3(getLocalTag().c_str(), "b2b_leg", getOtherId().c_str(),
                    "to", co_ev->remote_party.c_str(), "ruri",
                    co_ev->remote_uri.c_str());
    ...
    int res = dlg->sendRequest(SIP_METH_INVITE, &body, co_ev->hdrs,
                               SIP_FLAGS_VERBATIM);
    if (res < 0) {

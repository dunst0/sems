...
sip_via_parm::~sip_via_parm() { free_gen_params(&params); }
    ...
    switch (st) {
      case TR_BEG:
        switch (**c) {
          ...
          case 't':
          case 'T': st = TR_T; break;

          case 's':
          case 'S':
            if ((len >= 4) && ((*(*c + 1) == 'c') || (*(*c + 1) == 'C'))
                && ((*(*c + 2) == 't') || (*(*c + 2) == 'T'))
                && ((*(*c + 3) == 'p') || (*(*c + 3) == 'P'))) {
      ...
      case TR_SCTP:
        switch (**c) {
          ...
          case CR:
          case LF:
          case SP: t->val.len = *c - t->val.s; return 0;
...
end_by:
  switch (st) {
    case BY_HOST: v->host.set(beg, *c - beg); break;
    case BY_PORT: v->port.set(beg, *c - beg); break;
    case ST_LF:
    case ST_CRLF:
      switch (saved_st) {
        case BY_PORT:
    ...
    default: DBG("Unexpected end state: st = %i\n", st); return UNDEFINED_ERR;
  }
  ...
  for (; it != parm->params.end(); it++) {
    ...
    for (; c != end; c++) {
      switch (st) {
        ...
        case VP_R:
          ...
          switch (*c) {
            case 'e': // "re..."
            case 'E': st = VP_RECVD1; break;

            case 'p': // "rp..."
            case 'P': st = VP_RPORT1; break;
    ...
    switch (st) {
      case VP_BRANCH:
        ...
        break;
      case VP_RECVD: parm->recved = (*it)->value; break;
      case VP_RPORT:
        parm->has_rport = true;
        parm->rport     = (*it)->value;
        if (parm->rport.len) {
...
int parse_via(sip_via* via, const char* beg, int len)
{
  ...
  for (; c < end; c++) {
    switch (st) {
      ...
      case ST_LF:
      case ST_CRLF: st = saved_st; break;
    }
  }

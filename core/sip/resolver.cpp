...
int dns_ip_entry::next_ip(dns_handle* h, sockaddr_storage* sa)
{
  ...
  int& index = h->ip_n;
  if ((index < 0) || (index >= (int) ip_vec.size())) return -1;
...
{
  switch (t) {
    case dns_r_srv: return new dns_srv_entry();
    case dns_r_a:
      // case dns_r_aaaa:
      return new dns_ip_entry();
    case dns_r_naptr: return new dns_naptr_entry();
    default: return NULL;
  }
}
...
{
  switch (error) {
    case HOST_NOT_FOUND: DBG("Unknown domain: %s\n", domain); break;
    case NO_DATA: DBG("No records for %s\n", domain); break;
    case TRY_AGAIN: DBG("No response for query (try again)\n"); break;
...
string ip_entry::to_str()
{
  if (type == IPv4) {
    ...
    return int2str(cp[0]) + "." + int2str(cp[1]) + "." + int2str(cp[2]) + "."
           + int2str(cp[3]);
  }
  else {
    // not supported yet...
...
{
  if (rr->type != dns_r_a) return NULL;

  DBG("A:\tTTL=%i\t%s\t%i.%i.%i.%i\n", ns_rr_ttl(*rr), ns_rr_name(*rr),
      ns_rr_rdata(*rr)[0], ns_rr_rdata(*rr)[1], ns_rr_rdata(*rr)[2],
      ns_rr_rdata(*rr)[3]);
...
dns_base_entry* dns_srv_entry::get_rr(dns_record* rr, u_char* begin,
                                      u_char* end)
{
  if (rr->type != dns_r_srv) return NULL;
  ...
  if (dns_expand_name(&p, begin, end, name_buf, NS_MAXDNAME) < 0) {
    ERROR("dns_expand_name failed\n");
    return NULL;
  }

  DBG("SRV:\tTTL=%i\t%s\tP=<%i> W=<%i> P=<%i> T=<%s>\n", ns_rr_ttl(*rr),
      ns_rr_name(*rr), dns_get_16(rdata), dns_get_16(rdata + 2),
      dns_get_16(rdata + 4), name_buf);
...
{
  return target + ":" + int2str(port) + "/" + int2str(p) + "/" + int2str(w);
};
...
int rr_to_dns_entry(dns_record* rr, dns_section_type t, u_char* begin,
                    u_char* end, void* data)
{
  // only answer and additional sections
  if (t != dns_s_an && t != dns_s_ar) return 0;
...
dns_handle::dns_handle(const dns_handle& h) { *this = h; }

dns_handle::~dns_handle()
{
  if (ip_e) dec_ref(ip_e);

  if (srv_e) dec_ref(srv_e);
}

bool dns_handle::valid() { return (ip_e); }
...
dns_base_entry* dns_naptr_entry::get_rr(dns_record* rr, u_char* begin,
                                        u_char* end)
{
  ...
  if (rr->type != dns_r_naptr) return NULL;
  ...
  printf(
      "ENUM: TTL=%i P=<%i> W=<%i> FL=<%.*s> S=<%.*s> REG=<%.*s> REPL=<%.*s>\n",
      ns_rr_ttl(*rr), order, pref, fields[NAPTR_Flags].len,
      fields[NAPTR_Flags].s, fields[NAPTR_Services].len,
      fields[NAPTR_Services].s, fields[NAPTR_Regexp].len,
      fields[NAPTR_Regexp].s, fields[NAPTR_Replacement].len,
...
sip_target::sip_target(const sip_target& target) { *this = target; }
...
void sip_target_set::reset_iterator() { dest_list_it = dest_list.begin(); }

bool sip_target_set::has_next() { return dest_list_it != dest_list.end(); }
                             ...
                             unsigned int flags)
{
  do {
    ...
    if (!next_trsp.len) next_trsp = cstring("udp");
...
void sip_target_set::debug()
{
  ...
  for (list<sip_target>::iterator it = dest_list.begin(); it != dest_list.end();
       it++) {
    DBG("\t%s:%u/%s to target list", am_inet_ntop(&it->ss).c_str(),
...
{
  std::pair<iterator, bool> res = insert(make_pair(key, e));
...
int _resolver::query_dns(const char* name, dns_entry_map& entry_map,
                         dns_rr_type t)
{
  ...
  int dns_res_len =
      res_search(name, ns_c_in, (ns_type) t, dns_res, NS_PACKETSZ);
...
int _resolver::resolve_name(const char* name, dns_handle* h,
                            sockaddr_storage* sa, const address_type types,
                            dns_rr_type t)
{
  int ret;
  ...
  for (dns_entry_map::iterator it = entry_map.begin(); it != entry_map.end();
       it++) {
    if (!it->second) continue;

    b = cache.get_bucket(hashlittle(it->first.c_str(), it->first.length(), 0));
    // cache the new record
    if (b->insert(it->first, it->second)) {
      // cache insert successful
      DBG("new DNS cache entry: '%s' -> %s", it->first.c_str(),
...
int _resolver::str2ip(const char* name, sockaddr_storage* sa,
                      const address_type types)
{
  if (types & IPv4) {
    ...
    if (ret == 1) {
      ...
      ERROR("while trying to detect an IPv4 address '%s': %s", name,
            strerror(errno));
      return ret;
    }
  }
  ...
  if (types & IPv6) {
    ...
    if (ret == 1) {
      ...
      ERROR("while trying to detect an IPv6 address '%s': %s", name,
            strerror(errno));
      return ret;
    }
  }
                                  ...
                                  dns_handle*       h_dns)
{
  ...
  if (am_inet_pton(nh.c_str(), remote_ip) != 1) {
    ...
    if (!next_port) {
      ...
      if (disable_srv) {
        ...
        if (!resolver::instance()->resolve_name(srv_name.c_str(), h_dns,
                                                remote_ip, IPv4, dns_r_srv)) {
          return 0;
        }
  ...
  no_SRV:
    ...
    int err =
        resolver::instance()->resolve_name(nh.c_str(), h_dns, remote_ip, IPv4);
                               ...
                               sip_target_set*              targets)
{
  for (list<sip_destination>::const_iterator it = dest_list.begin();
    ...
    DBG("sip_destination: %.*s:%u/%.*s", it->host.len, it->host.s, it->port,

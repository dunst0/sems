...
int udp_trsp_socket::bind(const string& bind_ip, unsigned short int bind_port)
{
  ...
  if (addr.ss_family == AF_INET) {
    if (setsockopt(sd, IPPROTO_IP, DSTADDR_SOCKOPT, (void*) &true_opt,
                   sizeof(true_opt))
        == -1) {
      ERROR("%s\n", strerror(errno));
      close(sd);
    ...
    if (setsockopt(sd, IPPROTO_IPV6, DSTADDR6_SOCKOPT, (void*) &true_opt,
                   sizeof(true_opt))
        == -1) {
      ERROR("%s\n", strerror(errno));
      close(sd);
...
int udp_trsp_socket::set_recvbuf_size(int rcvbuf_size)
{
  if (rcvbuf_size > 0) {
    ...
    if (setsockopt(sd, SOL_SOCKET, SO_RCVBUF, (void*) &rcvbuf_size, sizeof(int))
        == -1) {
      ...
      if (getsockopt(sd, SOL_SOCKET, SO_RCVBUF, &set_rcvbuf_size, &optlen)
          == -1) {
...
int udp_trsp_socket::sendto(const sockaddr_storage* sa, const char* msg,
                            const int msg_len)
{
  int err =
      ::sendto(sd, msg, msg_len, 0, (const struct sockaddr*) sa, SA_len(sa));

  if (err < 0) {
    char host[NI_MAXHOST] = "";
    ERROR("sendto(%i;%s:%i): %s\n", sd, am_inet_ntop_sip(sa, host, NI_MAXHOST),
...
int udp_trsp_socket::sendmsg(const sockaddr_storage* sa, const char* msg,
                             const int msg_len)
{
  struct msghdr   hdr;
  struct cmsghdr* cmsg;
...
int udp_trsp_socket::send(const sockaddr_storage* sa, const char* msg,
                          const int msg_len, unsigned int flags)
{
  if (log_level_raw_msgs >= 0) {
    _LOG(log_level_raw_msgs, "send  msg to %s:%i\n--++--\n%.*s--++--\n",
         get_addr_str(sa).c_str(), ntohs(((sockaddr_in*) sa)->sin_port),
         msg_len, msg);
  }
  ...
  while (isRunning()) {
    ...
    if (trsp_socket::log_level_raw_msgs >= 0) {
      ...
      _LOG(trsp_socket::log_level_raw_msgs,
           ...
           am_get_port(&s_msg->remote_ip), s_msg->len, s_msg->buf);
    }
    ...
    for (cmsgptr = CMSG_FIRSTHDR(&msg); cmsgptr != NULL;
      ...
      if (cmsgptr->cmsg_level == IPPROTO_IP
        ...
        memcpy(&((sockaddr_in*) (&s_msg->local_ip))->sin_addr, dstaddr(cmsgptr),
               sizeof(in_addr));
      }

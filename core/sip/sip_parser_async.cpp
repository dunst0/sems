...
static int parse_header_async(sip_header* hdr, parser_state* pst, char* end)
{
  ...
  switch (st) {
    case H_NAME:
    case H_VALUE: return UNEXPECTED_EOT;
...
int skip_sip_msg_async(parser_state* pst, char* end)
{
  ...
  while (c < end) {
    switch (stage) {
      case ST_FL: err = skip_line_async(pst, end); break;

      case ST_HDRS: err = parse_headers_async(pst, end); break;

      case ST_BODY:
        if (!pst->content_len) return 0;
      ...
      default: ERROR("unkown state!!!"); return -1;
    }

...
class trans_stats
{
 ...
 public:
  ...
  unsigned int get_sent_requests() const { return sent_requests.get(); }
  unsigned int get_sent_replies() const { return sent_replies.get(); }
  unsigned int get_received_requests() const { return received_requests.get(); }
  unsigned int get_received_replies() const { return received_replies.get(); }
  unsigned int get_sent_request_retrans() const
  {
    return sent_request_retrans.get();
  }
  unsigned int get_sent_reply_retrans() const
  {
    return sent_reply_retrans.get();
  }
};
...
class _trans_layer
{
 ...
 public:
   ...
   */
  int cancel(trans_ticket* tt, const cstring& dialog_id, unsigned int inv_cseq,
             const cstring& hdrs);
   ...
   * 'Content-Length' header.
   */
  int send_sl_reply(sip_msg* req, int reply_code, const cstring& reason,

...
static int parse_sip_uri(sip_uri* uri, const char* beg, int len)
{
  ...
  for (; c != beg + len; c++) {
    switch (*c) {
      ...
      case '&':
        switch (st) {
          case URI_HNAME: DBG("Empty URI header\n"); return MALFORMED_URI;
  ...
  switch (st) {
    case URI_USER:
    case URI_PW: DBG("Missing host part\n"); return MALFORMED_URI;
    ...
    switch (st) {
      case URI_BEG:
        switch (*c) {
          ...
          default: DBG("Unknown URI scheme\n"); return MALFORMED_URI;
        }
        break;
      case SIP_S:
        switch (*c) {

                                       ...
                                       struct event_base*      evbase)
{
  ...
  tcp_trsp_socket* sock =
      new tcp_trsp_socket(server_sock, server_worker, sd, sa, evbase);
...
tcp_trsp_socket* tcp_trsp_socket::new_connection(
    tcp_server_socket* server_sock, tcp_server_worker* server_worker,
...
void tcp_trsp_socket::create_events()
{
  read_ev = event_new(evbase, sd, EV_READ | EV_PERSIST,
                      tcp_trsp_socket::on_sock_read, (void*) this);

  write_ev = event_new(evbase, sd, EV_WRITE, tcp_trsp_socket::on_sock_write,
                       (void*) this);
}
...
int tcp_trsp_socket::on_connect(short ev)
{
  ...
  if (error != 0) {
    DBG("*********** connection error (sd=%i): %s *********", sd,
        strerror(error));
    close();
    return -1;
  }
...
int tcp_trsp_socket::connect()
{
  ...
  DBG("connecting to %s:%i...", am_inet_ntop(&peer_addr).c_str(),
                          ...
                          const int msg_len, unsigned int flags)
{
  ...
  if (closed || (check_connection() < 0)) return -1;
    ...
    if (bytes < 0) {
      ...
      switch (errno) {
        ...
        default: // unforseen error: close connection
          ERROR("unforseen error: close connection (%i/%s)", errno,
                strerror(errno));
          close();
          break;
      }
...
void tcp_server_worker::add_connection(tcp_trsp_socket* client_sock)
{
  ...
  DBG("new TCP connection from %s:%u", client_sock->get_peer_ip().c_str(),
                            ...
                            const int msg_len, unsigned int flags)
{
  ...
  if (sock_it != connections.end()) {
    ...
    tcp_trsp_socket* new_sock =
        tcp_trsp_socket::new_connection(server_sock, this, sa, evbase);
...
void tcp_server_worker::run()
{
  ...
  pipe(fake_fds);
  ev_default = event_new(evbase, fake_fds[0], EV_READ | EV_PERSIST, NULL, NULL);
...
int tcp_server_socket::bind(const string& bind_ip, unsigned short bind_port)
{
  ...
  int true_opt = 1;
  if (setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, (void*) &true_opt,
                 sizeof(true_opt))
      == -1) {
    ERROR("%s\n", strerror(errno));
    close(sd);
...
{
  if (idle_timeout.tv_sec || idle_timeout.tv_usec) return &idle_timeout;
...
void tcp_trsp::run()
{
  ...
  INFO("TCP SIP server on %s:%i finished (%i)", sock->get_ip(),
       sock->get_port(), ret);
}

...
sip_header::~sip_header() { delete p; }
...
int parse_header_type(sip_header* h)
{
      ...
      switch (h->name.s[0]) {
        ...
        case 'C':
          switch (h->name.s[1]) {
            case 'a':
            case 'A':
              if (!lower_cmp(h->name.s + 2, SIP_HDR_CALL_ID + 2,
                             CALL_ID_len - 2)) {
                h->type = sip_header::H_CALL_ID;
              }
              break;
            ...
            case 'o':
            case 'O':
              if (!lower_cmp(h->name.s + 2, SIP_HDR_CONTACT + 2,
                             CONTACT_len - 2)) {
                h->type = sip_header::H_CONTACT;
              }
              break;
  ...
  if (h->type == sip_header::H_UNPARSED) h->type = sip_header::H_OTHER;

 ...
 */
inline static int mk_udp_hdr(struct udphdr* u, const sockaddr_storage* from,
                             const sockaddr_storage* to, unsigned char* buf,
                             int len, int do_chk)
{
  ...
  if (do_chk) {
    u->uh_sum =
        htons(udpv4_chksum(u, &from_v4->sin_addr, &to_v4->sin_addr, buf, len));
  }
  else {
                        ...
                        const sockaddr_storage* to, unsigned short mtu)
{
  ...
  if (unlikely(totlen) > 65535) return -2;
  memset(&snd_msg, 0, sizeof(snd_msg));
  snd_msg.msg_name    = SAv4(to);
  snd_msg.msg_namelen = SA_len(to);
  snd_msg.msg_iov     = &iov[0];

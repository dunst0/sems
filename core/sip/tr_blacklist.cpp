...
bl_addr::bl_addr() { ss_family = AF_INET; }

bl_addr::bl_addr(const bl_addr& addr) { memcpy(this, &addr, SA_len(&addr)); }
...
{
  if (l.ss_family != r.ss_family) return l.ss_family < r.ss_family;
...
void bl_timer::fire()
{
  DBG_BL("blacklist: %s/%i expired", am_inet_ntop(&addr).c_str(),
         am_get_port(&addr));
  tr_blacklist::instance()->remove(&addr);
}

bool blacklist_bucket::insert(const bl_addr& addr, unsigned int duration,
                              const char* reason)
{
  ...
  DBG_BL("blacklist: added %s/%i (%s/TTL=%.1fs)", am_inet_ntop(&addr).c_str(),
         am_get_port(&addr), reason, (float) duration / 1000.0);
...
{
  bool              res;
  blacklist_bucket* bucket =
      get_bucket(hashlittle(addr, SA_len(addr), 0) & BLACKLIST_HT_MASK);
  bucket->lock();

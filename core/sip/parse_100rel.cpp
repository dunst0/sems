...
#define READ_NUMBER(_no, _c_, _end_)                                           \
  do {                                                                         \
    ...
    for (fin = false; !fin && (_c_) < (_end_);) {                              \
      switch (*(_c_)) {                                                        \
        case '0' ... '9':                                                      \
          if (UINT_MAX - (_no) < (unsigned int) *(_c_)) {                      \
            INFO("not an uint32_t.\n");                                        \
            goto error;                                                        \
          }                                                                    \
          (_no) = (_no) *10 + *(_c_) - '0';                                    \
          break;                                                               \
        default: fin = true; continue;                                         \
      }                                                                        \
      (_c_)++;                                                                 \
    }                                                                          \
  } while (0)
...
#define EAT_TOKEN(_c_, _end_)                                                  \
  do {                                                                         \
    while ((_c_) < (_end_) && IS_TOKEN(*(_c_))) (_c_)++;                       \
  } while (0)

bool parse_rseq(unsigned int* _rseq, const char* start, int len)
{
  unsigned int rseq;
...
bool parse_rack(sip_rack* rack, const char* start, int len)
{
  ...
  if (parse_method(&method, method_str.s, method_str.len)) goto error;

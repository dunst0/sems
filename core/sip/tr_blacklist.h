...
typedef ht_map_bucket<bl_addr, bl_entry, ht_delete<bl_entry>, bl_addr_less>
    bl_bucket_base;
...
class blacklist_bucket : public bl_bucket_base
{
 ...
 public:
   ...
   */
  bool insert(const bl_addr& addr, unsigned int duration, const char* reason);
...
class _tr_blacklist : protected blacklist_ht
{
 ...
 public:
   ...
   * @param[in] duration in ms
   */
  void insert(const sockaddr_storage* addr, unsigned int duration,
              const char* reason);

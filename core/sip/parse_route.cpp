...
sip_route::~sip_route()
{
  for (list<route_elmt*>::iterator it = elmts.begin(); it != elmts.end();
       it++) {
    delete *it;
  }
}
...
bool is_loose_route(const sip_uri* fr_uri)
{
  ...
  if (!fr_uri->params.empty()) {
    ...
    for (; it != fr_uri->params.end(); it++) {
      if (((*it)->name.len == 2) && (!memcmp((*it)->name.s, "lr", 2))) {
        is_lr = true;
        break;
      }
    }
...
static int skip_2_next_route(const char*& c, const char*& eor, const char* end)
{
  assert(c && end && (c <= end));
    ...
    switch (st) {
      ...
      case RR_SEP_SWS:
        switch (*c) {
          ...
          case CR:
          case LF: break;
          default: st = RR_NXT_ROUTE; goto nxt_route;
        }
        break;
    }
  }
...
error:
  ...
  switch (st) {
    case RR_QUOTED: DBG("Malformed route header\n"); return -1;

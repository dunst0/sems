...
{
  if (!public_ip.empty()) return public_ip.c_str();
...
bool trsp_socket::match_addr(sockaddr_storage* other_addr) const
{
  ...
  if (addr.ss_family == AF_INET) {
    if (!memcmp(&((sockaddr_in*) &addr)->sin_addr,
                &((sockaddr_in*) other_addr)->sin_addr, sizeof(in_addr))) {
      return true;
    }
  }
  else if (addr.ss_family == AF_INET6) {
    if (!memcmp(&((sockaddr_in6*) &addr)->sin6_addr,
                &((sockaddr_in6*) other_addr)->sin6_addr, sizeof(in6_addr))) {
      return true;
    }
  }

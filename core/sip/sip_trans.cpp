...
void sip_trans::retransmit()
{
  ...
  if (logger) {
    ...
    logger->log(retr_buf, retr_len, &src_ip, &retr_addr,
                msg->u.request->method_str, reply_status);
  }
}
...
const char* _trans_type_lookup[] = {"0", "UAS", "UAC"};

#define trans_type(type) (_trans_type_lookup[type])

const char* _state_name_lookup[] = {
    "0",         "TRYING",    "CALLING",        "PROCEEDING", "PROCEEDING_REL",
    "COMPLETED", "CONFIRMED", "TERMINATED_200", "TERMINATED", "ABANDONED",
    "REMOVED"};

#define state_name(state) (_state_name_lookup[state])
...
void sip_trans::reset_timer(trans_timer* t, unsigned int timer_type)
{
  ...
  *tp = t;
  ...
  if (t) wheeltimer::instance()->insert_timer((timer*) t);
}
 ...
 */
void sip_trans::reset_timer(unsigned int timer_type, unsigned int expire_delay,
                            unsigned int bucket_id)
{
  ...
  DBG("New timer of type %s at time=%i (repeated=%i)\n", timer_name(timer_type),
      expires, timer_type >> 16);

  trans_timer* t = new trans_timer(timer_type, expires, bucket_id, this);
...
const char* sip_trans::type_str() const { return trans_type(type); }

const char* sip_trans::state_str() const { return state_name(state); }

void sip_trans::dump() const
{
  DBG("type=%s (0x%x); msg=%p; to_tag=%.*s; reply_status=%i; state=%s (%i); "
      "retr_buf=%p; timers [%s,%s,%s]\n",
      type_str(), type, msg, to_tag.len, to_tag.s, reply_status, state_str(),
      state, retr_buf, timers[0] == NULL ? "none" : timer_name(timers[0]->type),

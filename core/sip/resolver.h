...
{
  virtual dns_base_entry* get_rr(dns_record* rr, u_char* begin,
                                 u_char* end) = 0;
...
struct ip_entry : public dns_base_entry
{
  address_type type;
...
class _resolver : AmThread
{
 public:
  ...
  int resolve_name(const char* name, dns_handle* h, sockaddr_storage* sa,
                   const address_type types, dns_rr_type t = dns_r_a);

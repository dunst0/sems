...
bool _trans_layer::less_case_i::operator()(const string& lhs,
                                           const string& rhs) const
{
  return lower_cmp_n(lhs.c_str(), lhs.length(), rhs.c_str(), rhs.length()) < 0;
}
...
void _trans_layer::register_ua(sip_ua* ua) { this->ua = ua; }

int _trans_layer::register_transport(trsp_socket* trsp)
{
  int if_num = trsp->get_if();
  if (transports.size() <= (size_t) if_num) transports.resize(if_num + 1);
...
void _trans_layer::clear_transports() { transports.clear(); }
                                   ...
                                   string& n_contact)
{
  ...
  if (parse_nameaddr_list(contact_list, contact->value.s, contact->value.len)
      < 0) {
  ...
  for (list<cstring>::iterator ct_it = contact_list.begin();
    ...
    for (list<sip_avp*>::iterator p_it = na.uri.params.begin();
      ...
      if (!lower_cmp_n((*p_it)->name.s, (*p_it)->name.len, "transport", 9)) {
        found_trsp = true;
        if (lower_cmp_n((*p_it)->value.s, (*p_it)->value.len, trsp.s,
                        trsp.len)) {
                             ...
                             msg_logger* logger)
{
  ...
  if (req->via_p1->has_rport) {
    if (!req->via_p1->rport.len) {
      remote_port_str =
          int2str(ntohs(((sockaddr_in*) &req->remote_ip)->sin_port));
  ...
  for (list<sip_header*>::iterator it = req->hdrs.begin();
    ...
    switch ((*it)->type) {
      ...
      case sip_header::H_RECORD_ROUTE: reply_len += copy_hdr_len(*it); break;
  ...
  for (list<sip_header*>::iterator it = req->hdrs.begin();
    ...
    switch ((*it)->type) {
        ...
        if ((*it) == req->via1) { // 1st Via
          ...
          *(c++) = SP;
          ...
          if (req->via_p1->has_rport && !req->via_p1->rport.len) {
            ...
            len = (req->via_p1->rport.s + req->via_p1->rport.len)
                  - req->via1->value.s;
            ...
            *(c++) = '=';
            ...
            len = req->via_p1->eop
                  - (req->via_p1->rport.s + req->via_p1->rport.len);
      ...
      case sip_header::H_CALL_ID:
      case sip_header::H_CSEQ:
      case sip_header::H_RECORD_ROUTE: copy_hdr_wr(&c, *it); break;
    }
  }
  ...
  *c++ = LF;
  ...
  if (update_uas_reply(bucket, t, reply_code) == TS_REMOVED) goto end;
...
int _trans_layer::send_sf_error_reply(const trans_ticket* tt,
                                      const sip_msg* req, int reply_code,
...
static void prepare_strict_routing(sip_msg* msg, string& ext_uri_buffer)
{
  ...
  if (!route_uri || is_loose_route(route_uri)) return;
                               ...
                               cstring*            next_trsp)
{
  ...
  if (!next_trsp->len) {
    DBG("no transport specified, setting default one (%.*s)", default_trsp.len,
        default_trsp.s);
    *next_trsp = default_trsp;
  }

  DBG("next_hop:next_port is <%.*s:%u/%.*s>\n", next_hop->len, next_hop->s,
      *next_port, next_trsp ? next_trsp->len : 0, next_trsp ? next_trsp->s : 0);

  return 0;
}

static void set_err_reply_from_req(sip_msg* err, sip_msg* req, int code,
                                   const char* reason)
{
  err->type            = SIP_REPLY;
  ...
  if (!req->from->p) {
    ...
    parse_from_to((sip_from_to*) req->from->p, req->from->value.s,
                  req->from->value.len);
  }
  err->from = req->from;
  ...
  if (!req->to->p) {
    ...
    parse_from_to((sip_from_to*) req->to->p, req->to->value.s,
                  req->to->value.len);
  }
  err->to = req->to;
  ...
  if (!req->cseq->p) {
    ...
    parse_cseq((sip_cseq*) req->cseq->p, req->cseq->value.s,
               req->cseq->value.len);
  }
  err->cseq   = req->cseq;
                                     ...
                                     int code, const char* reason)
{
  ...
  parse_from_to((sip_from_to*) reply.from->p, reply.from->value.s,
                reply.from->value.len);

  translate_hdr(&reply, reply.to, req, req->to);
  reply.to->p = new sip_from_to();
  parse_from_to((sip_from_to*) reply.to->p, reply.to->value.s,
                reply.to->value.len);

  translate_hdr(&reply, reply.cseq, req, req->cseq);
  reply.cseq->p = new sip_cseq();
  parse_cseq((sip_cseq*) reply.cseq->p, reply.cseq->value.s,
             reply.cseq->value.len);
...
{
  int request_len =
      request_line_len(msg->u.request->method_str, msg->u.request->ruri_str);
  ...
  // generate it
  char* c = p_msg->buf;
  request_line_wr(&c, msg->u.request->method_str, msg->u.request->ruri_str);
...
int _trans_layer::send_request(sip_msg* msg, trans_ticket* tt,
                               ...
                               const cstring& _next_hop, int out_interface,
                               unsigned int flags, msg_logger* logger)
{
  ...
  if (!msg->u.request->ruri_str.len || !msg->u.request->method_str.len) {
    ERROR("empty method name or R-URI");
    return -1;
  }
  else {
    DBG("send_request to R-URI <%.*s>", msg->u.request->ruri_str.len,
...
try_next_dest:
  ...
  if (set_trsp_socket(msg, next_trsp, out_interface) < 0) return -1;
  ...
  DBG("Sending to %s:%i <%.*s...>\n", get_addr_str(&p_msg->remote_ip).c_str(),
      ntohs(((sockaddr_in*) &p_msg->remote_ip)->sin_port), p_msg->len,
      p_msg->buf);

  tt->_bucket =
      get_trans_bucket(p_msg->callid->value, get_cseq(p_msg)->num_str);
  tt->_bucket->lock();
  ...
  if (err < 0) {
    ...
    if (default_bl_ttl) {
      tr_blacklist::instance()->insert(&tt->_t->msg->remote_ip, default_bl_ttl,
                                       "503");
    }
    tt->_bucket->unlock();
    goto try_next_dest;
    ...
    if (tt->_t && (method != sip_request::ACK)) {
      ...
      if (tt->_t->targets->has_next()) {
        tt->_t->reset_timer(STIMER_M, M_TIMER, tt->_bucket->get_id());
      }
    }
    ...
    if (logger) {
      ...
      logger->log(msg_buffer, msg_len, &src_ip, &msg->remote_ip, method_str);
                         ...
                         unsigned int inv_cseq, const cstring& hdrs)
{
  ...
  switch (t->state) {
    ...
    case TS_COMPLETED:
      ERROR("Trying to cancel a request while in TS_COMPLETED state; inv_cseq: "
            "%u, i:%.*s\n",
            inv_cseq, dialog_id.len, dialog_id.s);
      t->dump();
      bucket->unlock();
  ...
  int request_len = request_line_len(cancel_str, req->u.request->ruri_str);

  request_len += copy_hdr_len(req->via1);

  request_len += copy_hdr_len(req->to) + copy_hdr_len(req->from)
                 + copy_hdr_len(req->callid)
                 + cseq_len(get_cseq(req)->num_str, cancel_str)
                 + copy_hdrs_len(req->route) + copy_hdrs_len(req->contacts);
  ...
  // generate it
  char* c = p_msg->buf;
  request_line_wr(&c, cancel_str, req->u.request->ruri_str);
  ...
  if (send_err < 0) {
    ...
    if (send_err < 0) {
      ...
      if (t->logger) {
        ...
        t->logger->log(p_msg->buf, p_msg->len, &src_ip, &p_msg->remote_ip,
                       cancel_str);
...
void _trans_layer::received_msg(sip_msg* msg)
{
  ...
  if (err) {
    ...
    if ((err != MALFORMED_FLINE) && (msg->type == SIP_REQUEST)
        && (msg->u.request->method != sip_request::ACK)) {
      send_sl_reply(msg, 400, cstring(err_msg), cstring(), cstring());
    }
  ...
  switch (msg->type) {
    case SIP_REQUEST:
      ...
      if ((t = bucket->match_request(msg, TT_UAS)) != NULL) {
        if (t->logger) {
          t->logger->log(msg->buf, msg->len, &msg->remote_ip, &msg->local_ip,
        ...
        switch (msg->u.request->method) {
          ...
          case sip_request::PRACK:
            bucket->unlock();
            if (!msg->rack) {
              send_sl_reply(msg, 400, cstring("Missing valid RSeq header"),
                            cstring(), cstring());
              DROP_MSG;
            }
            ...
            if ((inv_t = inv_bucket->match_1xx_prack(msg)) != NULL) {
              assert(msg->u.request->method != inv_t->msg->u.request->method);
    ...
    case SIP_REPLY:
      ...
      if ((t = bucket->match_reply(msg)) != NULL) {
        ...
        if (t->logger && msg->local_socket && msg->buf && msg->len) {
          t->logger->log(msg->buf, msg->len, &msg->remote_ip, &msg->local_ip,
                         get_cseq(msg)->method_str, msg->u.reply->code);
        }
        ...
        DBG("reply code = %i\n", msg->u.reply->code);
        if ((msg->u.reply->code >= 200) && (msg->u.reply->code < 300)) {
          bucket->unlock();
    ...
    default: ERROR("Got unknown message type: Bug?\n"); break;
  }
...
int _trans_layer::update_uac_reply(trans_bucket* bucket, sip_trans* t,
                                   sip_msg* msg)
{
  ...
  if (t->msg->u.request->method == sip_request::INVITE) {
    if (reply_code >= 300) {
      ...
      if (reply_code == 503
          && (t->state == TS_CALLING || t->state == TS_PROCEEDING)) {
        if (!(t->flags & TR_FLAG_DISABLE_BL)) {
          tr_blacklist::instance()->insert(&t->msg->remote_ip, default_bl_ttl,
                                           "503");
        }

        if (msg->local_socket) { // remote reply
          if (!try_next_ip(bucket, t, true)) forget_reply = true;
        }
        else { // local reply
          if (!try_next_ip(bucket, t, false)) goto end;
        }
      }

      // Final error reply
      switch (t->state) {
        case TS_CALLING: t->reset_all_timers();
      ...
      switch (t->state) {
        ...
        case TS_TERMINATED_200: // subsequent 2xx reply (no ACK sent)
          ...
          if ((to_tag.len != t->to_tag.len)
            ...
            //   this dialog...
            //
            DBG("Received 200 reply with different To-tag as the previous "
                "one.\n");
            goto end;
          }
    ...
    switch (t->state) {
      ...
      case TS_PROCEEDING:
...

        {
          ...
          if (msg->local_socket && !msg->local_socket->is_reliable())
                                     ...
                                     sip_msg* msg)
{
  ...
  DBG("transport = '%s'; is_reliable = %i", msg->local_socket->get_transport(),
...
int _trans_layer::update_uas_reply(trans_bucket* bucket, sip_trans* t,
                                   int reply_code)
{
  ...
  if (t->reply_status >= 300) {
    ...
    if (t->msg->u.request->method == sip_request::INVITE) {
      if (!reliable_trsp) t->reset_timer(STIMER_G, G_TIMER, bucket->get_id());
                                     ...
                                     sip_msg* msg)
{
  ...
  if (method != sip_request::ACK && method != sip_request::PRACK) {
  ...
  switch (t->state) {
    ...
    default: DBG("Bug? Unknown state at this point: %i\n", t->state); break;
  }
...
void _trans_layer::send_non_200_ack(sip_msg* reply, sip_trans* t)
{
  ...
  ack_len += copy_hdr_len(inv->via1) + copy_hdr_len(inv->from)
             + copy_hdr_len(reply->to) + copy_hdr_len(inv->callid);

  ack_len += cseq_len(get_cseq(inv)->num_str, method);

  if (!inv->route.empty()) ack_len += copy_hdrs_len(inv->route);
  ...
  if (!inv->route.empty()) copy_hdrs_wr(&c, inv->route);
  ...
  *c++ = LF;
  ...
  int send_err =
      inv->local_socket->send(&inv->remote_ip, ack_buf, ack_len, t->flags);
  if (send_err < 0) {
    ERROR("Error from transport layer\n");
  }
  else {
  ...
  switch (type) {
    case STIMER_A: // Calling: (re-)send INV
      ...
      if (tr->logger) {
        ...
        tr->logger->log(tr->msg->buf, tr->msg->len, &src_ip,
                        &tr->msg->remote_ip, tr->msg->u.request->method_str);
      }
    ...
    case STIMER_G: { // Completed: (re-)send response
      ...
      unsigned int retr_timer =
          (type == STIMER_E) ? E_TIMER << n : G_TIMER << n;
    ...
    case STIMER_BL:
      ...
      if (!(tr->flags & TR_FLAG_DISABLE_BL)) {
        ...
        if (default_bl_ttl) {
          tr_blacklist::instance()->insert(&tr->msg->remote_ip, default_bl_ttl,
                                           "timeout");
        }
      }
      bucket->remove(tr);
    ...
    default: ERROR("Invalid timer type %i\n", type); break;
  }
...
{
  if (transports.size() == 0) return 0;

  if (transports.size() == 1) return 0;
  ...
  socklen_t        remote_ip_len = 0;
  // UNUSED
  // trsp_socket* tsock=NULL;
  // UNUSED_END

  if (remote_ip->ss_family == AF_INET) {
    remote_ip_len = sizeof(sockaddr_in);
  }
  else {
    remote_ip_len = sizeof(sockaddr_in6);
  }

  if (connect(temp_sock, (sockaddr*) remote_ip, remote_ip_len) == -1) {
...
try_next_dest:
  ...
  if (use_new_trans) {
    string                     n_uri;
    cstring                    old_uri;
    std::unique_ptr<sip_trans> n_tr(copy_uac_trans(tr));
    ...
    if (set_trsp_socket(&tmp_msg, next_trsp, out_interface) < 0) return -1;
    ...
    if (set_trsp_socket(tr->msg, next_trsp, out_interface) < 0) return -1;
...
void trans_ticket::lock_bucket() const { _bucket->lock(); }

void trans_ticket::unlock_bucket() const { _bucket->unlock(); }
...
void trans_ticket::remove_trans() { _bucket->remove(_t); }

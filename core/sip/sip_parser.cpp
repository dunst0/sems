...
#define case_SIPVER_NUM(ch1, st1, st2)                                         \
  case st1:                                                                    \
    switch (**c) {                                                             \
...
int parse_method(int* method, const char* beg, int len)
{
  ...
  *method = sip_request::OTHER_METHOD;
  ...
  switch (len) {
    ...
    case PRACK_len:
      if (!memcmp(c, PRACKm, PRACK_len)) *method = sip_request::PRACK;
      break;
  }
...
static int parse_first_line(sip_msg* msg, char** c, char* end)
{
  ...
  for (; (*c < end) && **c; (*c)++) {
    switch (st) {
      case_SIPVER_TEXT('S', 's', FL_SIPVER1, FL_SIPVER2);
      case_SIPVER_TEXT('I', 'i', FL_SIPVER2, FL_SIPVER3);
      case_SIPVER_TEXT('P', 'p', FL_SIPVER3, FL_SIPVER4);
      ...
      case FL_SIPVER4: // '/'
        ...
        case_SIPVER_NUM('2', FL_SIPVER_DIG1, FL_SIPVER_SEP);
        case_SIPVER_NUM('.', FL_SIPVER_SEP, FL_SIPVER_DIG2);
...
int parse_headers(sip_msg* msg, char** c, char* end)
{
  ...
  if (!err) {
    for (list<sip_header*>::iterator it = hdrs.begin(); it != hdrs.end();
         it++) {
      sip_header* hdr = *it;
      switch (hdr->type) {
        case sip_header::H_CALL_ID: msg->callid = hdr; break;

        case sip_header::H_CONTACT: msg->contacts.push_back(hdr); break;

        case sip_header::H_CONTENT_LENGTH: msg->content_length = hdr; break;

        case sip_header::H_CONTENT_TYPE: msg->content_type = hdr; break;

        case sip_header::H_FROM: msg->from = hdr; break;

        case sip_header::H_TO: msg->to = hdr; break;

        case sip_header::H_VIA:
          if (!msg->via1) msg->via1 = hdr;
        ...
        case sip_header::H_CSEQ: msg->cseq = hdr; break;
...
int parse_sip_msg(sip_msg* msg, char*& err_msg)
{
  ...
  if (!parse_via(via.get(), msg->via1->value.s, msg->via1->value.len)
      && !via->parms.empty()) {
  ...
  if (!parse_cseq(cseq.get(), msg->cseq->value.s, msg->cseq->value.len)
      && cseq->num_str.len && cseq->method_str.len) {
  ...
  if (parse_from_to(from.get(), msg->from->value.s, msg->from->value.len)
      != 0) {

                    ...
                    int output_sample_rate, unsigned int nb_samples)
{
  ...
  while (cur_item && cur_item->play
         && (ret = cur_item->play->get(system_ts, buffer, output_sample_rate,
                                       nb_samples))
                <= 0) {
                    ...
                    int input_sample_rate, unsigned int size)
{
  ...
  while (cur_item && cur_item->record
         && (ret = cur_item->record->put(system_ts, buffer, input_sample_rate,
                                         size))
                < 0) {
  ...
  if (!cur_item || !cur_item->record) ret = size;
...
void AmPlaylist::flush()
{
  ...
  while (cur_item) gotoNextItem(false);
  cur_mut.unlock();
}

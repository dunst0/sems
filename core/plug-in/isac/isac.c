...
int Pcm16_2_iSAC(unsigned char* out_buf, unsigned char* in_buf,
                 unsigned int size, unsigned int channels, unsigned int rate,
                 long int h_codec);
int iSAC_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                 unsigned int size, unsigned int channels, unsigned int rate,
                 long int h_codec);

static long int iSAC_create(const char*             format_parameters,
                            const char**            format_parameters_out,
                            amci_codec_fmt_info_t** format_description);
static void iSAC_destroy(long int handle);

static unsigned int iSAC_bytes2samples(long int, unsigned int);
static unsigned int iSAC_samples2bytes(long int, unsigned int);

static amci_codec_fmt_info_t isac_fmt_description[] = {
    {AMCI_FMT_FRAME_SIZE, iSAC_FRAME_MS* iSAC_SAMPLE_RATE / 1000},
    {0, 0}};
...
CODEC(CODEC_iSAC_WB, Pcm16_2_iSAC, iSAC_2_Pcm16, AMCI_NO_CODEC_PLC, iSAC_create,
      iSAC_destroy, iSAC_bytes2samples, iSAC_samples2bytes)
#endif
END_CODECS

BEGIN_PAYLOADS
#if SYSTEM_SAMPLECLOCK_RATE >= 16000
PAYLOAD(-1, "isac", iSAC_SAMPLE_RATE, iSAC_SAMPLE_RATE, 1, CODEC_iSAC_WB,
        AMCI_PT_AUDIO_FRAME)
#endif
END_PAYLOADS
...
static unsigned int iSAC_bytes2samples(long int h_codec, unsigned int num_bytes)
{
  return num_bytes / 2;
}

static unsigned int iSAC_samples2bytes(long int     h_codec,
                 ...
                 long int h_codec)
{
  ...
  samples = WebRtcIsac_Decode(st, (void*) in_buf, size, outPtr, &speechType);
  if (samples < 0) {

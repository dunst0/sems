...
the original input. OPUS_APPLICATION_RESTRICTED_LOWDELAY   Configure the minimum
possible coding delay by disabling certain modes of operation.*/
...
static int opus_2_pcm16(unsigned char* out_buf, unsigned char* in_buf,
...
static int opus_plc(unsigned char* out_buf, unsigned int size,
                    unsigned int channels, unsigned int rate, long int h_codec);

static int pcm16_2_opus(unsigned char* out_buf, unsigned char* in_buf,
                        unsigned int size, unsigned int channels,
                        unsigned int rate, long int h_codec);
static long int opus_create(const char*             format_parameters,
                            const char**            format_parameters_out,
                            amci_codec_fmt_info_t** format_description);
static void opus_destroy(long int h_inst);

static int opus_negotiate_fmt(int is_offer, const char* params_in,
...
static amci_codec_fmt_info_t opus_fmt_description[] = {
    {AMCI_FMT_FRAME_LENGTH, 20},
    {AMCI_FMT_FRAME_SIZE, 20 * _OPUS_RATE / 1000},
    {0, 0}};

BEGIN_EXPORTS("opus", opus_load, AMCI_NO_MODULEDESTROY)

BEGIN_CODECS
CODEC_WITH_FMT(CODEC_OPUS, pcm16_2_opus, opus_2_pcm16, opus_plc, opus_create,
               opus_destroy, NULL, NULL, opus_negotiate_fmt)
END_CODECS
...
int opus_negotiate_fmt(int is_offer, const char* params_in, char* params_out,
                       unsigned int params_out_len)
{
...
static char* read_param(char* input, const char* param, char** param_value)
{
  ...
  *param_value = NULL;
  ...
  if (strncmp(input, param, param_size)) return input;
  if (*(input + param_size) != '=') return input;
  input += param_size + 1;
...
void decode_format_parameters(const char*   format_parameters,
                              unsigned int* maxbandwidth, int* useinbandfec,
                              int* stereo)
{
...
long int opus_create(const char*             format_parameters,
                     const char**            format_parameters_out,
                     amci_codec_fmt_info_t** format_description)
{
  ...
  if (format_parameters) {
    DBG("OPUS params: >>%s<<.\n", format_parameters);
    decode_format_parameters(format_parameters, &maxbandwidth, &useinbandfec,
                             &stereo);
  }

  codec_inst = (opus_state_t*) malloc(sizeof(opus_state_t));

  if (!codec_inst) return -1;

  DBG("OPUS: creating encoder with maxbandwidth=%u, stereo=%s, "
      "useinbandfec=%s\n",
      maxbandwidth, stereo ? "true" : "false", useinbandfec ? "true" : "false");

  codec_inst->opus_enc =
      opus_encoder_create(_OPUS_RATE, 1, _OPUS_APPLICATION_, &error);
  if (error) {
    DBG("OPUS: error %d while creating encoder state.\n", error);
    return -1;
  }

  opus_encoder_ctl(codec_inst->opus_enc,
                   OPUS_SET_FORCE_CHANNELS(stereo ? 2 : 1));
  ...
  opus_encoder_ctl(codec_inst->opus_enc,
                   OPUS_SET_PACKET_LOSS_PERC(_OPUS_PKT_LOSS_PCT_));
  opus_encoder_ctl(codec_inst->opus_enc,
                   OPUS_SET_COMPLEXITY(_OPUS_COMPLEXITY_));
  opus_encoder_ctl(codec_inst->opus_enc,
                   OPUS_SET_INBAND_FEC(useinbandfec ? 1 : 0));
  ...
  *format_description = opus_fmt_description;
  ...
  return (long int) codec_inst;
}

void opus_destroy(long int h_inst)
{
  opus_state_t* codec_inst;
...
int pcm16_2_opus(unsigned char* out_buf, unsigned char* in_buf,
                 ...
                 long int h_codec)
{
  ...
  res = opus_encode(codec_inst->opus_enc, (opus_int16*) in_buf,
                    size / 2 / channels, out_buf, AUDIO_BUFFER_SIZE);
                        ...
                        unsigned int rate, long int h_codec)
{
  ...
  if (0
      < (res = opus_decode(codec_inst->opus_dec, in_buf, size,
                           (opus_int16*) out_buf, AUDIO_BUFFER_SIZE / 2, 0))) {
                    ...
                    unsigned int channels, unsigned int rate, long int h_codec)
{
  ...
  if (0 < (res = opus_decode(codec_inst->opus_dec, NULL, 0,
                             (opus_int16*) out_buf, size / 2 / channels, 0))) {
    /* returns samples in encoded frame */
    res *= 2;
  }

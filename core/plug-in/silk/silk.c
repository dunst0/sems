...
int Pcm16_2_SILK(unsigned char* out_buf, unsigned char* in_buf,
                 unsigned int size, unsigned int channels, unsigned int rate,
                 long int h_codec);
int SILK_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                 unsigned int size, unsigned int channels, unsigned int rate,
                 long int h_codec);

long int SILK_NB_create(const char*             format_parameters,
                        const char**            format_parameters_out,
                        amci_codec_fmt_info_t** format_description);
long int SILK_MB_create(const char*             format_parameters,
                        const char**            format_parameters_out,
                        amci_codec_fmt_info_t** format_description);
long int SILK_WB_create(const char*             format_parameters,
                        const char**            format_parameters_out,
                        amci_codec_fmt_info_t** format_description);
long int SILK_UB_create(const char*             format_parameters,
                        const char**            format_parameters_out,
                        amci_codec_fmt_info_t** format_description);
void SILK_destroy(long int handle);

static unsigned int SILK_bytes2samples(long int, unsigned int);
static unsigned int SILK_samples2bytes(long int, unsigned int);
...
CODEC(CODEC_SILK_UB, Pcm16_2_SILK, SILK_2_Pcm16, AMCI_NO_CODEC_PLC,
      SILK_UB_create, SILK_destroy, SILK_bytes2samples, SILK_samples2bytes)
#endif
#if SYSTEM_SAMPLECLOCK_RATE >= 16000
CODEC(CODEC_SILK_WB, Pcm16_2_SILK, SILK_2_Pcm16, AMCI_NO_CODEC_PLC,
      SILK_WB_create, SILK_destroy, SILK_bytes2samples, SILK_samples2bytes)
#endif
#if SYSTEM_SAMPLECLOCK_RATE >= 12000
CODEC(CODEC_SILK_MB, Pcm16_2_SILK, SILK_2_Pcm16, AMCI_NO_CODEC_PLC,
      SILK_MB_create, SILK_destroy, SILK_bytes2samples, SILK_samples2bytes)
#endif
CODEC(CODEC_SILK_NB, Pcm16_2_SILK, SILK_2_Pcm16, AMCI_NO_CODEC_PLC,
      SILK_NB_create, SILK_destroy, SILK_bytes2samples, SILK_samples2bytes)
END_CODECS
...
static int create_SILK_encoder(SILK_state* st, unsigned int rtp_Hz,
...
static int create_SILK_decoder(SILK_state* st, unsigned int rtp_Hz)
{
...
static long int SILK_create(unsigned int rtp_Hz, unsigned int avg_bit_rate)
{
  ...
  if (create_SILK_encoder(st, rtp_Hz, avg_bit_rate)) goto error;

  if (create_SILK_decoder(st, rtp_Hz)) goto error;
...
static unsigned int SILK_bytes2samples(long int h_codec, unsigned int num_bytes)
{
  return num_bytes / 2;
}

static unsigned int SILK_samples2bytes(long int     h_codec,
                 ...
                 long int h_codec)
{
  ...
  do {
    /* Decode 20 ms */
    ret = SKP_Silk_SDK_Decode(st->psDec, &st->decControl, 0 /*lost*/, in_buf,
                              size, outPtr, &len);
    if (ret) {

...
/* or ATM-AAL packing  -> RFC3551 has the names AAL2-G726-xy for the big-endian
 * packing */
#define G726_PACK_RFC3551 1

static long int G726_create(const char*             format_parameters,
                            const char**            format_parameters_out,
                            amci_codec_fmt_info_t** format_description);
static void G726_destroy(long int h_inst);

static int Pcm16_2_G726_16(unsigned char* out_buf, unsigned char* in_buf,
                           unsigned int size, unsigned int channels,
                           unsigned int rate, long int h_codec);
static int G726_16_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                           unsigned int size, unsigned int channels,
                           unsigned int rate, long int h_codec);

static int Pcm16_2_G726_24(unsigned char* out_buf, unsigned char* in_buf,
                           unsigned int size, unsigned int channels,
                           unsigned int rate, long int h_codec);
static int G726_24_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                           unsigned int size, unsigned int channels,
                           unsigned int rate, long int h_codec);

static int Pcm16_2_G726_32(unsigned char* out_buf, unsigned char* in_buf,
                           unsigned int size, unsigned int channels,
                           unsigned int rate, long int h_codec);
static int G726_32_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                           unsigned int size, unsigned int channels,
                           unsigned int rate, long int h_codec);

static int Pcm16_2_G726_40(unsigned char* out_buf, unsigned char* in_buf,
                           unsigned int size, unsigned int channels,
                           unsigned int rate, long int h_codec);
static int G726_40_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                           unsigned int size, unsigned int channels,
                           unsigned int rate, long int h_codec);

static unsigned int g726_16_bytes2samples(long int, unsigned int);
static unsigned int g726_16_samples2bytes(long int, unsigned int);

static unsigned int g726_24_bytes2samples(long int, unsigned int);
static unsigned int g726_24_samples2bytes(long int, unsigned int);

static unsigned int g726_32_bytes2samples(long int, unsigned int);
static unsigned int g726_32_samples2bytes(long int, unsigned int);

static unsigned int g726_40_bytes2samples(long int, unsigned int);
static unsigned int g726_40_samples2bytes(long int, unsigned int);
                            ...
                            amci_codec_fmt_info_t** format_description)
{
  ...
  return (long int) cinst;
}

static void G726_destroy(long int h_inst)
{
  if (h_inst) free((struct G726_twoway*) h_inst);
}

// 2 bits/sample
static unsigned int g726_16_bytes2samples(long int     h_codec,
                                          unsigned int num_bytes)
{
  return num_bytes << 2;
}
static unsigned int g726_16_samples2bytes(long int     h_codec,
...
// 3 bits/sample
static unsigned int g726_24_bytes2samples(long int     h_codec,
                                          unsigned int num_bytes)
{
  return (num_bytes * 8) / 3;
}
static unsigned int g726_24_samples2bytes(long int     h_codec,
                                          unsigned int num_samples)
{
...
// 4 bits/sample
static unsigned int g726_32_bytes2samples(long int     h_codec,
                                          unsigned int num_bytes)
{
  return num_bytes << 1;
}
static unsigned int g726_32_samples2bytes(long int     h_codec,
                                          unsigned int num_samples)
{
...
// 5 bits/sample
static unsigned int g726_40_bytes2samples(long int     h_codec,
                                          unsigned int num_bytes)
{
  return (num_bytes * 8) / 5;
}
static unsigned int g726_40_samples2bytes(long int     h_codec,
                                          unsigned int num_samples)
{
                           ...
                           unsigned int rate, long int h_codec)
{
  ...
  short int*          sample_buf = (short int*) in_buf;
  struct G726_twoway* cs         = (struct G726_twoway*) h_codec;

  for (i = 0; i < PCM16_B2S(size); i += 4) {
    out_buf[i >> 2] = 0;
    for (j = 0; j < 4; j++) {
      char v = g723_16_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR,
                               &cs->to_g726);
...
#endif
      pcm_buf[4 * i + j] =
          g723_16_decoder(w, AUDIO_ENCODING_LINEAR, &cs->from_g726);
    }
  }
                           ...
                           unsigned int rate, long int h_codec)
{
  ...
  for (i = 0; i < PCM16_B2S(size); i += 8) {
    ...
    for (j = 0; j < 8; j++) {
#ifdef G726_PACK_RFC3551
      v |= (g723_24_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR,
                            &cs->to_g726))
           << (j * 3);
#else
      v |= (g723_24_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR,
                           ...
                           unsigned int rate, long int h_codec)
{
  ...
  for (i = 0; i < size; i += 3) {
    u_int32_t v =
        ((in_buf[i + 2]) << 16) | ((in_buf[i + 1]) << 8) | (in_buf[i]);
                           ...
                           unsigned int rate, long int h_codec)
{
  ...
  for (i = 0; i < PCM16_B2S(size); i += 2) {
    ...
    for (j = 0; j < 2; j++) {
      char v =
          g721_encoder(sample_buf[i + j], AUDIO_ENCODING_LINEAR, &cs->to_g726);
                           ...
                           unsigned int rate, long int h_codec)
{
  ...
  for (i = 0; i < size; i += 5) {
    v = ((u_int64_t) in_buf[i + 4]) << 32 | (u_int64_t)(in_buf[i + 3]) << 24
        | (u_int64_t)(in_buf[i + 2]) << 16 | (u_int64_t)(in_buf[i + 1]) << 8
        | (u_int64_t)(in_buf[i]);

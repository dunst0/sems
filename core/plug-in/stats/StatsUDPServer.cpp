...
int StatsUDPServer::init()
{
  ...
  if (setsockopt(sd, IPPROTO_IP, IP_TOS, (void*) &optval, sizeof(optval))
      == -1) {
...
void StatsUDPServer::run()
{
  ...
  while (true) {
    ...
    if (execute(msg_buf, reply, reply_addr) == -1) continue;
...
static int msg_get_param(char*& msg_c, string& p, char* line_buf,
                         unsigned int size)
{
  if (msg_get_line(msg_c, line_buf, size) != -1) {
    if (!strcmp(".", line_buf)) line_buf[0] = '\0';
                            ...
                            struct sockaddr_in& addr)
{
  ...
  if (cmd_str == "calls") {
    ...
    reply =
        ...
        "get_shutdownmode                   -  returns the shutdown mode's "
        "current state\n"
        "get_callsavg                       -  get number of active calls "
        "(average since the last query)\n"
        "get_callsmax                       -  get maximum of active calls "
        "since the last query\n"
        "get_cpsavg                         -  get calls per second (5 sec "
        "average)\n"
        "get_cpsmax                         -  get maximum of CPS since the "
        "last query\n"

        "dump_transactions                  -  dump transaction table to log "
        "(loglevel debug)\n"

        "DI <factory> <function> (<args>)*  -  invoke DI command\n"
        "\n"
        "When in shutdown mode, SEMS will answer with the configured 5xx "
        "errorcode to\n"
    ...
    if (cmd_str.substr(4, 8) == "loglevel") {
      ...
      reply =
          "sessioncount is " + int2str(AmSession::getSessionCount()) + ".\n";
    }
      ...
      reply = "CPS hard limit: " + int2str(sc->getCPSLimit().first)
              + ", CPS limit: " + int2str(sc->getCPSLimit().second) + "\n";
    }
    else if (cmd_str.substr(4, 12) == "shutdownmode") {
    ...
    if (p2 == string::npos) p2 = cmd_str.length();
    ...
    try {
      ...
      AmArg args;
      for (vector<string>::iterator it = s_args.begin(); it != s_args.end();
           it++) {
        args.push(it->c_str());
      ...
      reply =
          "Exception occured: AmDynInvoke::NotImplemented '" + e.what + "'\n";
    }
    catch (const std::exception& e) {
      reply = "Exception occured: " + string(e.what()) + "\n";
      return 0;
    }
                               ...
                               const struct sockaddr_in& reply_addr)
{
  int err = sendto(sd, reply.c_str(), reply.length() + 1, 0,
                   (const sockaddr*) &reply_addr, sizeof(struct sockaddr_in));

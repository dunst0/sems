...
void print_usage(const char* progname)
{
  fprintf(stderr,
          ...
          "\n"
          "Tips: \n"
          " o quote the command if it has arguments (e.g. %s -c \"set_loglevel "
          "1\")\n"
...
int str2i(const char* str, unsigned short int* result)
{
  int                i    = 0;
...
int main(int argc, char** argv)
{
  ...
  string             port      = "5040";
  string             cmd       = "calls";
  unsigned short int timeout_s = 5;
  ...
  for (map<char, string>::iterator it = args.begin(); it != args.end(); ++it) {
    ...
    switch (it->first) {
      ...
      case 't': {
        if (str2i(it->second.c_str(), &timeout_s) < 0) {
  ...
  unsigned short int port_i = addr.sin_port;
  addr.sin_family           = AF_INET;
  addr.sin_port             = htons(addr.sin_port);

  msg_buf = cmd;
  printf("sending '%s\\n' to %s:%d\n", msg_buf.c_str(), server.c_str(), port_i);

...
/*
  ...
  0		250		5			N/A	No transmission
  (DTX) 1		2150		43			6
  Vocoder (mostly for comfort noise)
  2		5950		119			9	Very noticeable
  artifacts/noise, good intelligibility 3		8000		160
  10	Artifacts/noise sometimes noticeable
  4		11000		220			14	Artifacts
  usually noticeable only with headphones 5		15000		300 11
  Need good headphones to tell the difference
  6		18200		364			17.5	Hard to tell the
  difference even with good headphones
  7		24600		492			14.5	Completely
  transparent for voice, good quality music 8		3950		79
  10.5	Very noticeable artifacts/noise, good intelligibility
...
long int speexNB_create(const char*             format_parameters,
                        const char**            format_parameters_out,
                        amci_codec_fmt_info_t** format_description);
long int speexWB_create(const char*             format_parameters,
                        const char**            format_parameters_out,
                        amci_codec_fmt_info_t** format_description);
long int speexUB_create(const char*             format_parameters,
                        const char**            format_parameters_out,
                        amci_codec_fmt_info_t** format_description);
void speex_destroy(long int handle);

/* static unsigned int speex_bytes2samples(long int, unsigned int); */
/* static unsigned int speex_samples2bytes(long int, unsigned int); */
...
long int speex_create(unsigned int sample_rate, const char* format_parameters,
                      amci_codec_fmt_info_t** format_description)
{
...
{
  SpeexState* ss;
  short int*  pcm    = (short int*) in_buf;
  char*       buffer = (char*) out_buf;
  div_t       blocks;

  ss = (SpeexState*) h_codec;

  if (!ss || channels != 1) return -1;
...
{
  SpeexState* ss;
  short int*  pcm        = (short int*) out_buf;
  int         frames_out = 0;
  ...
  return frames_out * ss->frame_size * sizeof(short int);
}

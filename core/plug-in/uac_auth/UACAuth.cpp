...
{
  if (!_instance) _instance = new UACAuthFactory(MOD_NAME);
  return _instance;
}
...
void UACAuthFactory::invoke(const string& method, const AmArg& args, AmArg& ret)
{
  if (method == "getHandler") {
    CredentialHolder* c =
        dynamic_cast<CredentialHolder*>(args.get(0).asObject());
    ...
    if ((c != NULL) && (cc != NULL)) {
      ...
      ERROR("wrong types in call to getHandler.  (c=%ld, cc= %ld)\n",
            (unsigned long int) c, (unsigned long int) cc);
    }
  }
  else if (method == "checkAuth") {
    // params: Request realm user pwd
    if (args.size() < 4) {
      ERROR("missing arguments to uac_auth checkAuth function, expected "
            "Request realm user pwd\n");
      throw AmArg::TypeMismatchException();
    }

    AmSipRequest* req = dynamic_cast<AmSipRequest*>(args.get(0).asObject());
    if (NULL == req) throw AmArg::TypeMismatchException();
...
bool UACAuth::process(AmEvent* ev) { return false; }

bool UACAuth::onSipEvent(AmSipEvent* ev) { return false; }

bool UACAuth::onSipRequest(const AmSipRequest& req) { return false; }
                         ...
                         AmBasicSipDialog::Status old_dlg_status)
{
  ...
  if (reply.code == 407 || reply.code == 401) {
    ...
    if (ri != sent_requests.end()) {
      ...
      if (!nonce_reuse
          && (((reply.code == 401)
               && getHeader(ri->second.hdrs, SIP_HDR_AUTHORIZATION, true)
                      .length())
              || ((reply.code == 407)
        ...
        if (do_auth(reply.code, auth_hdr, ri->second.method, auth_uri,
                    &(ri->second.body), result)) {
          ...
          // resend request
          if (dlg->sendRequest(ri->second.method, &(ri->second.body), hdrs,
                               flags)
              == 0) {
            processed = true;
...
bool UACAuth::onSendRequest(AmSipRequest& req, int& flags)
{
  ...
  if (!(flags & SIP_FLAGS_NOAUTH) && !challenge.nonce.empty()
      && do_auth(challenge, challenge_code, req.method, dlg->getRemoteUri(),
                 &req.body, result)) {
    // add headers
    if (req.hdrs == "\r\n" || req.hdrs == "\r" || req.hdrs == "\n") {
      req.hdrs = result;
  ...
  sent_requests[req.cseq] = SIPRequestInfo(req.method, &req.body,
                                           req.hdrs //,
                                                    // TODO: fix this!!!
                                           /*dlg->getOAState()*/);
  return false;
...
bool UACAuth::onSendReply(const AmSipRequest& req, AmSipReply& reply,
                          int& flags)
{
  return false;
}
...
bool UACAuth::tc_isequal(const std::string& s1, const std::string& s2)
{
  ...
  for (size_t i = 0; i < s1.length(); i++) res |= s1[i] ^ s2[i];
...
bool UACAuth::tc_isequal(const char* s1, const char* s2, size_t len)
{
  ...
  for (size_t i = 0; i < len; i++) res |= s1[i] ^ s2[i];
...
string UACAuth::find_attribute(const string& name, const string& header)
{
  ...
  while (true) {
    ...
    if (!pos1 || header[pos1 - 1] == ',' || header[pos1 - 1] == ' ') break;
...
bool UACAuth::parse_header(const string&           auth_hdr,
                      ...
                      const string& uri, const AmMimeBody* body, string& result)
{
  ...
  uac_calc_response(ha1, ha2, challenge, cnonce, qop_value, nonce_count,
                    response);
                           ...
                           HASHHEX sess_key)
{
  ...
  // 		MD5Update(&Md5Ctx, ":", 1);
  // 		MD5Update(&Md5Ctx, challenge.nonce.c_str(),
  // challenge.nonce.length()); 		MD5Update(&Md5Ctx, ":", 1);
...
void UACAuth::checkAuthentication(const AmSipRequest* req, const string& realm,
                                  ...
                                  AmArg& ret)
{
  ...
  if (auth_hdr.size()) {
    ...
    DBG("got realm '%s' nonce '%s', qop '%s', response '%s', username '%s' uri "
        "'%s' cnonce '%s'\n",
        r_challenge.realm.c_str(), r_challenge.nonce.c_str(),
        r_challenge.qop.c_str(), r_response.c_str(), r_username.c_str(),
        r_uri.c_str(), r_cnonce.c_str());
    ...
    //   r_challenge.nonce.c_str()); goto auth_end;
    // }
    // UNUSED_END
    ...
    uac_calc_HA2(req->method, r_uri, r_challenge, qop_auth_int ? hentity : NULL,
                 ha2);

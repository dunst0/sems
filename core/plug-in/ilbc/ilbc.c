...
static int iLBC_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                        unsigned int size, unsigned int channels,
...
static int Pcm16_2_iLBC(unsigned char* out_buf, unsigned char* in_buf,
                        unsigned int size, unsigned int channels,
                        unsigned int rate, long int h_codec);
static long int iLBC_create(const char*             format_parameters,
                            const char**            format_parameters_out,
                            amci_codec_fmt_info_t** format_description);
static void iLBC_destroy(long int h_inst);
static int iLBC_open(FILE* fp, struct amci_file_desc_t* fmt_desc, int options,
                     long int h_codec);
static int iLBC_close(FILE* fp, struct amci_file_desc_t* fmt_desc, int options,
                      long int h_codec, struct amci_codec_t* codec);

static unsigned int ilbc_bytes2samples(long int, unsigned int);
static unsigned int ilbc_samples2bytes(long int, unsigned int);

static amci_codec_fmt_info_t ilbc_fmt_description_30[] = {
    {AMCI_FMT_FRAME_LENGTH, 30},
    {AMCI_FMT_FRAME_SIZE, 240},
    {AMCI_FMT_ENCODED_FRAME_SIZE, 50},
    {0, 0}};

static amci_codec_fmt_info_t ilbc_fmt_description_20[] = {
    {AMCI_FMT_FRAME_LENGTH, 20},
    {AMCI_FMT_FRAME_SIZE, 160},
    {AMCI_FMT_ENCODED_FRAME_SIZE, 38},
    {0, 0}};

BEGIN_EXPORTS("ilbc", AMCI_NO_MODULEINIT, AMCI_NO_MODULEDESTROY)

BEGIN_CODECS
CODEC(CODEC_ILBC, Pcm16_2_iLBC, iLBC_2_Pcm16, iLBC_PLC, iLBC_create,
      iLBC_destroy, ilbc_bytes2samples, ilbc_samples2bytes)
END_CODECS
...
static unsigned int ilbc_bytes2samples(long int h_codec, unsigned int num_bytes)
{
...
static unsigned int ilbc_samples2bytes(long int     h_codec,
...
long int iLBC_create(const char*             format_parameters,
                     const char**            format_parameters_out,
                     amci_codec_fmt_info_t** format_description)
{
  ...
  if ((!format_parameters) || (!*format_parameters)
      || (!(mbegin = strstr(format_parameters, "mode")))) {
  ...
  *format_description =
      (mode == 30) ? ilbc_fmt_description_30 : ilbc_fmt_description_20;
...
void iLBC_destroy(long int h_inst)
{
  if (h_inst) free((char*) h_inst);
}

int Pcm16_2_iLBC(unsigned char* out_buf, unsigned char* in_buf,
                 ...
                 long int h_codec)
{
  ...
  for (i = 0; i < size / (2 * codec_inst->iLBC_Enc_Inst.blockl); i++) {
    ...
    iLBC_encode((unsigned char*) (out_buf + out_buf_offset), block,
                &codec_inst->iLBC_Enc_Inst);
...
static int iLBC_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                        ...
                        unsigned int rate, long int h_codec)
{
  return iLBC_2_Pcm16_Ext(out_buf, in_buf, size, channels, rate, h_codec,
                          1 /*Normal*/);
}
                    ...
                    unsigned int channels, unsigned int rate, long int h_codec)
{
  ...
  return iLBC_2_Pcm16_Ext(out_buf, NULL, in_size, channels, rate, h_codec,
                          0 /*PL*/);
}
...
int iLBC_open(FILE* fp, struct amci_file_desc_t* fmt_desc, int options,
              long int h_codec)
{
  DBG("ilbc_open.\n");
...
int iLBC_close(FILE* fp, struct amci_file_desc_t* fmt_desc, int options,
               long int h_codec, struct amci_codec_t* codec)
{
  DBG("iLBC_close.\n");
  return 0;

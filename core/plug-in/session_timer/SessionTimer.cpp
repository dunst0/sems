...
bool SessionTimer::onSendReply(const AmSipRequest& req, AmSipReply& reply,
                               int& flags)
{
  ...
  reply.hdrs +=
      SIP_HDR_COLSP(SIP_HDR_SESSION_EXPIRES) + int2str(session_interval)
      ...
      if (i_se < sst_cfg.getMinimumTimer()) {
        throw AmSession::Exception(422, "Session Interval Too Small",
                                   SIP_HDR_COLSP(SIP_HDR_MIN_SE)
                                       + int2str(sst_cfg.getMinimumTimer())
                                       + CRLF);
      }
    }
    else {
...
{
  if ((req.method == SIP_METH_INVITE) || (req.method == SIP_METH_UPDATE)) {
    remote_timer_aware = key_in_list(
        getHeader(req.hdrs, SIP_HDR_SUPPORTED, SIP_HDR_SUPPORTED_COMPACT),
        TIMER_OPTION_TAG);

...
static int pcm16_2_gsm(unsigned char* out_buf, unsigned char* in_buf,
                       unsigned int size, unsigned int channels,
                       unsigned int rate, long int h_codec);

static int gsm_2_pcm16(unsigned char* out_buf, unsigned char* in_buf,
                       unsigned int size, unsigned int channels,
                       unsigned int rate, long int h_codec);

static long int gsm_create_if(const char*             format_parameters,
                              const char**            format_parameters_out,
                              amci_codec_fmt_info_t** format_description);

static void gsm_destroy_if(long int h_codec);

static unsigned int gsm_bytes2samples(long int, unsigned int);
static unsigned int gsm_samples2bytes(long int, unsigned int);
...
BEGIN_CODECS
CODEC(CODEC_GSM0610, pcm16_2_gsm, gsm_2_pcm16, AMCI_NO_CODEC_PLC, gsm_create_if,
      (amci_codec_destroy_t) gsm_destroy_if, gsm_bytes2samples,
      gsm_samples2bytes)
END_CODECS
...
static unsigned int gsm_bytes2samples(long int h_codec, unsigned int num_bytes)
{
  return 160 * (num_bytes / 33);
}

static unsigned int gsm_samples2bytes(long int     h_codec,
                       ...
                       unsigned int rate, long int h_codec)
{
  ...
  if (blocks.rem) {
    ERROR(
        "gsm_2_pcm16: number of blocks should be integral (block size = 33)\n");
    return -1;
  }

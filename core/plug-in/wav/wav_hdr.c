...
#define bswap_16(A)                                                            \
  ((((u_int16_t)(A) &0xff00) >> 8) | (((u_int16_t)(A) &0x00ff) << 8))
#define bswap_32(A)                                                            \
  ((((u_int32_t)(A) &0xff000000) >> 24) | (((u_int32_t)(A) &0x00ff0000) >> 8)  \
...
static int wav_read_header(FILE* fp, struct amci_file_desc_t* fmt_desc)
{
  ...
  unsigned short int fmt             = 0;
  unsigned short int channels        = 0;
  unsigned int       rate            = 0;
  unsigned short int bits_per_sample = 0;
  unsigned short int sample_size     = 0;
  char               dummy[6]        = {'\0'};
  int                is_seekable     = 1;
...
int wav_open(FILE* fp, struct amci_file_desc_t* fmt_desc, int options,
             long int h_codec)
{
                     ...
                     long int h_codec, struct amci_codec_t* codec)
{
  ...
  hdr.channels     = cpu_to_le16((unsigned short int) fmt_desc->channels);
  ...
  hdr.bytes_per_second =
      cpu_to_le32(hdr.sample_rate * (unsigned int) hdr.sample_size);
  hdr.precision = cpu_to_le16((unsigned short int) (sample_size * 8));
...
int wav_close(FILE* fp, struct amci_file_desc_t* fmt_desc, int options,
              long int h_codec, struct amci_codec_t* codec)
{
...
int wav_mem_open(unsigned char* mptr, unsigned long int size,
                 unsigned long int* pos, struct amci_file_desc_t* fmt_desc,
...
int wav_mem_close(unsigned char* mptr, unsigned long int* pos,

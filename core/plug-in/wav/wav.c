 ...
 * This plug-in is a full description of supported codecs, payloads and file
 formats.
 *
 * Features: <ul>
 ...
 * Example declaration:
 ...
 BEGIN_FILE_FORMATS
 BEGIN_FILE_FORMAT( "Wav", "wav", "application/x-wav", wav_open, wav_close, 0,
 0) BEGIN_SUBTYPES SUBTYPE( WAV_PCM,  "Pcm16",  2, -1, -1, CODEC_PCM16 )
...
static int ALaw_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                        unsigned int size, unsigned int channels,
                        unsigned int rate, long int h_codec);

static int Pcm16_2_ULaw(unsigned char* out_buf, unsigned char* in_buf,
                        unsigned int size, unsigned int channels,
                        unsigned int rate, long int h_codec);

static int Pcm16_2_ALaw(unsigned char* out_buf, unsigned char* in_buf,
                        unsigned int size, unsigned int channels,
                        unsigned int rate, long int h_codec);

static unsigned int g711_bytes2samples(long int, unsigned int);
static unsigned int g711_samples2bytes(long int, unsigned int);

BEGIN_EXPORTS("wav", AMCI_NO_MODULEINIT, AMCI_NO_MODULEDESTROY)

BEGIN_CODECS
CODEC(CODEC_ULAW, Pcm16_2_ULaw, ULaw_2_Pcm16, AMCI_NO_CODEC_PLC,
      AMCI_NO_CODECCREATE, AMCI_NO_CODECDESTROY, g711_bytes2samples,
      g711_samples2bytes)
CODEC(CODEC_ALAW, Pcm16_2_ALaw, ALaw_2_Pcm16, AMCI_NO_CODEC_PLC,
      AMCI_NO_CODECCREATE, AMCI_NO_CODECDESTROY, g711_bytes2samples,
      g711_samples2bytes)
END_CODECS
...
BEGIN_SUBTYPES
SUBTYPE(WAV_PCM, "Pcm16", 8000, 1,
        CODEC_PCM16) // we only support 8000/1 channel !
...
static unsigned int g711_bytes2samples(long int h_codec, unsigned int num_bytes)
{
  /* ALAW and ULAW formats has one sample per byte */
  return num_bytes;
}

static unsigned int g711_samples2bytes(long int     h_codec,
...
static int ULaw_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                        ...
                        unsigned int rate, long int h_codec)
{
  unsigned short int* out_b = (unsigned short int*) out_buf;
...
int Pcm16_2_ALaw(unsigned char* out_buf, unsigned char* in_buf,
                 unsigned int size, unsigned int channels, unsigned int rate,
                 long int h_codec)
{
  unsigned char* out_b = out_buf;

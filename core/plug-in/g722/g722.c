...
int Pcm16_2_G722NB(unsigned char* out_buf, unsigned char* in_buf,
                   ...
                   long int h_codec);
int G722NB_2_Pcm16(unsigned char* out_buf, unsigned char* in_buf,
                   unsigned int size, unsigned int channels, unsigned int rate,
                   long int h_codec);

long int G722NB_create(const char*             format_parameters,
                       const char**            format_parameters_out,
                       amci_codec_fmt_info_t** format_description);
void G722NB_destroy(long int handle);

static unsigned int G722NB_bytes2samples(long int, unsigned int);
static unsigned int G722NB_samples2bytes(long int, unsigned int);
...
CODEC(CODEC_G722_NB, Pcm16_2_G722NB, G722NB_2_Pcm16, AMCI_NO_CODEC_PLC,
      G722NB_create, G722NB_destroy, G722NB_bytes2samples, G722NB_samples2bytes)
END_CODECS
                       ...
                       amci_codec_fmt_info_t** format_description)
{
  ...
  gs->encode_state =
      g722_encode_init(NULL, 64000, 0 /* G722_SAMPLE_RATE_8000 */);
  ...
  gs->decode_state =
      g722_decode_init(NULL, 64000, 0 /* G722_SAMPLE_RATE_8000 */);
  if (!gs->decode_state) {
...
static unsigned int G722NB_bytes2samples(long int     h_codec,
                                         unsigned int num_bytes)
{
  return num_bytes * 2;
}

static unsigned int G722NB_samples2bytes(long int     h_codec,

...
class AmLoggingFacility : public AmPluginFactory
{
 public:
   ...
   * @param msg   message
   */
  virtual void log(int level, pid_t pid, pthread_t tid, const char* func,
                   const char* file, int line, char* msg) = 0;
};
...
#define FACTORY_SESSION_EVENT_HANDLER_EXPORT_STR                               \
  XSTR(FACTORY_SESSION_EVENT_HANDLER_EXPORT)
...
#define FACTORY_SIP_EVENT_HANDLER_EXPORT_STR                                   \
  XSTR(FACTORY_SIP_EVENT_HANDLER_EXPORT)
...
//    e.g. AmPlugIn::registerApplication(...),
//    AmPlugIn::registerDIInterface(...) etc

#define EXPORT_MODULE_FUNC(class_name)                                         \
  extern "C" void* base_plugin_create() { return class_name::instance(); }

#define EXPORT_MODULE_FACTORY(class_name) EXPORT_MODULE_FUNC(class_name)
...
#define DEFINE_MODULE_INSTANCE(class_name, mod_name)                           \
  ...
  {                                                                            \
    if (_instance == NULL) _instance = new class_name(mod_name);               \
    return _instance;                                                          \
  }

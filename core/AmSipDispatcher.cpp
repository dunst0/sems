  ...
  if (!AmEventDispatcher::instance()->post(id, ev)) {
    if ((reply.code >= 100) && (reply.code < 300)) {
      if (AmConfig::UnhandledReplyLoglevel >= 0) {
        _LOG(AmConfig::UnhandledReplyLoglevel, "unhandled SIP reply: %s\n",
             reply.print().c_str());
      }
    }
    else {
...
void AmSipDispatcher::handleSipMsg(AmSipRequest& req)
{
  ...
  if (req.method == SIP_METH_CANCEL) {
    ...
    // Contact-user may contain internal dialog ID (must be tried before
    // using local_tag for identification)
    if (!req.user.empty() && ev_disp->post(req.user, ev)) return;

    if (ev_disp->post(local_tag, ev)) return;
  ...
  else if (req.method == SIP_METH_BYE || req.method == SIP_METH_PRACK) {

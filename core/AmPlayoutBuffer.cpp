...
void AmPlayoutBuffer::direct_write_buffer(unsigned int ts, ShortSample* buf,
                                          unsigned int len)
{
  buffer_put(w_ts, buf, len);
}

void AmPlayoutBuffer::write(u_int32_t ref_ts, u_int32_t rtp_ts, int16_t* buf,
...
void AmPlayoutBuffer::write_buffer(u_int32_t ref_ts, u_int32_t ts, int16_t* buf,
                                   u_int32_t len)
{
  buffer_put(w_ts, buf, len);
}
...
void AmPlayoutBuffer::buffer_put(unsigned int ts, ShortSample* buf,
                                 unsigned int len)
{
  buffer.put(ts, buf, len);

  if (ts_less()(w_ts, ts + len)) w_ts = ts + len;
}

void AmPlayoutBuffer::buffer_get(unsigned int ts, ShortSample* buf,
                                 unsigned int len)
{
  buffer.get(ts, buf, len);

  if (ts_less()(r_ts, ts + len)) r_ts = ts + len;
}
...
u_int32_t AmAdaptivePlayout::next_delay(u_int32_t ref_ts, u_int32_t ts)
{
  ...
  if (it != o_stat.end()) o_stat.erase(it);
  ...
  int32_t D = int32_t(
      D_r
      + double(D_r1 - D_r)
            * ((double(o_stat.size()) + 1.0) * (1.0 - loss_rate) - double(r)));

  if (++idx >= ORDER_STAT_WIN_SIZE) idx = 0;
                                     ...
                                     int16_t* buf, u_int32_t len)
{
  ...
  if (short_scaled.mean() > 2.0) {
    if (shr_threshold < 3000) shr_threshold += 10;
  }
  else if (short_scaled.mean() < 1.0) {
    if (shr_threshold > 100) shr_threshold -= 2;
  }
  ...
  // PLC
  if (n_len < (int32_t) len) w_ts += n_len - len;

  if (w_ts != old_wts) plc_cnt = 0;
...
void AmAdaptivePlayout::direct_write_buffer(unsigned int ts, ShortSample* buf,
                                            unsigned int len)
{
                                        ...
                                        u_int32_t packet_len)
{
  ...
  // safety
  if (packet_len > MAX_PACKET_SAMPLES) return s;

  // not possible to stretch packets shorter than 10ms
  if (packet_len < TEMPLATE_SEG) return s;
  ...
  while (true) {
    ...
    if (factor > 1.0) {
      // expansion
      srch_beg = tmpl - (int) ((float) TEMPLATE_SEG * (factor - 1.0))
                 - SEARCH_REGION / 2;
      srch_end = srch_beg + SEARCH_REGION;

      if (srch_beg < p_buf_beg) srch_beg = p_buf_beg;

      if (srch_end + DELTA >= tmpl) srch_end = tmpl - DELTA;
    }
    else {
      // compression
      srch_end = tmpl + (int) ((float) TEMPLATE_SEG * (1.0 - factor))
                 + SEARCH_REGION / 2;
      ...
      if (srch_beg - DELTA < tmpl) srch_beg = tmpl + DELTA;
    }
    // stop if search region size < 0
    if (srch_beg >= srch_end) break;
...
void AmJbPlayout::direct_write_buffer(unsigned int ts, ShortSample* buf,
                                      unsigned int len)
{
  buffer_put(ts, buf, len);
}
...
void AmJbPlayout::prepare_buffer(unsigned int audio_buffer_ts, unsigned int ms)
{
  ...
  while (m_jb.get(audio_buffer_ts, ms, buf, &nb_samples, &ts)) {
    ...
    if (last_ts_i && ts_less()(m_last_rtp_endts, ts)) {
      int concealed_size = m_plcbuffer->conceal_loss(ts - m_last_rtp_endts,
                                                     (unsigned char*) buf);
      if (concealed_size > 0)
  ...
  if (ts_less()(m_last_rtp_endts, audio_buffer_ts + ms)) {
    ...
    int concealed_size = m_plcbuffer->conceal_loss(
        audio_buffer_ts + ms - m_last_rtp_endts, (unsigned char*) buf);
    if (concealed_size > 0)
      direct_write_buffer(m_last_rtp_endts, buf, PCM16_B2S(concealed_size));
...
void AmJbPlayout::write(u_int32_t ref_ts, u_int32_t rtp_ts, int16_t* buf,
                        u_int32_t len, bool begin_talk)
{

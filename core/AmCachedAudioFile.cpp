...
AmFileCache::~AmFileCache()
{
  if ((data != NULL) && munmap(data, data_size)) {
...
int AmFileCache::load(const std::string& filename)
{
  ...
  if ((data = mmap((caddr_t) 0, sbuf.st_size, PROT_READ, MAP_PRIVATE, fd, 0))
      == (caddr_t)(-1)) {
...
int AmFileCache::read(void* buf, size_t* pos, size_t size)
{
    ...
    , loop(false)
{
  ...
  if (iofmt->mem_open
      && !(ret = (*iofmt->mem_open)(
               (unsigned char*) cache->getData(), cache->getSize(), &ofpos, &fd,
               AmAudioFile::Read, f_fmt->getHCodecNoInit()))) {
...
void AmCachedAudioFile::rewind() { fpos = begin; }

/** Closes the file. */
void AmCachedAudioFile::close() { fpos = 0; }

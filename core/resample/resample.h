...
class Resample
{
 ...
 public:
   ...
   * num_samples - number of samples to put into dst (for stereo 1 sample means
   * 2 samples values, left and right) rstate - resampling state
   *
...
{
 private:
  static inline float updownsample(float* src, unsigned int current,
                                   unsigned int curfrc);
...
class ResampleFactory
{
 public:
  ...
  static Resample* createResampleObj(bool doPad, float maxRatio,

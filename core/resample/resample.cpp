...
int Resample::put_samples(signed short* samples, unsigned int num_samples)
{
  if (current >= MOVE_THRESH) {
    this->samples.erase(this->samples.begin(),
                        this->samples.begin() + current - filter_delay);
  ...
  for (i = 0; i < 15; i++) {
    for (k = 0; k < 256; k++) {
      if (i == 7 && k == 0) {
        ...
        sinc[i][k] = sin((i + k / 256.0 - 7) * PI) / ((i + k / 256.0 - 7) * PI);
      }
    }
  }
...
inline float ResampleSincMono::upsample(float* src, unsigned int current,
                                        unsigned int curfrc)
{
  ...
  sinc_val = (sinc[14][(curfrc >> 24) + 1] - sinc[14][curfrc >> 24]) * curfloat
             + sinc[14][curfrc >> 24];
  samp = src[current - 7] * sinc_val;

  sinc_val = (sinc[13][(curfrc >> 24) + 1] - sinc[13][curfrc >> 24]) * curfloat
             + sinc[13][curfrc >> 24];
  samp += src[current - 6] * sinc_val;

  sinc_val = (sinc[12][(curfrc >> 24) + 1] - sinc[12][curfrc >> 24]) * curfloat
             + sinc[12][curfrc >> 24];
  samp += src[current - 5] * sinc_val;

  sinc_val = (sinc[11][(curfrc >> 24) + 1] - sinc[11][curfrc >> 24]) * curfloat
             + sinc[11][curfrc >> 24];
  samp += src[current - 4] * sinc_val;

  sinc_val = (sinc[10][(curfrc >> 24) + 1] - sinc[10][curfrc >> 24]) * curfloat
             + sinc[10][curfrc >> 24];
  samp += src[current - 3] * sinc_val;

  sinc_val = (sinc[9][(curfrc >> 24) + 1] - sinc[9][curfrc >> 24]) * curfloat
             + sinc[9][curfrc >> 24];
  samp += src[current - 2] * sinc_val;

  sinc_val = (sinc[8][(curfrc >> 24) + 1] - sinc[8][curfrc >> 24]) * curfloat
             + sinc[8][curfrc >> 24];
  samp += src[current - 1] * sinc_val;

  sinc_val = (sinc[7][(curfrc >> 24) + 1] - sinc[7][curfrc >> 24]) * curfloat
             + sinc[7][curfrc >> 24];
  samp += src[current] * sinc_val;

  sinc_val = (sinc[6][(curfrc >> 24) + 1] - sinc[6][curfrc >> 24]) * curfloat
             + sinc[6][curfrc >> 24];
  samp += src[current + 1] * sinc_val;

  sinc_val = (sinc[5][(curfrc >> 24) + 1] - sinc[5][curfrc >> 24]) * curfloat
             + sinc[5][curfrc >> 24];
  samp += src[current + 2] * sinc_val;

  sinc_val = (sinc[4][(curfrc >> 24) + 1] - sinc[4][curfrc >> 24]) * curfloat
             + sinc[4][curfrc >> 24];
  samp += src[current + 3] * sinc_val;

  sinc_val = (sinc[3][(curfrc >> 24) + 1] - sinc[3][curfrc >> 24]) * curfloat
             + sinc[3][curfrc >> 24];
  samp += src[current + 4] * sinc_val;

  sinc_val = (sinc[2][(curfrc >> 24) + 1] - sinc[2][curfrc >> 24]) * curfloat
             + sinc[2][curfrc >> 24];
  samp += src[current + 5] * sinc_val;

  sinc_val = (sinc[1][(curfrc >> 24) + 1] - sinc[1][curfrc >> 24]) * curfloat
             + sinc[1][curfrc >> 24];
  samp += src[current + 6] * sinc_val;

  sinc_val = (sinc[0][(curfrc >> 24) + 1] - sinc[0][curfrc >> 24]) * curfloat
             + sinc[0][curfrc >> 24];
  samp += src[current + 7] * sinc_val;
                                          ...
                                          float              scale)
{
  ...
  samp = src[current + 7]
         * sinc[(sinc_current >> 32) & 0xffffffff]
               [(sinc_current & 0xffffffff) >> 24];
                               ...
                               unsigned int num_samples)
{
  ...
  if (rate < 1.0) { // downsampling, need to scale sinc apropriatly
    ...
    while (num_samples > 0) {
      ...
      samp = ResampleSincMono::downsample(&samples[0], current, curfrc,
                                          sinc_increment, rate);
...
inline float ResampleSincStereo::upsample(float* src, unsigned int current,
                                          unsigned int curfrc)
{
  float sinc_val, curfloat;
  float samp;
                                            ...
                                            float              scale)
{
  ...
  // todo: linear interpolation between sinc values
  samp = src[current * 2 + 14]
         * sinc[(sinc_current >> 32) & 0xffffffff]
               [(sinc_current & 0xffffffff) >> 24];

  sinc_current += sinc_increment;
  samp += src[current * 2 + 12]
          * sinc[(sinc_current >> 32) & 0xffffffff]
                [(sinc_current & 0xffffffff) >> 24];

  sinc_current += sinc_increment;
  samp += src[current * 2 + 10]
          * sinc[(sinc_current >> 32) & 0xffffffff]
                [(sinc_current & 0xffffffff) >> 24];

  sinc_current += sinc_increment;
  samp += src[current * 2 + 8]
          * sinc[(sinc_current >> 32) & 0xffffffff]
                [(sinc_current & 0xffffffff) >> 24];

  sinc_current += sinc_increment;
  samp += src[current * 2 + 6]
          * sinc[(sinc_current >> 32) & 0xffffffff]
                [(sinc_current & 0xffffffff) >> 24];

  sinc_current += sinc_increment;
  samp += src[current * 2 + 4]
          * sinc[(sinc_current >> 32) & 0xffffffff]
                [(sinc_current & 0xffffffff) >> 24];

  sinc_current += sinc_increment;
  samp += src[current * 2 + 2]
          * sinc[(sinc_current >> 32) & 0xffffffff]
                [(sinc_current & 0xffffffff) >> 24];
                                 ...
                                 unsigned int num_samples)
{
  ...
  if (rate < 1.0) { // downsampling, need to scale sinc apropriatly
    ...
    while (num_samples > 0) {
      ...
      samp_l = ResampleSincStereo::downsample(&samples[0], current, curfrc,
                                              sinc_increment, rate);
      samp_r = ResampleSincStereo::downsample(&samples[1], current, curfrc,
                                              sinc_increment, rate);
...
inline float ResampleLinMono::updownsample(float* src, unsigned int current,
                                           unsigned int curfrc)
{
  float samp;

  samp = src[current]
         + (src[current + 1] - src[current]) * (curfrc / 4294967296.0);
...
inline float ResampleLinStereo::updownsample(float* src, unsigned int current,
                                             unsigned int curfrc)
{
  float samp;

  samp = src[current * 2]
         + (src[current * 2 + 2] - src[current * 2]) * (curfrc / 4294967296.0);
...
Resample* ResampleFactory::createResampleObj(bool doPad, float maxRatio,
                                             interpolType interpol_type,

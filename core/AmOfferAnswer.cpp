...
AmOfferAnswer::OAState AmOfferAnswer::getState() { return state; }
...
const AmSdp& AmOfferAnswer::getLocalSdp() { return sdp_local; }

const AmSdp& AmOfferAnswer::getRemoteSdp() { return sdp_remote; }

/** State maintenance */
void AmOfferAnswer::saveState() { saved_state = state; }
...
int AmOfferAnswer::onRequestIn(const AmSipRequest& req)
{
  ...
  if ((req.method == SIP_METH_INVITE || req.method == SIP_METH_UPDATE
    ...
    if (sdp_body) err_code     = onRxSdp(req.cseq, *sdp_body, &err_txt);
  }
...
int AmOfferAnswer::onRxSdp(unsigned int m_cseq, const AmMimeBody& body,
                           const char** err_txt)
{
...
int AmOfferAnswer::onReplyOut(AmSipReply& reply)
{
  ...
  if (!has_sdp && !generate_sdp) {
    ...
    if (reply.cseq_method == SIP_METH_INVITE) {
      if ((reply.code == 183) || ((reply.code >= 200) && (reply.code < 300))) {
        ...
        //  -> force SDP
        generate_sdp = (state == OA_OfferRecved) || (state == OA_None)
                       || (state == OA_Completed);
      }
    }
  ...
  if (reply.cseq_method == SIP_METH_INVITE && reply.code < 300) {
    ...
    if (has_sdp && (state == OA_Completed || state == OA_OfferSent)
        && reply.cseq == cseq) {
      has_sdp = false;
    }
  }
...
int AmOfferAnswer::onReplySent(const AmSipReply& reply)
{
  ...
  if ((reply.code >= 200) && (reply.cseq_method != SIP_METH_CANCEL)
      && (reply.cseq == cseq) && (reply.cseq_method != SIP_METH_INVITE)) {
    // transaction has been removed:
    //  -> cleanup OA state
...
int AmOfferAnswer::getSdpBody(string& sdp_body)
{
  switch (state) {
    ...
    case OA_OfferSent: DBG("Still waiting for a reply\n"); return -1;

...
AmContentType::~AmContentType() { clearParams(); }
...
void AmMimeBody::clearPart(Parts::iterator position) { parts.erase(position); }
...
int AmContentType::parse(const string& ct)
{
  ...
  switch (st) {
    ...
    case CT_SUBTYPE: subtype = string(beg, c - beg); break;
  }
...
int AmContentType::parseParams(const char* c, const char* end)
{
  ...
  for (list<sip_avp*>::iterator it_ct_param = avp_params.begin();
    ...
    if (p->type == Param::BOUNDARY) mp_boundary = p;
  ...
  switch (len) {
    case BOUNDARY_len:
      if (!lower_cmp(c, BOUNDARY_str, len)) {
        ...
        type = Param::OTHER;
      break;
    default: type = Param::OTHER; break;
  }
...
int AmMimeBody::parseSinglePart(unsigned char* buf, unsigned int len)
{
  ...
  for (list<sip_header*>::iterator it = hdrs.begin(); it != hdrs.end(); it++) {
    ...
    if ((*it)->type == sip_header::H_CONTENT_TYPE) {
      ...
      sub_part_hdrs +=
          c2stlstr((*it)->name) + COLSP + c2stlstr((*it)->value) + CRLF;
    }
  }
...
int AmMimeBody::parse(const string& content_type, const unsigned char* buf,
                      unsigned int len)
{
  if (ct.parse(content_type) < 0) return -1;
...
void AmContentType::setType(const string& t) { type = t; }

void AmContentType::setSubType(const string& st) { subtype = st; }

bool AmContentType::isType(const string& t) const
{
  return !lower_cmp_n(t.c_str(), t.length(), type.c_str(), type.length());
}

bool AmContentType::isSubType(const string& st) const
{
  return !lower_cmp_n(st.c_str(), st.length(), subtype.c_str(),
                      subtype.length());
}

void AmMimeBody::setHeaders(const string& hdrs) { this->hdrs = hdrs; }
...
{
  return (!payload || !content_len) && parts.empty();
}

bool AmContentType::hasContentType(const string& content_type) const
{
  if (content_type.empty() && type.empty() && subtype.empty()) return true;

  if (content_type.empty() != (type.empty() && subtype.empty())) return false;

  // Quick & dirty comparison, might not always be correct
  string cmp_ct = type + "/" + subtype;
  return !lower_cmp_n(cmp_ct.c_str(), cmp_ct.length(), content_type.c_str(),
                      content_type.length());
}

string AmContentType::getStr() const
{
  if (type.empty() && subtype.empty()) return "";

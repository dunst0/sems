...
{
  switch (ev->event_id) {
    case B2ABTerminateLeg: terminateLeg(); break;
  }
}
...
AmB2ABCallerSession::~AmB2ABCallerSession() { delete connector; }
...
{
  if (callee_status != None) AmB2ABSession::terminateOtherLeg();
                                        ...
                                        const string& headers)
{
  ...
  B2ABConnectLegEvent* ev =
      new B2ABConnectLegEvent(remote_party, remote_uri, local_party, local_uri,
...
void AmB2ABCallerSession::setupCalleeSession(
    AmB2ABCalleeSession* callee_session, B2ABConnectLegEvent* ev)
{
  ...
  MONITORING_LOG(other_id.c_str(), "dir", "out");
...
void AmB2ABCalleeSession::onB2ABEvent(B2ABEvent* ev)
{
  if (ev->event_id == B2ABConnectLeg) {
    try {
      ...
      MONITORING_LOG4(getLocalTag().c_str(), "b2b_leg", other_id.c_str(),
                      "from", co_ev->local_party.c_str(), "to",
                      co_ev->remote_party.c_str(), "ruri",
      ...
      relayEvent(
          new B2ABConnectOtherLegExceptionEvent(500, "unexpected exception"));
      setStopped();
    }
  }
...
void AmB2ABCalleeSession::onSipReply(const AmSipRequest&      req,
...
void AmSessionAudioConnector::block() { released.set(false); }

/* mark as released by not owning entity */
void AmSessionAudioConnector::release() { released.set(true); }

/* wait until released  */
void AmSessionAudioConnector::waitReleased() { released.wait_for(); }

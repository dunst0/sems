...
AmSipDialog* AmSession::createSipDialog() { return new AmSipDialog(this); }

void AmSession::setCallgroup(const string& cg) { callgroup = cg; }

string AmSession::getCallgroup() { return callgroup; }
...
{
  if (getStopped() || isProcessingMedia()) return;
...
{
  if (sess_evh != NULL) ev_handlers.push_back(sess_evh);
}
...
const string& AmSession::getRemoteTag() const { return dlg->getRemoteTag(); }

const string& AmSession::getLocalTag() const { return dlg->getLocalTag(); }

const string& AmSession::getFirstBranch() const { return dlg->get1stBranch(); }
...
bool AmSession::processingCycle()
{
  DBG("vv S [%s|%s] %s, %s, %i UACTransPending, %i usages vv\n",
      ...
      sess_stopped.get() ? "stopped" : "running", dlg->getUACTransPending(),
      dlg->getUsages());
  ...
  switch (processing_status) {
    case SESSION_PROCESSING_EVENTS: {
      ...
      DBG("^^ S [%s|%s] %s, %s, %i UACTransPending, %i usages ^^\n",
          ...
          s_stopped ? "stopped" : "running", dlg->getUACTransPending(),
          dlg->getUsages());
...
void AmSession::process(AmEvent* ev)
{
  ...
  if (zrtp_p_ev) {
    onZRTPProtocolEvent((zrtp_protocol_event_t) zrtp_p_ev->event_id,
                        zrtp_p_ev->stream_ctx);
    return;
  }

  AmZRTPSecurityEvent* zrtp_s_ev = dynamic_cast<AmZRTPSecurityEvent*>(ev);
  if (zrtp_s_ev) {
    onZRTPSecurityEvent((zrtp_security_event_t) zrtp_s_ev->event_id,
                        zrtp_s_ev->stream_ctx);
    return;
  }
#endif
}
                           ...
                           AmBasicSipDialog::Status old_dlg_status)
{
  ...
  if (dlg->getStatus() < AmSipDialog::Connected && reply.code == 180) {
    onRinging(reply);
  }

  if (old_dlg_status != dlg->getStatus()) {
    DBG("Dialog status changed %s -> %s (stopped=%s) \n",
        AmBasicSipDialog::getStatusStr(old_dlg_status), dlg->getStatusStr(),
...
{
  if (dlg->getStatus() == AmSipDialog::Connected) dlg->bye();
  setStopped();
}
...
void AmSession::onNoPrack(const AmSipRequest& req, const AmSipReply& rpl)
{
  ...
  // might time-out/fail).
  if (dlg->getStatus() < AmSipDialog::Connected) setStopped();
}

void AmSession::onAudioEvent(AmAudioEvent* audio_ev)
{
  if (audio_ev->event_id == AmAudioEvent::cleared) setStopped();
}
...
void AmSession::onBye(const AmSipRequest& req) { setStopped(); }
...
void AmSession::onSendReply(const AmSipRequest& req, AmSipReply& reply,
                            int& flags)
{
...
bool AmSession::getSdpOffer(AmSdp& offer)
{
  ...
  if (RTPStream()->getSdpMediaIndex() < 0) offer.media.clear();
 ...
 public:
  ...
  bool operator()(const SdpPayload& left, const SdpPayload& right)
  {
    for (vector<string>::iterator it = AmConfig::CodecOrder.begin();
         it != AmConfig::CodecOrder.end(); it++) {
      if (strcasecmp(left.encoding_name.c_str(), it->c_str()) == 0
          && strcasecmp(right.encoding_name.c_str(), it->c_str()) != 0)
        return true;
    ...
    if (m_it->type == MT_AUDIO && m_it->transport == TP_RTPAVP
      ...
      if (answer_media.payloads.empty()
          || ((answer_media.payloads.size() == 1)
              && (answer_media.payloads[0].encoding_name
                  == "telephone-event"))) {
...
int AmSession::onSdpCompleted(const AmSdp& local_sdp, const AmSdp& remote_sdp)
{
  ...
  if (local_sdp.media.empty() || remote_sdp.media.empty()) {
    ...
    ERROR("Local SDP:\n%s", debug_str.empty() ? "<empty>" : debug_str.c_str());
  ...
  //       remote_sdp.media[0].attributes.begin(),
...
void AmSession::onEarlySessionStart() { startMediaProcessing(); }

void AmSession::onSessionStart() { startMediaProcessing(); }
...
int AmSession::sendUpdate(const AmMimeBody* body, const string& hdrs)
{
...
void AmSession::onPrack2xx(const AmSipReply& reply) { /* TODO: SDP */}
...
void AmSession::onZRTPProtocolEvent(zrtp_protocol_event_t event,
...
void AmSession::onZRTPSecurityEvent(zrtp_security_event_t event,
                                    zrtp_stream_t*        stream_ctx)
{
...
int AmSession::writeStreams(unsigned long long int ts, unsigned char* buffer)
{
  ...
  AmRtpAudio* stream = RTPStream();
  if (stream->sendIntReached()) { // FIXME: shouldn't depend on checkInterval
                                  // call before!
    unsigned int f_size = stream->getFrameSize();
    int          got    = 0;

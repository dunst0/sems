...
int AmAudioRtpFormat::setCurrentPayload(Payload pl)
{
  if (this->codec_id != pl.codec_id) {
    DBG("setCurrentPayload({%u, '%s', %u, %u, %u, '%s'})\n", pl.pt,
        pl.name.c_str(), pl.clock_rate, pl.advertised_clock_rate, pl.codec_id,
        pl.format_parameters.c_str());
                    ...
                    int output_sample_rate, unsigned int nb_samples)
{
  ...
  if (ret < 0) return ret; // like nothing received?
  ...
  u_int32_t size = PCM16_S2B(playout_buffer->read(
      user_ts, (ShortSample*) ((unsigned char*) samples), nb_samples));
...
void AmRtpAudio::getSdpAnswer(unsigned int index, const SdpMedia& offer,
                              SdpMedia& answer)
{
...
int AmRtpAudio::init(const AmSdp& local, const AmSdp& remote,
...
int AmRtpAudio::setCurrentPayload(int payload)
{
  if (payload != this->payload) {
    ...
    int res =
        ((AmAudioRtpFormat*) fmt.get())->setCurrentPayload(payloads[index]);
                                      ...
                                      unsigned char* buffer)
{
  ...
  if (!use_default_plc) {
    ...
    s = default_plc(buffer, PCM16_S2B(ts_diff), fmt->channels, getSampleRate());
...
unsigned int AmRtpAudio::default_plc(unsigned char* out_buf, unsigned int size,

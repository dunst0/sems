...
bool AmUriParser::isEqual(const AmUriParser& c) const
{
  return (uri_user == c.uri_user)
         && (!strcasecmp(uri_host.c_str(), c.uri_host.c_str()))
         && (uri_port == c.uri_port);
}
    ...
    switch (s[p]) {
      ...
      case '<':
        switch (st) {
          ...
          case ST3: DBG("ERROR skip_uri(): Second < found\n"); return -1;
          case ST5: st = ST2; break;
          case ST6: st = ST4; break;
        }
        break;
      ...
      case '>':
        switch (st) {
          case ST1: DBG("ERROR skip_uri(): > is first\n"); return -2;
  ...
  while (pos < uri.length()) {
    ...
    switch (st) {
      ...
      case uSHDR: {
        switch (c) {
          case '>': {
            uri_headers = uri.substr(p1 + 1, pos - p1 - 1);
            st          = uS6;
            p1          = pos;
          } break;
          case '\t':
      ...
      case uSPARAM: {
        switch (c) {
          case '>': {
            uri_param = uri.substr(p1 + 1, pos - p1 - 1);
            st        = uS6;
            p1        = pos;
          } break;
          case '?': {
...
static void add_param(map<string, string>& params, const string& name,
                      const string& value)
{
    ...
    if (!quoted) {
      ...
      if (c == '\"') {
        ...
        if (st == pS1) {
          ...
          add_param(params, line.substr(p1, p2 - p1),
...
bool AmUriParser::parse_contact(const string& line, size_t pos, size_t& end)
{
  ...
  if ((size_t) p0 > pos) {
    ...
    // skip leading WS
    while (dn_b < line.size() && line[dn_b] == ' ') {
      dn_b++;
    }

    // skip trailing WS
    while (dn_e > 0 && line[dn_e - 1] == ' ') {
      dn_e--;
    }
                                            ...
                                            const string& param_list)
{
  ...
  if (list_of_params.empty()) {
    ...
    do {
      ...
      length =
          (end == string::npos) ? list_of_params.size() - start : end - start;
...
string AmUriParser::nameaddr_str() const
{
  ...
  for (map<string, string>::const_iterator it = params.begin();
       it != params.end(); it++) {
    res += ";" + it->first;
    if (!it->second.empty()) res += "=" + it->second;
  }

  return res;
}

string AmUriParser::print() const { return nameaddr_str(); }

...
AmAudioFormat::~AmAudioFormat() { destroyCodec(); }

void AmAudioFormat::setRate(unsigned int sample_rate) { rate = sample_rate; }
...
{
  return (codec && r.codec && (r.codec->id == codec->id)
          && (r.bytes2samples(1024) == bytes2samples(1024))
          && (r.channels == channels) && (r.rate == rate));
}
...
void AmAudioFormat::initCodec()
{
  ...
  if (codec && codec->init) {
    if ((h_codec = (*codec->init)(sdp_format_parameters.c_str(),
                                  &sdp_format_parameters_out, &fmt_i))
        == -1) {
...
long int AmAudioFormat::getHCodec()
{
  if (!codec) getCodec();
  return h_codec;
}
...
unsigned int AmLibSamplerateResamplingState::resample(unsigned char* samples,
                                                      unsigned int   s,
                                                      double         ratio)
{
  ...
  if (resample_state) {
    ...
    if (src_err) {
      ...
      signed short int* samples_s =
          (signed short int*) (unsigned char*) samples;
...
AmInternalResamplerState::AmInternalResamplerState()
    : rstate(NULL)
{
  rstate = ResampleFactory::createResampleObj(
      true, 4.0, ResampleFactory::INTERPOL_SINC, ResampleFactory::SAMPLE_MONO);
}

AmInternalResamplerState::~AmInternalResamplerState()
{
  if (rstate != NULL) ResampleFactory::destroyResampleObj(rstate);
}

unsigned int AmInternalResamplerState::resample(unsigned char* samples,
                                                unsigned int s, double ratio)
{
  if (rstate == NULL) {
  ...
  rstate->put_samples((signed short int*) samples, PCM16_B2S(s));
  s = rstate->resample((signed short int*) samples, ratio,
                       PCM16_B2S(s) * ratio);
...
int AmAudio::get(unsigned long long int system_ts, unsigned char* buffer,
                 int output_sample_rate, unsigned int nb_samples)
{
  ...
  size = resampleOutput((unsigned char*) samples, size, getSampleRate(),
                        output_sample_rate);

  if (size > 0) memcpy(buffer, (unsigned char*) samples, size);

  return size;
}

// returns bytes written, else -1 if error (0 is OK)
int AmAudio::put(unsigned long long int system_ts, unsigned char* buffer,
                 int input_sample_rate, unsigned int size)
{
  ...
  if (max_rec_time > -1 && rec_time >= max_rec_time) return -1;

  memcpy((unsigned char*) samples, buffer, size);
  size = resampleInput((unsigned char*) samples, size, input_sample_rate,
                       getSampleRate());
...
void AmAudio::stereo2mono(unsigned char* out_buf, unsigned char* in_buf,
                          unsigned int& size)
{
  short int* in  = (short int*) in_buf;
  short int* end = (short int*) (in_buf + size);
  short int* out = (short int*) out_buf;
...
int AmAudio::decode(unsigned int size)
{
  ...
  if (codec->decode) {
    s = (*codec->decode)(samples.back_buffer(), samples, s, fmt->channels,
                         getSampleRate(), h_codec);
    if (s < 0) return s;
    samples.swap();
  }
...
unsigned int AmAudio::resampleInput(unsigned char* buffer, unsigned int s,
                                    int input_sample_rate,
                                    int output_sample_rate)
{
  if ((input_sample_rate == output_sample_rate)
      && !input_resampling_state.get()) {
    return s;
  }
  ...
  return resample(*input_resampling_state, buffer, s, input_sample_rate,
                  output_sample_rate);
}

unsigned int AmAudio::resampleOutput(unsigned char* buffer, unsigned int s,
                                     int input_sample_rate,
                                     int output_sample_rate)
{
  if ((input_sample_rate == output_sample_rate)
  ...
  return resample(*output_resampling_state, buffer, s, input_sample_rate,
                  output_sample_rate);
}

unsigned int AmAudio::resample(AmResamplingState& rstate, unsigned char* buffer,
                               unsigned int s, int input_sample_rate,
                               int output_sample_rate)
{
  return rstate.resample((unsigned char*) buffer, s,
...
unsigned int AmAudio::scaleSystemTS(unsigned long long int system_ts)
{
  ...
  unsigned long long int user_ts =
      system_ts * ((unsigned long long int) getSampleRate() / 100)
      / (WALLCLOCK_RATE / 100);
...
int AmAudio::incRecordTime(unsigned int samples) { return rec_time += samples; }
...
void DblBuffer::swap() { active_buf = !active_buf; }

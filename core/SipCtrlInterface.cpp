...
int _SipCtrlInterface::alloc_udp_structs()
{
  ...
  udp_servers =
      new udp_trsp*[AmConfig::SIPServerThreads * AmConfig::SIP_Ifs.size()];

  if (udp_sockets && udp_servers) return 0;
...
int _SipCtrlInterface::init_udp_servers(int if_num)
{
  udp_trsp_socket* udp_socket = new udp_trsp_socket(
      if_num,
      AmConfig::SIP_Ifs[if_num].SigSockOpts
          | (AmConfig::ForceOutboundIf ? trsp_socket::force_outbound_if : 0)
          | (AmConfig::UseRawSockets ? trsp_socket::use_raw_sockets : 0),
...
int _SipCtrlInterface::alloc_tcp_structs()
{
  ...
  if (tcp_sockets && tcp_servers) return 0;
...
int _SipCtrlInterface::init_tcp_servers(int if_num)
{
  ...
  tcp_socket->set_connect_timeout(
      AmConfig::SIP_Ifs[if_num].tcp_connect_timeout);
  ...
  if (file_exists(cfgfile) && !cfg.loadFile(cfgfile)) {
    ...
    if (cfg.hasParameter("default_bl_ttl")) {
      trans_layer::default_bl_ttl =
          cfg.getParameterInt("default_bl_ttl", trans_layer::default_bl_ttl);
    }
...
{
  return trans_layer::instance()->cancel(tt, stl2cstr(dialog_id), inv_cseq,
                                         stl2cstr(hdrs));
}
                            ...
                            unsigned int flags, msg_logger* logger)
{
  ...
  if (!req.body.empty()) {
    ...
    msg->content_type =
        new sip_header(0, SIP_HDR_CONTENT_TYPE, stl2cstr(content_type));
    msg->hdrs.push_back(msg->content_type);
    req.body.print(body);
    msg->body = stl2cstr(body);
  }

  int res = trans_layer::instance()->send_request(
      msg, &req.tt, stl2cstr(dialog_id), stl2cstr(next_hop), out_interface,
      flags, logger);
  delete msg;
...
void _SipCtrlInterface::stop() { stopped.set(true); }
                            ...
                            msg_logger* logger)
{
  ...
  return trans_layer::instance()->send_reply(&msg, (trans_ticket*) &rep.tt,
                                             stl2cstr(dialog_id),
                                                  ...
                                                  AmSipRequest&       req)
{
  ...
  if (msg->content_type) {
    if (req.body.parse(c2stlstr(msg->content_type->value),
                       (unsigned char*) msg->body.s, msg->body.len)
        < 0) {
  ...
  for (list<sip_header*>::const_iterator it = msg->hdrs.begin();
    ...
    switch ((*it)->type) {
      ...
      case sip_header::H_REQUIRE:
        req.hdrs +=
            c2stlstr((*it)->name) + ": " + c2stlstr((*it)->value) + CRLF;
        break;
      ...
      case sip_header::H_MAX_FORWARDS:
        if (!str2int(c2stlstr((*it)->value), req.max_forwards)
            || (req.max_forwards < 0) || (req.max_forwards > 255)) {
          trans_layer::instance()->send_sf_error_reply(
              &tt, msg, 400, "Incorrect Max-Forwards");
          return false;
        }
        break;
    }
  ...
  if (req.max_forwards < 0) req.max_forwards = AmConfig::MaxForwards;
...
inline bool _SipCtrlInterface::sip_msg2am_reply(sip_msg* msg, AmSipReply& reply)
{
  ...
  if (msg->local_socket) reply.trsp = msg->local_socket->get_transport();

  return true;
}

#define DBG_PARAM(p) DBG("%s = <%s>\n", #p, p.c_str());
...
void _SipCtrlInterface::handle_sip_request(const trans_ticket& tt, sip_msg* msg)
{
  ...
  if (!sip_msg2am_request(msg, tt, req)) return;
  ...
  DBG("^^ M [%s|%s] Ru SIP request %s handled ^^\n", req.callid.c_str(),
      req.to_tag.c_str(), req.method.c_str());
}
...
void _SipCtrlInterface::handle_sip_reply(const string& dialog_id, sip_msg* msg)
{
  ...
  DBG("^^ M [%s|%s] ru SIP reply %u %s handled ^^\n", reply.callid.c_str(),
      reply.from_tag.c_str(), reply.code, reply.reason.c_str());
}
                                             ...
                                             sip_trans* tr, trans_bucket* buk)
{
  ...
  switch (evt) {
    ...
    default: ERROR("BUG: unexpected timout event type '%d'.\n", evt); return;
  }
...
void _SipCtrlInterface::prepare_routes_uac(const list<sip_header*>& routes,
                                           string&                  route_field)
{
...
void _SipCtrlInterface::prepare_routes_uas(const list<sip_header*>& routes,
                                           string&                  route_field)
{
  if (!routes.empty()) {

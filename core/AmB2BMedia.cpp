...
void AudioStreamData::initialize(AmB2BSession* session)
{
  ...
  if (session->getEnableDtmfRtpDetection()) stream->force_receive_dtmf = true;
    ...
    , incoming_payload(UNDEFINED_PAYLOAD)
{
  if (session)
    ...
    stream = NULL; // not initialized yet
}
...
void AudioStreamData::changeSession(AmB2BSession* session)
{
  if (!stream) {
    ...
    if (session) {
      ...
      clear(); // free the stream and other stuff because it can't be used
               // anyway
  }
}
...
{
  // TODO: optimize: clear & create the dtmf_detector only if the dtmf_sink
  // changed
  clearDtmfSink();
  ...
  if (dtmf_sink && stream) {
    ...
    dtmf_detector->setInbandDetector(AmConfig::DefaultDTMFDetector,
...
bool AudioStreamData::initStream(PlayoutType playout_type, AmSdp& local_sdp,
                                 AmSdp& remote_sdp, int media_idx)
{
  resetStats();
  ...
  // TODO: try to init only in case there are some payloads which can't be
  // relayed
  stream->forceSdpMediaIndex(media_idx);

  stream->setOnHold(
      false); // just hack to do correctly mute detection in stream->init
...
void AmB2BMedia::changeSessionUnsafe(bool a_leg, AmB2BSession* new_session)
{
  ...
  bool needs_processing =
      a && b && a->getRtpRelayMode() == AmB2BSession::RTP_Transcoding;
  ...
  for (RelayStreamIterator j = relay_streams.begin(); j != relay_streams.end();
       ++j) {
    AmRtpStream& a = (*j)->a;
    AmRtpStream& b = (*j)->b;
...
int AmB2BMedia::writeStreams(unsigned long long ts, unsigned char* buffer)
{
  ...
  AmLock lock(mutex);
  for (AudioStreamIterator i = audio.begin(); i != audio.end(); ++i) {
    if (i->a.writeStream(ts, buffer, i->b) < 0) {
      res = -1;
      break;
    }
    if (i->b.writeStream(ts, buffer, i->a) < 0) {
      res = -1;
      break;
    }
  }
...
{
  return (m.transport == TP_RTPAVP) || (m.transport == TP_RTPSAVP)
                                          ...
                                          const string& relay_public_address)
{
  ...
  SdpConnection orig_conn =
      parser_sdp.conn; // needed for the 'quick workaround' for non-audio media
      ...
      if (it->port) { // if stream active
        ...
        try {
          if (a_leg) {
            ...
            replaceRtcpAttr(*it, relay_address,
                            audio_stream_it->a.getLocalRtcpPort());
          }
          else {
            audio_stream_it->b.setLocalIP(relay_address);
            it->port = audio_stream_it->b.getLocalPort();
            replaceRtcpAttr(*it, relay_address,
                            audio_stream_it->b.getLocalRtcpPort());
          }
      ...
      if (it->port) { // if stream active
        ...
        try {
          if (a_leg) {
            ...
            replaceRtcpAttr(*it, relay_address,
                            (*relay_stream_it)->a.getLocalRtcpPort());
          }
          else {
            ...
            replaceRtcpAttr(*it, relay_address,
                            (*relay_stream_it)->b.getLocalRtcpPort());
          }
          if (!replaced_ports.empty()) replaced_ports += "/";
          replaced_ports += int2str(it->port);
        }
...
void AmB2BMedia::updateStreamPair(AudioStreamPair& pair)
{
  ...
  if (pair.b.getInput())
    pair.a.setRelayStream(NULL); // don't mix relayed RTP into the other's input
  else
  ...
  if (pair.a.getInput())
    pair.b.setRelayStream(NULL); // don't mix relayed RTP into the other's input
  else
...
void AmB2BMedia::updateAudioStreams()
{
  ...
  TRACE("handling SDP change, A leg: %c%c, B leg: %c%c\n",
        have_a_leg_local_sdp ? 'X' : '-', have_a_leg_remote_sdp ? 'X' : '-',
  ...
  if (m.port) {
    if (session) {
      ...
      stream->setRtpRelayTransparentSeqno(
          session->getRtpRelayTransparentSeqno());
      stream->setRtpRelayTransparentSSRC(session->getRtpRelayTransparentSSRC());
                               ...
                               const AmSdp& remote_sdp, RelayController* ctrl)
{
  TRACE("%s (%c): updating streams with local & remote SDP\n",
        a_leg ? (a ? a->getLocalTag().c_str() : "NULL")
  ...
  for (vector<SdpMedia>::const_iterator m = remote_sdp.media.begin();
    ...
    if (m->type == MT_AUDIO) {
      ...
      if (a_leg) {
        ...
        updateRelayStream(&relay_stream.a, a, connection_address, *m,
                          &relay_stream.b);
      }
      else {
        DBG("updating B-leg relay_stream");
        updateRelayStream(&relay_stream.b, b, connection_address, *m,
                          &relay_stream.a);
      }
      ++rstream;
    }
  ...
  try {
    ...
    for (vector<SdpMedia>::iterator m = sdp.media.begin(); m != sdp.media.end();
      ...
      if (m->type == MT_AUDIO && as != audio.end()) {
        ...
        if (a_leg)
          as->a.getSdpOffer(as->media_idx, *m);
        else
          as->b.getSdpOffer(as->media_idx, *m);
        ++as;
      }
      else {
                                  ...
                                  bool use_zero_con)
{
  ...
  for (m = offer.media.begin(); m != offer.media.end(); ++m) {
    ...
    if (media.type != MT_AUDIO) {
      ...
      media.port = 0;
      continue;
    } // copy whole media line except port
    if (m->port == 0) {
      media = *m;
      ++i;
      continue;
    } // copy whole inactive media line

    if (a_leg)
      i->a.getSdpAnswer(i->media_idx, *m, media);
    else
      i->b.getSdpAnswer(i->media_idx, *m, media);

    media.send =
        false; // should be already because the stream should be on hold
...
void AmB2BMedia::debug()
{
  ...
  DBG("B2B media session %p ('%s' <-> '%s'):", this,
      a ? a->getLocalTag().c_str() : "?", b ? b->getLocalTag().c_str() : "?");
  DBG("\tOA status: %c%c / %c%c", have_a_leg_local_sdp ? 'X' : '-',
      have_a_leg_remote_sdp ? 'X' : '-', have_b_leg_local_sdp ? 'X' : '-',

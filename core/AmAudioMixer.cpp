...
AmAudioMixer::AmAudioMixer(int external_sample_rate)
{
  ...
  sink_connector = new AmAudioMixerConnector(mixer, sink_channel, NULL,
                                             &srcsink_mut, &sinks);
}
...
AmAudio* AmAudioMixer::addSource(int external_sample_rate)
{
  ...
  AmAudioMixerConnector* conn = new AmAudioMixerConnector(
      mixer, src_channel, sources.empty() ? sink_connector : NULL);
                               ...
                               unsigned int nb_samples)
{
  ...
  mixer.GetChannelPacket(channel, system_ts, buffer, nb_samples,
                         mixer_sample_rate);
                               ...
                               unsigned int size)
{
  ...
  if (mix_channel != NULL) {
    ...
    mix_channel->get(system_ts, (unsigned char*) mix_buffer, input_sample_rate,
                     size);
  }
  return size;
}

...
/** @def AMCI_FMT_FRAME_LENGTH  frame length in ms (for framed codecs; must be
 * multiple of 10) see codec_init */
 ...
 *
 * @param format_parameters      [in]  parameters as passed by fmtp tag, NULL if
 none * @param format_parameters_out  [out] parameters passed back to fmtp, NULL
 if none * @param format_description     [out] pointer to describing block, with
 amci_codec_fmt_info_t array; zero-terminated. NULL if none *
 <table><tr><td><b>key</b></td><td><b>value</b></td></tr> *
 <tr><td>AMCI_FMT_FRAME_LENGTH (1)</td><td>  frame length in ms (for framed
 codecs; must be multiple of 10)</td></tr> *     <tr><td>AMCI_FMT_FRAME_SIZE
 (2)</td><td>  frame size in samples</td></tr> *
 <tr><td>AMCI_FMT_ENCODED_FRAME_SIZE (3)</td><td>  encoded frame
 size</td></tr></table> * @return -1 if failed, else some handle which will be
...
/**
 * \brief Codec's function for negotiating codec format; this needs to be
 * dry-run, i.e. no codec instantiated
 */
...
#define BEGIN_EXPORTS(name, module_load, module_destroy)                       \
  struct amci_exports_t amci_exports = {name, module_load, module_destroy,
 ...
 * @hideinitializer
 */
#define CODEC(id, intern2type, type2intern, plc, init, destroy, bytes2samples, \
              samples2bytes)                                                   \
  {id,      intern2type,   type2intern,   plc, init,                           \
   destroy, bytes2samples, samples2bytes, 0},

/**
   A codec with negotiate_fmt function
   @hideinitializer
 */
#define CODEC_WITH_FMT(id, intern2type, type2intern, plc, init, destroy,       \
                       bytes2samples, samples2bytes, negotiate_fmt)            \
  {id,      intern2type,   type2intern,   plc,          init,                  \
   destroy, bytes2samples, samples2bytes, negotiate_fmt},
...
#define END_FILE_FORMATS                                                       \
  {                                                                            \
    0, 0, 0, 0, 0, 0, 0, (struct amci_subtype_t[])                             \
    {                                                                          \
      {                                                                        \
        -1, 0, -1, -1, -1                                                      \
      }                                                                        \

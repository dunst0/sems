...
void PayloadMask::set_all() { memset(bits, 0xFF, sizeof(bits)); }
...
int AmRtpStream::hasLocalSocket() { return l_sd; }
...
void AmRtpStream::setLocalPort(unsigned short int p)
{
  ...
  if (setsockopt(l_sd, SOL_SOCKET, SO_REUSEADDR, (void*) &true_opt,
                 sizeof(true_opt))
      == -1) {
    ERROR("%s\n", strerror(errno));
    close(l_sd);
...
int AmRtpStream::compile_and_send(const int payload, bool marker,
                                  ...
                                  unsigned int size)
{
  ...
  if (session && session->enable_zrtp) {
    ...
    zrtp_status_t status = zrtp_process_rtp(
        session->zrtp_session_state.zrtp_audio, (char*) rp.getBuffer(), &size);
    switch (status) {
...
AmRtpStream::AmRtpStream(AmSession* _s, int _if)
    ...
    , active(false)
    , // do not return any data unless something really received
    mute(false)
...
{
  if (!l_rtcp_port) setLocalPort();

  return l_rtcp_port;
}

int AmRtpStream::getRPort() { return r_port; }
...
void AmRtpStream::handleSymmetricRtp(struct sockaddr_storage* recv_addr,
                                     bool                     rtcp)
{
  if ((!rtcp && passive) || (rtcp && passive_rtcp)) {
    ...
    if ((!rtcp && (port != r_port)) || (rtcp && (port != r_rtcp_port))
        ...
        || ((recv_addr->ss_family == AF_INET6)
            && (memcmp(&in6_addr->sin6_addr, &in6_recv->sin6_addr,
...
void AmRtpStream::setPassiveMode(bool p)
{
  ...
  if (p) {
    DBG("The other UA is NATed or passive mode forced: switched to passive "
        "mode.\n");
  }
  else {
...
void AmRtpStream::getSdpAnswer(unsigned int index, const SdpMedia& offer,
                               SdpMedia& answer)
{
  sdp_media_index = index;
  getSdp(answer);
...
int AmRtpStream::init(const AmSdp& local, const AmSdp& remote,
                      bool force_passive_mode)
{
  ...
  // second pass on remote SDP - initialize payload IDs used by remote
  // (remote_pt)
  ...
  while (sdp_it != remote_media.payloads.end()) {
    ...
    if (sdp_it->encoding_name.empty()
        || (local_media.transport == TP_RTPAVP && sdp_it->payload_type < 20)) {
      ...
      for (p_it = payloads.begin(); p_it != payloads.end(); ++p_it) {
        if (!strcasecmp(p_it->name.c_str(), sdp_it->encoding_name.c_str())
            && (p_it->advertised_clock_rate
                == (unsigned int) sdp_it->clock_rate)) {
...
{
  DBG("RTP Stream instance [%p] resuming (receiving=true, clearing "
      "biffers/TS/TO)\n",
      this);
  clearRTPTimeout();
  receive_mut.lock();
  mem.clear();
  receive_buf.clear();
  while (!rtp_ev_qu.empty()) rtp_ev_qu.pop();
  receive_mut.unlock();
  receiving = true;
...
void AmRtpStream::setOnHold(bool on_hold) { hold = on_hold; }

bool AmRtpStream::getOnHold() { return hold; }
...
void AmRtpStream::bufferPacket(AmRtpPacket* p)
{
  ...
  if (relay_enabled) { // todo: ZRTP
    ...
    if (relay_raw || (is_dtmf_packet && !active)
      ...
      if (NULL != relay_stream && (!(relay_filter_dtmf && is_dtmf_packet))) {
  ...
  if (session && session->enable_zrtp) {
    ...
    zrtp_status_t status = zrtp_process_srtp(
        session->zrtp_session_state.zrtp_audio, (char*) p->getBuffer(), &size);
    switch (status) {
      case zrtp_status_ok: {
        ...
        if (p->parse() < 0) {
          ...
          if (p->payload == getLocalTelephoneEventPT()) {
            ...
            if (!receive_buf.insert(ReceiveBuffer::value_type(p->timestamp, p))
                     .second) {
              // insert failed
              mem.freePacket(p);
            }
...
void AmRtpStream::clearRTPTimeout() { gettimeofday(&last_recv_time, NULL); }

int AmRtpStream::getDefaultPT()
{
  for (PayloadCollection::iterator it = payloads.begin(); it != payloads.end();
       ++it) {
    // skip telephone-events payload
    if (it->codec_id == CODEC_TELEPHONE_EVENT) continue;

    // skip incompatible payloads
    PayloadMappingTable::iterator pl_it = pl_map.find(it->pt);
    if ((pl_it == pl_map.end()) || (pl_it->second.remote_pt < 0)) continue;
...
{
  if (!receiving && !passive) return RTP_EMPTY;
  ...
  if (monitor_rtp_timeout && AmConfig::DeadRtpTime && (diff.tv_sec > 0)
...
void AmRtpStream::recvRtcpPacket()
{
  ...
  else if (!recved_bytes)
    return;

  static const cstring empty;
  if (logger)
    logger->log((const char*) buffer, recved_bytes, &recv_addr, &l_rtcp_saddr,
                empty);
  ...
  if (!relay_enabled || !relay_stream || !relay_stream->l_sd) return;
  ...
  if (AmConfig::UseRawSockets) {
    ...
    err = sendto(relay_stream->l_rtcp_sd, buffer, recved_bytes, 0,
                 (const struct sockaddr*) &rtcp_raddr, SA_len(&rtcp_raddr));
  }
  ...
  if (logger)
    logger->log((const char*) buffer, recved_bytes, &relay_stream->l_rtcp_saddr,
                &rtcp_raddr, empty);
}
...
{
  DBG("%sabled RTP relay filtering of RTP DTMF (2833 / 4733) for RTP stream "
      "instance [%p]\n",
      filter ? "en" : "dis", this);
...
{
  if (n_used >= MAX_PACKETS) return NULL; // full

  while (used[cur_idx]) cur_idx = (cur_idx + 1) & MAX_PACKETS_MASK;
...
#define BOOL_STR(b) ((b) ? "yes" : "no")
  ...
  if (hasLocalSocket() > 0) {
    DBG("\t<%i> <-> <%s:%i>", getLocalPort(), getRHost().c_str(), getRPort());
  }
  else {
    DBG("\t<unbound> <-> <%s:%i>", getRHost().c_str(), getLocalPort());
  }

  if (relay_stream) {
    DBG("\tinternal relay to stream %p (local port %i)", relay_stream,
        relay_stream->getLocalPort());
  }
  else
    DBG("\tno relay");

  DBG("\tmute: %s, hold: %s, receiving: %s", BOOL_STR(mute), BOOL_STR(hold),
      BOOL_STR(receiving));

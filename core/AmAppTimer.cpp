...
class app_timer : public timer
{
 ...
 public:
  ...
  // timer interface
  void fire() { AmAppTimer::instance()->app_timer_cb(this); }
};

class direct_app_timer : public timer
{
 public:
  DirectAppTimer* dt;
  ...
  void fire() { AmAppTimer::instance()->direct_app_timer_cb(this); }
};
...
app_timer* _AmAppTimer::erase_timer(const string& q_id, int id)
{
  ...
  if (it != user_timers.end()) {
    ...
    if (t_it != it->second.end()) {
      ...
      if (it->second.empty()) user_timers.erase(it);
    }
  }
...
void _AmAppTimer::setTimer(const string& eventqueue_name, int timer_id,
                           double timeout)
{
  // microseconds
  unsigned int expires;
  ...
  if (timeout < 0) { // in the past
    ...
    expires = (double) MAX_TIMER_SECONDS * 1000.0 * 1000.0
...
void _AmAppTimer::removeTimers(const string& eventqueue_name)
{
  ...
  if (it != user_timers.end()) {
    for (AppTimers::iterator t_it = it->second.begin();
         t_it != it->second.end(); t_it++) {
      if (NULL != t_it->second) remove_timer(t_it->second);
    }

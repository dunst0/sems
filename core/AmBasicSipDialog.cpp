...
AmSipRequest* AmBasicSipDialog::getUACTrans(unsigned int t_cseq)
{
  ...
  if (it == uac_trans.end()) return NULL;
...
AmSipRequest* AmBasicSipDialog::getUASTrans(unsigned int t_cseq)
{
  ...
  if (it == uas_trans.end()) return NULL;
...
{
  if (outbound_interface >= 0) return outbound_interface;
  ...
  if (get_local_addr_for_dest(dest_ip, local_ip) < 0) {
    ERROR("No local address for dest '%s' (local_tag='%s')", dest_ip.c_str(),
          local_tag.c_str());
    goto error;
  }
...
void AmBasicSipDialog::resetOutboundIf() { setOutboundInterface(-1); }
...
void AmBasicSipDialog::termUasTrans()
{
  while (!uas_trans.empty()) {
    ...
    DBG("terminating UAS transaction (%u %s)", req.cseq,
        req.cseq_method.c_str());

    reply(req, 481, SIP_REPLY_NOT_EXIST);

    it = uas_trans.find(req_cseq);
    if (it != uas_trans.end()) uas_trans.erase(it);
  }
}
...
{
  if (!onRxReplySanity(reply)) return;
  ...
  // final reply but not for 2xx INV reply (wait for 200 ACK)
  if ((reply.code >= 200)
      && ((reply.cseq_method != SIP_METH_INVITE) || (reply.code >= 300))) {
...
{
  if ((reply.code > 100) && (reply.code < 300) && !reply.to_uri.empty()
      && !reply.to_tag.empty()
      && (remote_uri.empty()
...
int AmBasicSipDialog::onTxReply(const AmSipRequest& req, AmSipReply& reply,
                                int& flags)
{
                            ...
                            const string& hdrs, int flags)
{
  ...
  if (t_it == uas_trans.end()) {
    ...
    ERROR("request cseq=%i; reply code=%i; callid=%s; local_tag=%s; "
          "remote_tag=%s\n",
          req.cseq, code, callid.c_str(), local_tag.c_str(),
          remote_tag.c_str());
                                  ...
                                  const string& hdrs, int flags)
{
  ...
  int res = SipCtrlInterface::send(
      ...
      outbound_interface, send_flags, logger);
  if (res) {

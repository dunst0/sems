...
FCTMF_SUITE_BGN(test_jsonarg)
{
  ...
  {
    string s = "{\"jsonrpc\": \"2.0\", \"id\": \"11\", \"error\": "
               "{\"message\": \"(1062, \\\"Duplicate entry '5447' for key "
               "'PRIMARY'\\\")\", \"code\": -32603}},";
    // DBG("s='%s'\n",s.c_str());
  ...
  {
    // string s = "{\"jsonrpc\": \"2.0\", \"result\": {\"timestamp\": 62,
    // \"analysis\": {}}, \"id\": \"11\"}";
    string s = "{\"result\": {}}";
    AmArg  rpc_params;
    fct_chk(json2arg(s.c_str(), rpc_params));
  ...
  FCT_TEST_BGN(json_number_e_powneg1)
  {
    ...
    AmArg rpc_params;
    fct_chk(json2arg(s.c_str(), rpc_params));
    // DBG("res: %s, type %d\n",  AmArg::print(rpc_params["result"]).c_str(),
    // rpc_params["result"].getType());
  ...
  FCT_TEST_BGN(json_tofro_equality)
  {
    ...
    // fct_chk(a1==a2);
    // DBG("a1 = '%s', a2 = '%s', \n", AmArg::print(a1).c_str(),
    // AmArg::print(a2).c_str());
  }
  FCT_TEST_END();
}
FCTMF_SUITE_END();

...
class SyslogLogFac : public AmLoggingFacility
{
 ...
 public:
  SyslogLogFac()
      : AmLoggingFacility("syslog")
      , facility(LOG_DAEMON)
  {
    init();
  }

  ~SyslogLogFac() { closelog(); }
...
bool SyslogLogFac::setFacility(const char* str)
{
  ...
  else if (strlen(str) == 6 && !strncmp(str, "LOCAL", 5) && isdigit(str[5])
           && str[5] - '0' < 8) {
...
void SyslogLogFac::log(int level, pid_t pid, pthread_t tid, const char* func,
                       const char* file, int line, char* msg)
{
  ...
  syslog(log2syslog_level[level], "%s: %s [#%lx] [%s %s:%d]",
         log_level2str[level], msg, (unsigned long int) tid, func, file, line);
#else
  syslog(log2syslog_level[level], "[#%lx] [%s, %s:%d] %s: %s",
         (unsigned long int) tid, func, file, line, log_level2str[level], msg);
#endif
#else /* NO_THREADID_LOG */
#ifdef LOG_LOC_DATA_ATEND
  syslog(log2syslog_level[level], "%s: %s [%s] [%s:%d]", log_level2str[level],
         msg, func, file, line);
#else
  syslog(log2syslog_level[level], "[%s, %s:%d] %s: %s", func, file, line,
...
#else /* !_DEBUG */
#ifdef LOG_LOC_DATA_ATEND
  syslog(log2syslog_level[level], "%s: %s [%s:%d]", log_level2str[level], msg,
         file, line);
#else
  syslog(log2syslog_level[level], "[%s:%d] %s: %s", file, line,
         log_level2str[level], msg);
#endif
 ...
 * Run log hooks
 */
void run_log_hooks(int level, pid_t pid, pthread_t tid, const char* func,
                   const char* file, int line, char* msg)
{
    ...
    if (begin_name && begin_offset && end_offset && begin_name < begin_offset) {
      ...
      *end_offset     = '\0';
      ...
      if (status == 0) {
        ...
        _LOG(ll, "%s : %s+%s", symbollist[i], funcname, begin_offset);
      }
      else {
        // demangling failed. Output function name as a C function with
        // no arguments.
        _LOG(ll, "%s : %s()+%s", symbollist[i], begin_name, begin_offset);
      }
    }
    else {
